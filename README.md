...
| | What | For whom |
|---|---|---|
| [`catalog/`](catalog/README.ja.md) (7 tables) | what exists and what it is called: families, series, parts, packages, cores, documents, mirror versions | the keys every other table joins on |
| [`evidence/`](evidence/README.ja.md) (32 tables) | what the documents say, **spelled as they spell it**, every row with its provenance (`basis`) and confidence (`confidence`) | anyone checking correctness |
...
[evidence/README.ja.md](evidence/README.ja.md) (what each table and column means),
...
- `tools/extract_*.py`, `tools/build_*.py`: extraction from EVT headers, datasheets and reference manuals, and table generation (run order in `evidence/README.ja.md`); `tools/build_index.py` derives the index
