...
review/    render_assets.py（**図のpixel描画**。原本hashを照合してから、図領域を
           ...
           **レジスタのbit図を組み直す**（`31 30 … 16`のbit番号行を表のヘッダ行へ
           ...
           （`Reser`+`ved`→`Reserved`）は連結し、**隣セルにも二重取りされた末尾断片は
           同ページの記述表のName列が否定するときだけ落とす**（`HSYNCSCS`→`HSYNCS`・
           `WWDG_STOPTOP`→`WWDG_STOP`。44セル。`PB11`/`ODR11`のような正当な数字末尾は
           名称列に在るので触らない）、TIMのCCMRのような出力名/入力名の
