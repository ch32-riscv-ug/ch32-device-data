...
11. **括弧付きの指数を指数のまま残す**（converter 1.9.2）。QingKe V3/V4/V5マニュアルの
    PMP表にあるNAPOT領域幅`2^(G+2)`が`2(G+2)`に潰れ、**掛け算に読めていた**（zh/en計6箇所）。
    括弧付きの上付きを**3つのゲートが揃って**拒んでいた——安価な前判定
    `has_subscript_shape`、`reattach_cell_subscripts`内のセル単位ゲート、そして
    「`^`の直後は英数字」という最後の歯止め。脚注番号と指数は**括弧の中身**で分ける
    （数字だけなら脚注、英字か演算子を含めば指数）。`^`を書くのは**基底が数字のときだけ**
    なので、`C_in（HSE）`のような括弧付き下付きには当たらない。

    同じ版で`clean_reset_column`が`…`を消さないようにした。「英数字を含まない値は説明列の
    残骸」という規則が、「行が続く」印の`…`を飲み込んでいた（QingKeの1ページだけで10セル）。
    1.9.1で`W1`/`R0`を消していたのと**同じ規則**で、白名簿と否定条件の両方が漏れうる。

12. **列境界は語に跨がれない溝であること**（converter 1.9.4）。1.9.3で見出し語に
    `产品特性`を足したところ、それまで分割されなかったページが**悪い位置で**分割される
    ようになった——x0のギャップ方式が右カラムの**内側**（字下げの段の間）を溝と誤り、
    右カラムの語33個を途中で切って、`crop`が跨いだ語を両側に入れるため切れ端が
    ページ末尾に積み上がった（CH32V205DS0.zh p1）。いまは候補を2つ出し
    （x0のギャップと、語の占有幅をx軸へ投影した空白帯）、**跨ぐ語が最小のもの**を採り、
    最良でも3語以上跨ぐなら分割しない。左カラムの一番長い語が溝に届くのは普通なので
    跨ぎ0は要求しない。

    **この欠陥の間、`check_markdown_parity`は68/68 cleanのままだった**——切れ端も実在する
    bundleの行で、順序も保たれていたため。「全行が順序どおり在るか」という機械検査は
    分割の取り違えを見られない。版面の画像と比べるしかない。

13. **表領域の外に落ちた最外列を取り込む**（converter 1.10.0）。罫線ベースの表検出が
    最外列の外枠を拾えないと、その列は表bboxの外に残る——しかも**行としても残らない**。
    その行の中心はbbox内なので`reading_order`から外れるからで、結果として値は
    **Markdownのどこにも出ていなかった**。CH32M030RM.zh p7の表は`名称/访问/描述/复位值`
    として出て、`位`列（`[31:15]`・`14`・`[13:12]`。x 78.5-115.2に対しbboxは121.8から）が
    消え、レジスタのフィールドがどのbitなのか読めなかった。`复位值`/`Reset value`列が
    右端の外へ押し出されるのは同じ欠陥の鏡像。

    bboxの外のグリフを**表自身の行境界**で束ねると1行帯にちょうど1つ入る——入れ先が
    一意で、それがそのまま判定条件になる。素朴な版は1,000表超に当たり、そのほぼ全部が
    図を表と誤検出したページの散乱ラベル（`XUSRAMMFS_DPUSBFSFS_DM`）だった。4条件で49表
    になる: 本物の罫線表（3行3列以上・セルの6割以上に中身）、外のグリフが1列ぶんの幅
    （表の30%以下）で縁に接する（隙間8pt以下）、**すべての**行帯に中身がある（穴のある列は
    曖昧なので触らない）、どの行帯も24字以内。`extracted_rows`は触らないので凍結tool 19本は
    この変更を見られない。

14. **走査線に刻まれたrasterを1枚に縫い直す**（converter 1.10.1）。PDFが別行立ての
    数式を**高さ0.72ptの帯277枚**で描くことがある。1枚ずつでは`render_assets`の大きさに
    届かないので、exporterが`<!-- image: … -->`を277行吐き、数式の中身は
    **Markdownに存在しなかった**（V205DS0.en p64・L103DS0.zh p49）。resource名を持たない
    小画像をy方向の帯でまとめ、10枚以上で囲いが縦横8pt以上なら1枚にする（278→2）。
    `render_assets`も**横長で面積のある**画像（幅80pt以上・高さ12pt以上・面積1500pt²以上）
    を描くようにした——別行立ての数式は167×24ptで、正方形の閾値40ptに永遠に届かない。

15. **順序が壊れたセルはグリフから組み直す**（converter 1.10.1）。脚注の上付きと下付きが
    同じ基底に付くとき、pdfplumberは脚注を基底の行・下付きを次の行に置くので
    `V_DD12A(1)`が`'V (1)\nDD12A'`として届く。**文字は全部あって順序だけが違う**ので
    挿し込みでは直せず、既存の歯止め（結果がグリフの読み順と一致すること）が正しく拒否
    する。そこで挿し込みが諦めたら、グリフを行→xに並べ直して組み直す。**間違える挿入
    位置が存在しない**のが要点で、以前の緩い fallback が`每2^20个`を`每2个…121pp^20m`に
    壊したのはまさにそこだった。歯止めは2つ——**24字以内のセルだけ**（グリフは空白を
    持たないので散文を組み直すと`Thedatabusisdriven`になる。419件中338件が該当）と、
    文字集合の完全一致。結果: `VDD12A(1)`・`IDD12A(1)(2)`・`VHSEH(1)`・`tSU(LSI)(1)`。

16. **caption の無い表をページ跨ぎで繋ぐ**（converter 1.10.2）。
    `continues_from_previous`は`bool(consecutive and previous_logical_id)`——
    *caption付き*の表を見たことが前提だったので、**caption の無い表は決して継続しな
    かった**。CH32H417DS0.enの比較表は2つの論理表に割れ、症状が3つ同時に出ていた:
    最終セルが`USBHS (USB`で切れ、次ページに`'2.0)'`だけの**幽霊行**（空のrowspan=3
    セルがPDUSBのグループ見出しを奪う）、そして刷り直されたヘッダが列を増やす。
    `fold_boundary_spills`と`drop_repeated_headers`は結合表の中でしか働かないので
    **どちらも届いていなかった**——1つの根が3つの欠陥に見えていた。繋ぐ条件は列数一致・
    **列境界が2pt以内で一致**・前の表が下端1/4で終わる・この表が上端1/3で始まる。
    全corpusで該当は**18組**（既に繋がっている5,734組に対して）で、どれも本物の継続表。
    `fold_boundary_spills`も「1列のセル」条件を外し、**直上のセルと列範囲が完全に一致**
    すればcolspanを許すようにした。`logical_id`が動いたが`operating_conditions`は
    2,796行でbyte一致のまま。

17. **ページ境界で切られた縦の結合セルを戻す**（`extend_boundary_spans`。exporter側）。
    背の高い結合セル（`VIL`が標準I/OとFT I/Oの両ブロックを覆う）は2つに切られ、前ページ側
    は下端で止まるセル、後ろ側は**中身の無いセル**になる（箱に上枠が無い）。結合表では
    別の2セルになり、後ろ側は`fold_boundary_spills`が畳む行から始まるため**描画時に落ち、
    行が1列足りなくなる**——H417DS0.en p104では「FT I/O pin, input low level voltage」の
    3行が7列でなく6列になり、条件がSymbol欄に出て**Unit欄が消えていた**。ページ境界から
    始まり・中身が空で・**直上の非空セルと列範囲が完全に一致**するとき、上へ吸収する。
    見える文字は変わらない（吸収するのは空セル）——変わるのは行の列数が揃うことと、
    `VIL`がrowspan=7になって原本の箱と一致することだけ。

18. **項目17の吸い上げを「自分の中身を持たない行」に限る**、そして**空になった境界行を
    グリッドから消す**（`drop_empty_boundary_rows`）。17の署名は「箱が切られた」場合と
    「**次ページの行が正当にその列だけ空**」の場合で見分けが付かず、後者に発火すると
    直上の値がその行に被さって**値の捏造**になった——`CH32M030DS2.zh` p3/p4で引脚25
    `ISP1`の行が`PA8`の`ADC_IN7/MCO/CMP3_OUT0/ISN1`と`SPI_MOSI_3`を持つ形で出ていた。
    全corpus実測: 発火11,015件のうち**1,185件が中身のある行**で、9,830件は
    `fold_boundary_spills`が畳んで空になった行。取りこぼす側（結合を戻さず空セルのまま）
    は列数が合っているので無害で、誤りの向きが非対称。
    もう一方の副作用は、畳んで吸い上げた後に**セルが1つも残らない行**が消えないこと。
    exporterには「畳んだ行はrowspanに覆われていなければ落とす」規則があったが、
    **覆うrowspanを作るのが17自身**なので必ず覆われて残り、空の`<tr>`と1つ多い
    rowspanになっていた（`VIL`が7対PDF 6、`PDUSB`が5対4、引脚8が3対2）。境界行かつ
    セル0個の**2,283件**をグリッドから消す（跨ぐセルの下端を詰め、以降の行を繰り上げる）。
    畳んだ行に残った空セル（`Typ.`のように直上も空で吸い上げ対象外になった列）も、
    下へ続くものは次の行から始まるセルへ読み替える。検算: タグを外した本文で旧出力と
    比較して11,695ファイル中1ファイルだけ差があり、**欠けた語0・増えた語0**。

19. **一覧セルの判定で括弧の直前の空白を数えない**（`_item_list_cell`。exporter側）。
    比較表の周辺機能欄が`ADTM3*GPTMCRC2*USARTSPII2CUSBDUSBFSCANRTC2*WDG2*OPA`という
    読めない1語になっていた。bundleのセル文字列は改行を保っており、潰していたのは
    折り返し結合。一覧判定が**先頭行`2*ADC (TKey)`の括弧前の空白1文字**で降りていた
    （zh版は`2*ADC（TKey）`で空白が無いので通っていた）。空白の条件をただ外すと315セルに
    広がり大半が折り返し本文（`ADON bit`／`set to 1 or`）で壊れるが、**括弧前だけ**なら
    24セルで、9件が目的の一覧・11件はブロック図ラベル・2件は狭い列の折り返し見出し。

20. **グリッドから丸ごと抜けた列を埋める**（`fill_grid_holes`。converter 1.11.0）。
    罫線が細いとpdfplumberはその列のセルを1つも作らない——`WCH-LinkUserManual.zh` p4の
    機能比較表（15行5列）は列0のセルがヘッダ行にしか無く、`RISC-V模式`・
    `ARM-SWD模式-HID设备`…**14個の行見出しがどのセルにも入っていなかった**。列0の座標は
    ヘッダのセルから、行の座標は同じ行の他のセルから決まるので入れ先は一意。条件は
    「**同じ列に3つ以上の穴があり、その3つ以上に文字が在る**」——散発の穴（図を表と
    誤検出したページのラベル断片。全corpus 90個/50表）を外すとこの1表だけになる。
    `recover_outer_column`（項目15）と同じ族なので同じ層に置き、`extracted_rows`は
    触らない。同じ表の✓/✗印は**グリフとして存在しない**（ベクタ描画）ので復元不能
    ——geometryの1,019グリフを数えてもその帯には1文字も無い。

21. **最外列の取り込み（項目15）を3つ緩めた**（converter 1.12.0）。`CH32L103RM.en` p13の
    「`Bit`列が空・幽霊列が在る・`[31:21]`が消えている」という3つの指摘は1つの根で、
    p13側の断片は表bboxが x=109.9 から始まり`Bit`列（x 60.5〜109.9）が表の外にあった。
    (a) **隙間の上限8ptを「縁までの全幅が1列ぶん」に置き換え**——隙間は「列幅−文字幅」で
    決まるので左寄せの広い列では大きくなり、閾値として筋が悪い。p13は**8.4ptで0.4pt
    足りずに落ちていた**。全幅で見ると図の断片（`['Syst','em','Bus']`。隙間219pt）は落ち、
    本物の`Bit`/`Reset value`列は通る。(b) **3行→2行**（bit説明表は「見出し＋1行」の
    断片になりやすい）。(c) **左右の両方**を見る（以前は片方を取り込んだ時点でreturnし、
    `Bit`と`Reset value`が両方外に出ている表で右が残っていた）。
    実測: 49表→**94表を追加**（左のみ47・右のみ34・両側13）で**408セル**。中身は全部
    `Bit`/`Reset value`/`复位值`/`位`/レジスタ名の完全な列。これらは行の中心が表bbox内の
    ため**`reading_order`からも外れ**、`CH32V003RM.en` p132では`Reset value 0xFFFF`が
    表の外に`Reset`/`value`/`0xFFF`/`F`と**値を分断された本文**として落ちていた。

22. **取り込んだ列の綴りを`cell_html`の折り返し判定へ渡す**（`spell_glyphs`）。
    表の外から取り込む列はpdfplumberのセル抽出を通らないので区切りが入らず、
    `Resetvalue`・`Filterregister0`になっていた。**視覚行の切れ目に`\n`を入れる**のが
    正しい——狭い列では見出しも値も折り返され、`Reset`/`value`は空白で繋ぐべきで
    `0xFFF`/`F`は繋いではいけない、という判定を`cell_html`が既に持っている
    （`'Reset\nvalue'`→`Reset value`、`'0xFFF\nF'`→`0xFFFF`、
    `'Filter register\n0'`→`Filter register 0`）。同じ視覚行の語間の空白は隙間から
    復元する（グリフ幅の中央値の0.35倍超・両側ASCII）——全corpus実測で35セルが変わり
    全部`Reset value`型の見出し、閾値0.25〜0.50で結果が同じ＝隙間が明確に分かれている。
    値のセルは1つも変わらない。

23. **斜めに割れた角セル/折り返し見出しの二重出力を落とす**
    （`strip_duplicated_span_lines`）。pdfplumberは斜めに割れた角セル
    （`Product model`／`Resource differences`）を**結合セルとして1つ**出したうえで
    **下の行にもう1つ**出すことがある。その行が列数を超えるので以降のセルが右へずれ、
    `CH32H417DS0.en` p3では`QEU6`〜`REU6`（型番の末尾）が`128`〜`60`（ピン数）の
    **2つ右**に出ていた——比較表の先頭という最も読まれる場所で型番を読み違える形。
    落とすのは「行が列数を超え・覆っている結合セルと**列範囲が重なり**・結合セルが
    **2行以内**でその1行と**完全一致**」のときだけ。**2行以内が要**——長い説明セルは
    次の行の文字まで飲み込むことがあり、そのときは小さいセルの方が本物なので消すと
    データが失われる（`QingKeV4_Processor_Manual.en` p37は`Floating-point unit status
    FS FS Meaning 00 OFF …`が9行ぶんを飲み、`FS Meaning`等が独立セルで在る。この型が
    40件）。全corpus実測: 部分一致1,273件→重なり179→列数超え149→**完全一致かつ2行以内
    74件・26文書**で、これは全部本物の二重出力。消しても同じ綴りが結合セルに残るので
