...
| `af-N` | The AF number is written alongside in the same column (`SDA(AF7)`) | Write N to that pad's AF number | No | 4,507 |
...
**One disagreement, and it is the RM's.** The CH32H417 RM writes step 8 of fast page programming as "set FTPG to start fast page programming", but `FTPG` is the enable bit it already set in step 4; the driver uses `CR_PG_STRT` and `register_fields.csv` has `PG_STRT` at bit 21, as on every other family of that system. **The columns therefore hold the driver's reading** (`fast page, direct writes (PAGE_PG, then PG_STRT)`) and the RM's wording is kept in `basis` as `!rm:program_method(...)` -- the same treatment as `fast_program_bytes` on CH32V103, where the judgement went the other way and the RM's value is the one in the column. `confidence` stays `conflict`, so a consumer that fails closed on conflicts is unaffected.
...
Definitions of the AFIO route selectors and the value → route mapping. A `remap-N` in pin_functions.csv is resolved by following remap_routes (selector × value → signal/pad) → remap_fields (which bits of which register). The source is candidates/ (a join of EVT headers + RM register tables + RM remap grid + datasheet pin tables), and **`confidence` says whether two documents agree**: `confirmed` where the datasheet's pin table and the reference manual both state the route (4,354 rows), `reference` where only one does (483). The manual may state it as a grid or inside a register-field description; which of the two does not change the count, because disagreement here is between documents rather than between a manual's editions -- no route in the corpus is stated differently by a manual's Chinese and English editions. Rows whose value was taken from the grid against the pin table stay `reference`: the pin table puts the signal on that pad but does not say that value (CH32V103's TIM3 writes `_1` on both PB4 and PC6, and the manual places them at 2 and 3). `remap_fields.csv` is still all `reference` -- its per-source record exists but promoting it is separate work. One row is `conflict`: where the RM's remap grid and the datasheet's pin table give the same pad a different selector value, the pin table's value is kept and the grid's is written into `basis` as `!rm-remap-grid(value=N)`, the same `!<source>(<column>=<value>)` DSL every other table uses. Today that is CH32M030's `ADC_ETR`: the manual's Table 6-15 puts `ADC_ETRGIN_RM=0` (default) on PB6 and `=1` on PA14, while the pin table has PA14 as the default and PB6 as `ADC_ETR_1`. Both editions of each document agree with themselves, so this is a disagreement between documents, not between translations. `index/conflicts.csv` carries it with the grid's value in `alternative`. The H41x/X315 line uses AF numbers rather than remap and is out of scope (held by `af-N` in pin_functions).

Three columns need care when reading.

**The `:xx` tag names the manual editions that stated the route.** It used to be a hardcoded `:en` on every row even though both editions are read, so routes only the Chinese edition states claimed English (503 description-derived routes, and 29 grid-derived routes are English-only). Rows with no manual source carry no tag.
...
**different fact from the recommended operating range** in `operating_conditions.csv`, so it
is a separate table. Both have `symbol == "V_DD"` with a `min` and a `max`, but they mean
opposite things, and mixed together they cannot be told apart (when they were mixed once, the
supply voltage in `index/parts.csv` for `CH32L103` turned from `1.8..3.6V` into `-0.3..4.0V`).
Generated by `pipeline/extract/datasheet/extract_absolute_maximum.py`.

Only the datasheet's absolute-maximum table is read (`表3-1 绝对最大值参数表` /
`Table 3-1 Absolute maximum ratings`; the chapter number is 3-1 in some documents and 4-1 in
others). Measured over the whole corpus: **33 tables in 33 editions, one per document, and all
16 documents that have both editions are 1/1 symmetric.** The table's shape differs from
`operating_conditions`: there is **no `condition` header and no `typ` column** -- the condition
is written in an unnamed column, and some editions do not even have that column. Hence no
`typ` column here.

Symbols, values and prose are read the same way as in `operating_rows.py` (subscript repair,
and the check that the unit matches the quantity the symbol's stem claims, `UNIT_FOR`).

**Not collected**:

- **rows whose symbol does not name a physical quantity** (60 of the 310 rows in the English
  editions): `∑I_INJ(PIN)` (total injected current over all I/Os), the `|△V_DD_x|` family
  (voltage differences between like supply pins), and spellings broken by a `△` that landed on
  a line of its own
- **rows only the Chinese edition has** (12): the displayed text is taken from the English
  edition by design
...
wake-up-time rows; the base rows come from `pipeline/extract/datasheet/operating_rows.py`,
the port of the frozen `tools/build_operating.py`, which was retired on 2026-09-09).
...
uv run pipeline/extract/datasheet/extract_absolute_maximum.py  # absolute_maximum_ratings (the absolute-maximum table only; a different fact from operating_conditions)
...
uv run pipeline/extract/rm/extract_opa_cmp_registers.py   # opa_cmp_registers (EVT headers + RM register tables, from bundles. Reads the whole RM; minutes)
uv run pipeline/extract/rm/extract_clock_enables.py       # clock_enables (EVT rcc.h + RM register tables, from bundles. Reads the whole RM; minutes)
...
uv run pipeline/extract/rm/extract_usbpd_plumbing.py      # usbpd_plumbing (after clock_enables. EVT headers + RM, from bundles)
uv run pipeline/extract/rm/extract_registers.py  # register_blocks/registers/register_fields + index/register_layouts (EVT headers + whole RM, from bundles; about 16 minutes). No cache -- a stale --rm-cache once rolled the canonical back to a pre-revision RM reading
uv run pipeline/extract/rm/extract_flash_program_method.py # flash_program_method (after register_fields, for ctlr_bit_names. EVT flash driver + RM flash chapter, from bundles)
