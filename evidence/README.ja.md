...
**型番の図がデータシートのどの版面に在るか。** 1行が `(part_number, kind)` で、`kind` はいまの
ところ `pinout` だけ。生成 README はこれを `#page=N` のリンクにするので、パッケージ対応表から
辿った読者が表紙ではなく図そのものに着く。

**`page_zh` と `page_en` を別の列に持つのは、両版で割り付けが違うから**（`CH32V103R8T6` の
ピン配置図は中文版 p.11・英語版 p.13）。食い違いではないので、この表は構造上 `conflict` を
持たない——`confirmed` は両版が図を描いていること、`reference` は片版だけということ。いまは
目録の 103 型番すべてが `confirmed`。

**1枚の図が複数の型番を代表する。** 図の見出しは完全な型番とは限らず、伏字（`CH32V103Cx` は
C6T6・C8T6・C8U6 を覆う）・温度グレードの桁を省いた形（`CH32V006E8R`）・スラッシュで連ねた形
（`CH32V303RxT6/CH32V303RCT7`）がある。目録の型番へ展開して1型番1行にし、同じページを指す。

**ページは見出しが載っている版面**で、ピン配置の章（`2.1 引脚排列` / `2.1 Pinouts`）を
**行の順**に歩いて決める。次の節の見出しより上に在る図はまだこの章のもの——`CH32V003` の SOP8
図は、下半分が `2.2 引脚描述` であるページの一番上に在る。
