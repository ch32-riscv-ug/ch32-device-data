...
（208行）。証拠は食い違いを片方に寄せず両論を残す規則ですが、その記録は11の表に散っていて、
...
**132行に相手の値が入り、76行は空**です。`memory_configs`（67行）と `timers`（1行）は
...
いま残る20行は**原文で裁定した資料側の齟齬**です。中文版が異を唱えている行の `alternative` は、
