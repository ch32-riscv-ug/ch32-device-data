...
families.csv   11行  ファミリー一覧（mirror repository = 文書の単位）
  └ series.csv   27行  シリーズ（die）。core・ISA・共通スペック
      └ products.csv  103行  注文型番。packageごとに変わる値
          └ （今後）pins            pin表・route selector
```
...
1行1注文型番。package・flash・pin数などの実値。`listed_as`は比較表での略記（`CH32V208CB`→`CH32V208CBU6`、ワイルドカード`C6x6`→C6T6/C6U6）。
...
- **列順**: 左から重要な値（識別子 → スペック → package詳細 → 出典）。その後ろに`*_confidence`ブロック、`*_basis`ブロックを同じ順で並べます
