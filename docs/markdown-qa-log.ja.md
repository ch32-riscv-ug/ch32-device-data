...
`CCRCFAIL`（3文書）と `CMDSENTC`（2文書）だけが残る。SDIO の状態レジスタの
フィールド名で、**そのページの記述表に載っていない**（`CH32FV2x_V3xRM.en` p588 の
`description_names` は `CEATAEND`・`DATACOUNT`・`Reserved` の4件だけ）。同じ章の別ページの
記述表には在るはずなので、名前の出所を「同じレジスタ章」まで広げれば解ける見込み。
**広げる範囲は別に測ってからにする**——名前源が広がると偽の候補も増える。
