...
`strip_straddling_dupes` / `strip_boundary_dupes` / `clean_reset_column` /
`split_line_merges`（合計9,288箇所）は**今回は動かしていません**。上の4つと違い、
どれも**グリフを消す/行を組み替える判断**を含み、しかも `option_bytes`・
`debug_wiring`・`operating_conditions` が読むセルに直接効きます。1.7.0で
「繋いだだけで1,207行が消えた」のと同じ形の事故を起こしうるので、**先に
「その形に依存している消費者が居ないか」を1つずつ測ってから**にします。
再挑戦の条件: 各修復について (1) 変わるセルを列単位で数え、(2) それを読む抽出器の
出力差分を`--out`で取り、(3) 差分が改善だと確認できたとき。
