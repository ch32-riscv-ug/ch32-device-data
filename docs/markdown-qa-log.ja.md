...
## 縦に結合された値のセルが、覆われた行で消えていた（2026-09-10）

「続きの断片で単位が落ちる」の再突入条件として「値を1つも持たない行は出さない」を決めようと
して、**その4行が雑音ではなく取りこぼしだった**と分かった。

資料は「2つの条件で同じ値」を**値の欄の縦結合**で書く:

| 文書 | 行 | 共有している値 |
|---|---|---|
| `CH32H417DS0` p.104/p.90 | `t_erase_32k` の `DBMODE = 0/1`（32K/64K bytes） | `3` / `10 ms` |
| 同 | `t_erase_sec` の `DBMODE = 0/1`（4K/8K bytes） | `3` / `10 ms` |
| `CH32V103DS0` p.22/p.20 | `V_DDA` の「ADCを使う／使わない」 | `5.5 V`（min は行ごと） |
| `CH32V203DS0` p.39/p.33・`CH32V208DS0` p.27/p.23 | 同 | `2.4` / `3.6 V` |

`extracted_rows` は結合セルを**先頭の行にだけ**置くので、覆われた行が値ゼロになる。
**5文書とも原本を描画して版面が縦結合であることを確かめた**（今日の教訓——文字層だけを
見ても版面との食い違いは見えない——をそのまま適用した）。

`fill_rowspans` で **min/typ/max の欄だけ**写す。記号と項目名は呼ぶ側が別の継ぎ方をしていて
（記号は空欄なら前の行、項目名は空欄でなければ更新）、ここで写すと二重になる。

全corpus実測で**10行・18セル・5文書の zh/en 対称**。正本は**3,178行のまま5行が埋まり、
値ゼロの行が 4 → 0**（confidence 分布は不変）。生成器を `--out` で走らせた出力と正本が
byte 一致することも確かめた。

## 続きの断片へ記号・単位・項目名を持ち越した——`V_PVD` の17行が入った（2026-09-10）

表の途中でページが変わると、そこから**単位の欄が空になる**（資料は「上の行と同じ」を空欄で
書く）。`read_edition` は表ごとに状態を捨てるので、続きの断片で単位が落ち、`UNIT_FOR` の検査に
掛かって**行ごと消えていた**。

素直に持ち越すと**15行の「値ゼロ行」**を連れてきた（それまで単位が空で落ちていた行が、
継いだ単位で通ってしまう）。歯止めが2つ要った:

1. **min/typ/max がどれも空の行は出さない**。前項で縦結合の取りこぼしを直して正本の値ゼロ行が
   0 になったので、この規則は「増えた行が値を持つか」の検査として働く。
2. **続きの断片の1行目は項目名の後半**——`t_CONV` の `Total conversion time` は前ページで切れ、
   断片には `(including sampling time)` だけが載る。置き換えると項目名が断片になるので繋ぐ。
   小文字か括弧で始まるものだけ（`fold_header_wrap` と同じ見分け方）。

### 増えた32行

| 記号 | 件数 | 中身 |
|---|---:|---|
| `V_PVD` | 17 | PLS レベルの閾値（V002 6・V003 3・V103 8）。**en 版だけが表をページで割っていて後半が読めていなかった**——値は zh 版と完全一致 |
| `t_Iatr` | 5 | 通常トリガの変換遅延 |
| `t_CONV` | 3 | 総変換時間 |
| `ACC_HSI` | 2 | HSI の確度 |
| ほか | 5 | `V_IOFFSET`・`V_OH`（単位が付いた）・`N_END`・`C_ADC`・`T_J` |

**32行すべて、引用ページの1行に値が揃うことを機械確認**（confirmed 行は zh/en 両版で）。

### 新しい conflict 1件は資料側

`T_J`（結温度範囲）の「型号末位为7の芯片」が **en 105℃ / zh 115℃**。en 版 `CH32V007DS0` p.41 を
描画して 105 を確認した。周囲の環境温度は両版とも 105 なので zh の 115 のほうがもっともらしいが、
...
## confidence の内訳を検査に載せた——5表の数が古かった（2026-09-10）

「conflict が起きたらそのまま取り込めるのか」を確かめるうちに見つけた。**新しい conflict が
1件も気づかれずに入ることは無い**——`docs/worklist.ja.md` の「証拠の表の conflict は N 行」を
`check_docs` が数え直すので、総数が動けば必ず落ちる（今日 20 → 21 で実際に落ちた）。
ただし**表ごとの内訳**は `product_attributes` の3行だけを `PROSE` で見ていて、他は野放しだった。

実測: 信頼度表が confidence の内訳を書いている28行のうち、**5行がずれていた**。

| 表 | 文書 | 実際 | 何が起きたか |
|---|---|---|---|
| `pin_functions` | conflict 12 | **18** | CH32V407/V467 の `FSMC_NADV` 6行（**F-60**。2026-09-04 に en 版 RM が加わって出た） |
| `option_bytes` | confirmed 90 / ref 8 | **98 / 0** | 同じ en 版 RM の到着で、V407 の zh 単独 8行が両版そろった |
| `option_byte_fields` | confirmed 91 / conflict 4 / ref 11 | **101 / 3 / 2** | 第3の根拠（EVTヘッダ・bit幅の規則）が片方を支持して confirmed に上がったぶん。異議は `basis` の `!…` に残る |
| `evt_examples` | confirmed 1,556 / ref 37 | **1,573 / 35** | H417 EVT v1.6 の取り込みで行が増えた（行数の欄だけ直っていた） |
| `registers` | ref 2,229 | **2,170** | 2,762+2,229+4 = 4,995 で行数 4,936 とも合っていない、古い数 |

**どれも裁定そのものは済んでいた**（F-60 は台帳、option_byte_fields は R-30）。ずれていたのは
数のほうで、つまり「conflict が増えたのに説明が更新されない」形の腐り方をしていた。

`check_docs.check_confidence` を足した——信頼度表の `confidence` 欄を全行読み、書いてある
`confirmed`/`ref`/`conflict` の数を生成物から数え直す。左端の綴りから表を引けない行は
`CONFIDENCE_NAMES` に対応を書く決まりで、**書き忘れも落ちる**。`product_attributes` の
`PROSE` 3行は同じことを見るので外した。

**入れたあと壊して落ちることを確認**——`conflict 18` を `12` に戻すと `pin_functions` を名指しで
落ち、行の綴りを変えると「どの表の数か対応が無い」で落ちた。

## F-60 を直した——見出しを読めない表が、前の格子を終わらせていなかった（2026-09-10）

`extract_remap` は remap 格子を「列見出しで field と値を決め、続きのページは見出しを持たない
ので**列数が同じなら続き**と見なす」形で読む。**見出しが読めない表を黙って飛ばしていた**のが
穴で、`pending`（直前の格子の列→field 対応）が生き残ったまま次の表に当たる。

CH32V407 の**表10-40（FSMC）**は列見出しが `FSMCEN=0默认映射`/`FSMCEN=1重映射` で、
`COLUMN_HEADER`（`*_RM\d?=値`）の形ではないので読めない。飛ばされたあと、次ページの
**見出し無しの断片25行**（`NADV PB7 PB7`・`A16 PD11 PD11`…）が直前の
**`ADC2_ETRGREG_RM`（ADC のトリガ選択）の続き**として読まれ、FSMC のアドレス線やデータ線まで
ADC の remap 経路として出ていた。

### 直し方——表題を持つ表は前の格子を終わらせる

続きの断片は**表題を持たない**（表題は表の先頭の上にあるから）。だから表題があれば「別の表だ」
と言える。`is_header_row`（自分で格子を始める形）でも終わらせる——表題の無い表がそれでも
自分の格子を始めることがあるため。

| 直し方 | 消えた偽経路 | 増えた経路 |
|---|---:|---:|
| `is_header_row` で終わらせる | 80 | 0 |
| **表題で終わらせる**（＋`is_header_row`） | **82** | **0** |

表題のほうが2件多い——`Alternate function(1)` のように行見出しの綴りが揺れると
`is_header_row` が当たらないため（FV2x.en の `ETH_RM=0 FSMC_NADV PB7` 2件）。
消えた82件は全部 FSMC の信号が ADC/ETH の field に付いたもので、**残した経路は1件も減っていない**。

### 結果

| | 前 | 後 |
|---|---:|---:|
| `remap_routes` | 4,836 | **4,838**（落ちていた `afio-fsmc-remap,1,FSMC_NADV,PB7` の V407/V467 が戻った） |
| `pin_functions` の PB7/FSMC_NADV | conflict 6 | **confirmed 6** |
| `index/pinout` の同6行 | `remap-0`（偽の格子値） | **`remap-1`** |
| conflict 総数 | 191 | **185** |

**索引が間違った経路を公開していた**のがいちばん重い——索引は pin 表と格子が食い違ったら
格子を採る側なので、偽の `remap-0` がそのまま `index/pinout.csv` に出ていた。

### 残した穴（F-61）

同じ走査で、**選択ビットが `*_RM` でない格子19表**（`FSMCEN=`/`DVPEN=`/`SPI3EN=` や
`FSMCEN=1&USBHS2EN=1&RB_UD_RST_SIE=0` という3ビットの論理積）を丸ごと読み飛ばしていると
分かった。zh/en は対称（V407 5/5・FV2x 4/4・L103.zh 1）。`(field, value)` の対では論理積の
条件を表せないので、モデルを決めずに読むと嘘の経路になる——**記録のみ**にした。
F-60 の修理でこれらが前の格子に混ざることは無くなったので、読み飛ばしても害は無い。

## ヘッダ語が改行で割れていた——6行が入った（2026-09-10）

F-60 と同じ「状態が生き残る」形を `operating_rows` でも探した。**自分の表題を持つのに
「前の表の続き」として読まれている表が5つ**あり、そのうち `CH32V103DS0.zh` p.26 の
`表3-19 通用I/O静态特性` を見たら、**ヘッダ自体は正しく `符号｜参数｜条件｜最小值｜典型值｜
最大值｜单位` と並んでいた**。読めなかったのは `符\n号`——狭い欄で語が改行で割れていて、
`norm_header` が **ASCII の空白しか落としていなかった**。

全corpus実測: 改行で読めていないヘッダ語は**26セル・21表**（`单\n位` 13・`最小\n值` 4・
`最大\n值` 4・`符\n号` 2・`Symbo\nl` 2・`Un\nit` 1）。うち**6表**はそのせいで見出しとして
通らず、直前の表の列の並びを借りていた。空白は種類を問わず落とすようにした。

併せて `extract_absolute_maximum` の `norm_header`（語彙だけ違う写し）を消して、
`operating_rows.norm_header(cell, mapping)` を呼ぶ形にした（正規化を二重に持たない）。

### 範囲で書かれた典型値で、行が丸ごと落ちていた

`keep_row` は min/typ/max の**どれか1つ**が値として読めないと行ごと落とす。落ちている316行の
内訳を測ると、17行は条件文が値の欄に流れ込んだ本物の誤読、3行は中文の語、99行は添字が `*` に
化けた式（資料側）で、**7行だけが「典型値が範囲」**だった:

| 記号 | min | typ | max | |
|---|---|---|---|---|
| `V_HV` 栅极驱动器的电源电压 | 6 | **6~24** | 26 | V（L103 zh/en・V007 zh/en） |
| `V_HV` 内部高压调节器の入力 | 18 | **20~48** | 50 | V（V007 zh/en） |

min も max も数で書いてあるのに、典型値が範囲なだけで**その2つも失われていた**。数と区切り
1つだけの形（`NUMERIC_RANGE`）を値として受けるようにした——条件文（`T_A = -40℃~105℃` は
`=` と `℃` を含む）は通らない。区切りは版で全角/半角に割れるので `norm_value` で `~` に揃える
（揃えないと偽の食い違いになり、全角のままでは公開する表にCJK帯の字が入る）。

### 結果

**3,209 → 3,215 行**（増6・消0・conflict 21のまま）。増えた6行はどれも zh/en 両版で
confirmed、原本の表（`表3-2-2 其它电源工作条件` ほか）と照合済み。
併せて `CH32V103` の `F_PLL_IN`/`F_PLL_OUT` 2行が **reference → confirmed**（zh 側の表が
読めるようになって両版が揃った）。`index/parts.csv` と生成 README は不変。

## 「表題の無い表は前の表の続き」を全抽出器で点検した（2026-09-10）

F-60 の形——**続きの断片かどうかを列数だけで決め、表題を見ていない**——が他にもあるか、
継続規則を持つ抽出器を全部見た。判定は同じ1つの問い: **自分の表題を持つ表が、前の表の
続きとして読まれていないか。** 続きの断片は表題を持たない（表題は表の先頭の上にあるから）。

| 抽出器 | 継続の決め方 | 表題を持つのに続きにされた表 | 直し |
|---|---|---:|---|
| `extract_remap` | 列数＋隣接ページ | **多数**（FSMC の断片25行が ADC の格子へ） | ✅ 直した（偽経路82件・F-60） |
| `operating_rows` | 列数＋（`carry_from`／表題／列境界） | **4**（消費電流表。列が `典型值` 2本の別の形） | ✅ 締めた（出力は byte 不変） |
| `extract_dma_requests` | 列数だけ | **40**（`表32-4 用户选择字信息结构`・`Table 13-2 TIMx internal trigger connections` ほか） | ✅ 締めた（出力は byte 不変） |
| `extract_pins` | **caption の y で切る** | 0 | 直し不要（元から表題基準） |
| `extract_products` | **列のx範囲が1対1で全部一致** | 0（続きは全corpusで2回だけ、どちらも表題無し） | 直し不要 |
| `extract_low_power` | **`logical_id` で束ねる**（共通L1層） | 0 | 直し不要 |
| `extract_absolute_maximum` | 表題で選び、続きは列のx対応（`align_cols`） | 0 | 新設時から表題基準 |
| `extract_memory`・`extract_option_bytes` | 表の中の**行**の継続（表跨ぎではない） | — | 対象外 |

`operating_rows` と `extract_dma_requests` は**出力が1バイトも変わらなかった**——前者は
借りた並びで読んだ行が `keep_row` の値検査に、後者は要求名の語彙にそれぞれ落ちていたから。
つまり今まで守っていたのは**偶然の下流の検査**で、構造としては開いていた。F-60 は同じ形で
実際に索引に嘘を出していたので、出力が変わらなくても閉じておく。

**残る「列数だけ」の継続は0。**

## README の数も検査に載せた——索引の README が古い主張を載せていた（2026-09-10）

信頼度表の confidence の内訳と同じ腐り方が、各ディレクトリの README にもあった。
「`<表名> … N 行」の形で書いている数を全 README で機械的に突き合わせたところ、
**本物の食い違いが2箇所**（残りは別の量を指す綴りで、当たったのは検査の網の粗さ）:

| 場所 | 書いてあった | 実際 |
|---|---|---|
| `evidence/README.{ja.,}md` | `registers` **4,995行** | **4,936**（信頼度表にも同じ古い数が残っていた） |
| `index/README.{ja.,}md` | 相手の値が入るのは **115行**・空は **68** | **109 / 76** |

さらに **主張そのものが古い**箇所が1つ。`index/README` は
「`operating_conditions` の30行のうち8行は**綴りの差だけ**（`mS` と `ms`、`0.8VDD` と
`0.8*VDD`、`VI/O` と `VIO`）」と書いていたが、**その8行は 2026-09-09 に対応付けの側で外して
いる**（単位と値を揃えてから突き合わせる）。いま残る21行は原文で裁定した資料側の齟齬で、
README だけが「綴りの差が混ざっている」と言い続けていた。

`check_docs` の `PROSE` に12本足した（26 → 38 箇所）——`index/README` の
相手の値あり／なし・`product_attributes` と `operating_conditions` と `memory_configs` の
conflict 数、`evidence/README` の `registers` 行数を、日英とも数え直す。
...
## 条件の列を持たない特性表を読めるようにした——時序270行が入った（2026-09-10）

ヘッダ語彙の穴を洗っていて、`参数及描述`／`Parameter & Description` が `HEADER_MAP` に無いと
分かった。ところがそれを足しても1表も通らない——**その表は `条件` の列を持たない**からで、
`{symbol, min, condition}` を求める検査に落ちていた。絶対最大定格表と同じ形。

全corpus実測: この並び（`符号｜参数｜最小值｜最大值｜单位`、`条件` 無し）の表は**66**
（33表×2版、CH32H417DS0・CH32V205DS0・CH32V20x_30xDS0・CH32V407DS0）で、
**全部が外部バス（FSMC の PSRAM/NOR/NAND/SDRAM）・DVP・Ethernet の MII/RMII/RGMII/SMI の
時序表**だった。**表題の語彙は要らない——形そのものが選別になっている。**

### 3つ踏んだ

1. **ページの門を通しても表ごとの絞りで落ちる。** ページ規則では66表中60が届かないので
   ページの門に「この形の表があるページ」を足したが、その先の
   「届いていないページでは表題が当たった表だけ」でまた落ちていた。
2. **続きの断片の外枠が数pt動く。** `same_edges` は全ての列の境界を2.0ptで見るので届かない
   （`CH32H417DS0.en` の表3-36 は見出しが 408.6、続きが 404.7）。**左端と右端だけを4.0pt**で
   見て、中の並びは列数で決める。これで表ごとの zh/en の行数が**33表すべて一致**した
   （それまでは 126/91・55/68・90/100・58/75 と非対称で、A11 で一度撤退した形だった）。
3. **値の欄が断片の全行を覆う1つのセルになっている断片があった。** `CH32V407DS0.zh` p.61 で、
   最大値の `5` は版面では2行目（`t_V(BL_NE)`）のものなのに、pdfplumber が5行ぶんの縦結合と
   して記録し、文字を先頭行に置いていた（**描画して確認**）。`fill_rowspans` がそれを4行へ
   広げ、en 版との**偽 conflict が4件**出た。全行を覆う縦結合は写さないことにし（実測で
   該当は corpus 全体でこの1つだけ。本物の縦結合はどれも表の一部を覆う）、さらに
   **その断片自体を読まない**ようにした——どの行の値かは版面にしか無いので、
   「片方が読めない」＝ en のみの `reference` が正しい。

### 結果

**3,215 → 3,485 行**（増270・消0・**新しい conflict 0**・既存行の確度の移動0）。
増えた270行は confirmed 262／reference 8。**全270行を別実装の読み直しで裏取り**した
（見出しの列名で min/max/unit を取り、続きは外枠で追う読み手。confirmed は zh/en 両版に、
reference は en に在ることを確認。`4tHCLK`/`4*tHCLK` の綴り差は `same_value` と同じ正規化で吸収）。
併せて `CH32H417DS0.en` p.123 の表3-35 を描画して値を目視照合。
`index/parts.csv` と生成 README は不変。

## I2C 接口特性表の2段見出しを開いた——288行が入った（2026-09-10）

前項で残った「あと一歩で揃わない表」のうち、`标准I2C`／`快速I2C` で列が分かれる形。
`符号｜参数｜标准I2C（最小值,最大值）｜快速I2C（最小值,最大值）｜单位` の**2段見出し**で、
**1行が2つの事実**（標準モードと高速モード）を言う。

**モードは条件そのもの**なので `condition` に置いて行を2つに開いた（`mode_body`）。
全corpus実測: この形は**33表**（16文書×2版＋`CH32V006DS2.zh`）で、1段目が
`(symbol, parameter, None, None, None, None, unit)`・2段目が `(None, None, min, max, min, max, None)`
と**33表すべて同じ**——形で確実に見分けられる。

### 3つ踏んだ

1. **ページの門にも足りていなかった。** 表ごとの絞りに足しても、続きの断片が載るページを
   ページの門が飛ばす。**その形の表を読んだ次のページも開ける**（`shape_page`）ようにした。
2. **外枠だけを続きの根拠にすると後ろの無関係な断片まで拾った。** 版面の外枠はどの表も
   だいたい同じなので、`t_CONV | Fast I2C`・`V_OLSAT | 快速I2C` のような行が出た（en 42行・
   zh 51行。正しくは18行ずつ）。**ページ隣接**と**自分の表題を持つ表が来たら打ち切り**を足した
   （F-60 と同じ形）。同じ緩さが時序表の側にもあったので、そちらにもページ隣接を足した。
3. **`C_b` の最大値が `400,`** で行ごと落ちていた（`CH32H417DS0.en` p.113）。版面を描画すると
   **本当に `400,` と刷ってある**（中文版は `400`）。値の末尾のコンマは値の一部ではないので
   落とす（全corpus実測でこの1セルだけ）。誤植は **F-62** に記録した。

### 結果

**3,485 → 3,775 行**（増290・消0・**新しい conflict 0**・全部 confirmed）。
288行が I2C（16文書 × 18行。落ちる3行はどちらの版も `t_r(SDA)/t_r(SCL)` の2記号畳み）、
残る2行は H417 の FSMC 時序で、ページの門を開けたことで届いた続きの断片
（`t_d(CLKL_DATA)`・`t_d(CLKL_NBLH)`。両版で `2 ns`）。

**288行すべてを別実装の読み直しで裏取り**（2段見出しを直に読み、モードは左右の位置で対応。
中文版と英語版で綴りが違うので名前ではなく位置で突き合わせる）。残る2行も原本の行を目視。
`index/parts.csv` と生成 README は不変。

## `min` を持たない ADC 誤差表を受けた——「あと一歩」の最後（2026-09-10）

ヘッダが揃わない表を数え直したら46まで減っていて、内訳は**33が絶対最大定格表**（別の正本が
読む）と**11の ADC 誤差表**だった。後者は `符号｜参数｜条件｜典型值｜最大值｜单位` で、
`条件` は在るのに **`min` が無い**——受入条件が `{symbol, min, condition}` だったので落ちていた。

### 緩めすぎた

最初に「値の欄が1つでもあれば受ける」にしたら**131行**入り、`t_wusleep`・`t_wustop`・`I_DD` の
**低消費電流の表**（A11。`extract_low_power` の担当）まで飲み込んだ——同じ事実が2つの経路から
出て、**新しい conflict が2件**（どちらも既知の A11 の zh/en 齟齬）。
受けるのは**`典型值` と `最大值` が揃う**形だけに絞った（低消費電流の表は `典型值` しか持たない）。

### 結果

**3,775 → 3,780 行**（増5・消0・conflict 21のまま）。増えたのは `CH32M030DS0` の
`表3-35 ADC误差` の `ET ±12`／`EO ±3`／`EG ±6`／`ED ±10`／`EL ±10`（LSB）で、両版一致。
他の ADC 誤差表の行は既に別経路で入っていた。

併せて `KEEP` に **`EG`（利得誤差）** を足した——`ET`（総合）・`EO`（オフセット）・`ED`（微分
非線形）・`EL`（積分非線形）と同族で単位も LSB なのに、綴りの並び `E[DLOT0]` から `G` だけ
抜けていた（全corpus 4行）。`EN`（等価入力電圧雑音）は単位が nV/√Hz で別の量なので、
単位の検査で落ちたままにする。

**「あと一歩で揃わない表」は、絶対最大定格（別の正本）を除いて 0 になった。**

## 値の大小を検査に載せた——4,030行で崩れは1行、それは資料の誤植（2026-09-11）

今日足した約570行を含めて、`operating_conditions`（3,780行）と `absolute_maximum_ratings`
（250行）で **min ≤ typ ≤ max** が成り立つかを数として読める値だけで測った。

**崩れていたのは1行**——`CH32V005;CH32V006` の `V_PVDhyst` が `5 / 20 / 6 mV`。
**版面を描画すると英語版が本当に `6` と刷っている**（中文版は `60`、兄弟の `CH32V002` も
`5/20/60`）ので、英語版が 0 を落とした誤植。行はすでに `conflict` で両論を持っていた。
**F-63** に記録した。

`check_tables.ordered_values` を足した。**列を1つずらして読むと、たいてい大小が崩れる**ので、
これは「読み違い」を捕まえる網になる（今日は `CH32V407DS0.zh` p.61 の縦結合の誤りを
描画で見つけたが、値の大小では気づけなかった——`max=5` は `min=tHCLK` と比べられない）。
資料の誤植と読み違いはデータの形では区別が付かないので、**名前で固定した例外の外に出たら
落とす**（`KNOWN_UNORDERED_VALUES`。いまは F-63 の1行だけ）。式や記号
（`0.8*VDD`・`5*tHCLK`・`±10`・`∞`）は大小を言えないので比べない。

**外して落ちることを確認**（例外を消すと `V_PVDhyst` を名指しで落ちる）。
