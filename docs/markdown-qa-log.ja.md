...
**最終**: 修復 **1,158セル・21文書**（うち索引由来230）、危険な短縮0・索引切り落ち0、parity 68/68 clean。
...
- **名前セルの下段が数字列だけだとbit番号行と見分けられない**（FV2x_V3xRM.en p447のFMCFGR下位。
  `FBM15`…`FBM0`の下段が`15 14 … 0`なので、bit番号行として扱われ`FBM`×16と`FB15M`…の2行に
  割れる）。図の内側の箱のy位置で「名前セルの下段」と分かるが、bit図の組み直しの入口を触る
  変更になるので保留。
