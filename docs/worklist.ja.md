...
| F-68 | **`remap_routes` の `basis` が、格子が言っているのに言っていないことにする**（F-66 の直しが狭すぎた）。`route_sources` の照合が (signal, pad) の完全一致か「裸ラベルが signal の末尾」しか見ない | **336行**（2026-09-15の監査）: `SDIO_D0` と格子の `SD0` 32・`LTDC_G4` と `G[4]` 96・UHSIF の PORT22以降 26・`ETH_MII_RXD0` と `ETH_RXD0` 38・pad の綴り（`PC14-OSC32_IN` と `PC14`）86・`T2CH1ETR` と `TIM2_CH1`＋`TIM2_ETR` 58 | ツール | 🔧 **一部だけ解消**（F-69 の修理で、説明文も述べている経路は `rm-field-description` を名乗るようになった）。**残りはあいまい照合では埋めない**——2026-09-15に試して、`SD0`↔`SDIO_D0`・`G[4]`↔`LTDC_G4`・`ETH_RXD0`↔`ETH_MII_RXD0`・`PORT9`↔`UHSIF_PORT9` は末尾一致でも `canonical_signal` でも揃わず、**周辺ごとの語彙の事実**だと分かった（`SDn` は `SDIO_Dn`、`G[n]` は `LTDC_Gn` という規則）。入れる場所は `signal_vocabulary` で、1つずつ原文で裁定するもの。pad の綴り（`PC14-OSC32_IN`）と `T2CH1ETR` の分解は別系統 |
