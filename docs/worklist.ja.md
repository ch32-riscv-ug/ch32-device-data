...
- [ ] **境界で1字が重なる隣接セル（要選別）** — bit図に限って「左セルの末尾＝右セルの先頭」を
  数えると523件だが、大半は**隣り合う別フィールドの偶然の一致**（`TIM7RST`|`TIM6RST`・`PINC`|`CIRC`）。
  本物（`DBP`|`PLS[2:0]` 型）を選り分ける判定がまだ無い。上の16件のように「復元先が `Name` 欄にある」
  を条件にできるはず。
