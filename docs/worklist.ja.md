...
| 表示（G系） | 12 | 0 |
| 既知の穴（F系） | 54 | 11（下の F 台帳で ✅ が付いていない行の数。資料側の記録が F-7・F-33・F-43〜46・F-51・F-62・F-63。F-4 と F-24 は残りだけが資料側で実害なし。**ツール側の穴は2**——F-61（論理積の条件を `(field, value)` で表せない remap 格子。2026-09-15に半分修理し、残りはモデル待ち）と F-66（`remap_routes` の `basis` が実際に効いた出所を言っていない）。F-64（signal を先に合わせて食い違いを対にする）と F-65（自動更新に台帳の再記録を足し、`workflow_ledger` で機械化）は 2026-09-15 に修理。F-60 は 2026-09-10 に修理（見出しを読めない表が前の格子を終わらせていなかった）。F-57・F-58 は 2026-08-29、F-59 は 2026-09-04 に解決） |
...
| F-61 | **選択ビットが `*_RM` でない remap 格子を丸ごと読み飛ばしている**。列見出しが `FSMCEN=0`/`DVPEN=1` のような**周辺の有効化ビット**や、`FSMCEN=1&USBHS2EN=1&RB_UD_RST_SIE=0` のような**3つのビットの論理積**で、`COLUMN_HEADER`（`*_RM\d?=値`）に当たらない | 全corpus実測（2026-09-15に測り直し）: **16表**。うち4表（CH32V407RM zh/en の DVP・SDIO）は**3列のうち2列が普通の `*_RM`** で、1列だけが論理積 | ツール | 🔧 **半分修理**（2026-09-15）。`read_header` が「1列でも読めないと表ごと捨てる」のを**読めない列だけ `None` にする**に変えた——論理積の列は `(field, value)` で表せないので読まない（推測しない）が、同じ表の `DVP_RM=0/1`・`SDIO_RM=0/1` は読める。`extract_remap` の経路は CH32V407RM で **374 → 424**（zh/en 対称・消失0）で、版面（表10-42・10-43）と全件照合した。**併せて `1DVP_RM` という在りもしない field を直した**——`DVPEN=1DVP_RM=0` の `1` を巻き込む正規表現の誤りで、緩和で露出した（field は英字始まりに限る。全corpus実測で数字始まりの field は0）。<br>**残るのは論理積の列そのもの**（FV2x の DVPEN/SDIOEN/SPI3EN 系と V407 の FSMC）。`remap_fields` の selector は AFIO の field を前提にしていて、RCC の有効化ビットをselector にすると `remap_fields` に相手が無く `check_tables` の結合が落ちる。**モデルを決めるのが先**（記録のみ）。<br>**正本は1バイトも動かない**——この50経路は datasheet の pin 表から既に出ていて、格子はその裏づけになっただけ。ただし裏づけが `basis` に出ない（F-66） |
...
| F-66 | **`remap_routes`/`remap_fields` の `basis` が、実際に効いた出所を言っていない**。`build_remap` は経路の `basis` を **`value == 0` かどうかだけ**で決めていて、既定値の行は`candidates(datasheet-pin-table-default:en)`、それ以外は一律`candidates(datasheet-pin-table+rm-remap-grid:en)` を名乗る。`remap_fields` の `basis` も固定文字列 | **`CH32X035RM` は zh/en とも remap 格子から0経路**（`CH32V205RM`・`CH32X315RM` も0）なのに、CH32X035 の経路234行のうち**186行が `rm-remap-grid` を名乗る**（2026-09-15実測）。候補側の `_valid_values_source` も同じ型の誤りで、格子の経路と RM 散文の経路を混ぜた集合を「RM remap格子」と呼んでいる | ツール | ⬜ **未着手**（2026-09-15）。直すには経路ごとの出所を `build_candidate` から `build_remap` まで通す必要がある（`selection` は `{selector, values}` しか持たない）。**値そのものは誤っていない**——datasheet の pin 表が根拠で、`basis` がそれに加えて格子を名乗っているのが誤り |
