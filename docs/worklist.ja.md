...
| 既知の穴（F系） | 47 | 7 = 資料が決めない 1（F-4残り。実害なし）＋ 実機待ち 1（F-11）＋ 資料側の記録（F-6/7・F-24残り・F-33・F-43〜46・F-51）＋ ツール側だが実害なしと判断（F-53）＋ 未着手（F-55） |
...
| F-54 | `evidence/remap_routes.csv` に**導出列 `peripheral`/`role` が残っていた**（2026-08-26 の区分やり直しで外す設計にしたが、CSV を再生成していなかった） | 2列・4,919行 | 運用 | ✅ **解消**（2026-08-28。F-50 の再生成で反映）。`ROUTE_COLUMNS` は 2026-08-26 からこの2列を持っていないので、**ツールと生成物が2日ずれていた**。語彙で導出した読みは `index/routes.csv` が付ける（[data-layout](data-layout.ja.md) の「証拠に語彙導出列は不可」）。**consumer には列が2つ減る変更**なので、単独の差分として見えるようにここに記録する |
| F-55 | `表3-4 引脚复用和重映射功能`（V30x datasheet）を**どの抽出も読んでいない** | (pad, signal) 466対のうち 217対が `pin_functions` に無い | ツール | ⬜ 未着手（2026-08-28 に発見）。pad × 周辺群の相互参照表で、封装ごとの番号表ではないので `PIN_TABLE_TITLE` に当たらない。**V307側の表**（RGMII を含み V307 固有対81件・V317 固有対0件）。217対には粗い切り出しの屑が混ざっている可能性があるので、まず中身を確かめること。**F-50 とは別**——V317 の ETH をここから補える見込みは無い |
