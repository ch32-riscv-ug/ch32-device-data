...
- [x] ✅ **D16 PDF構造化ワークフローのPoC・実現可能性・移行計画** — 対象を電気特性に限定せず、**datasheet全34版＋RM全21版**と定義。原本PDF→全ページ構造化bundle→schema/hash/review検査→領域別抽出→Markdown/HTMLという共通経路を[設計・実測・計画](structured-document-workflow.ja.md)にした。V003英版datasheet 37ページは本文・語・図形・表・結合セル座標が旧経路と全件一致し、`extract_products`の4型番も一致。V003英版RM 188ページを変換し、`extract_registers`も310 fields・notes 16件まで一致した。既存`tools/`とCSVはbaselineとして凍結し、新実装は工程別の`pipeline/`で並走、入力bundleが揃ったCSVから旧新比較後に置き換える方針。原文block IDは表示分割と独立させ、将来HTML anchorとしてCSVからURLで指せる。人向け表示を文書・章・原本ページのどの単位にするかは未決定。**今回の範囲はここまで**。55版の実変換、既存抽出器の切替、性能最適化、承認運用、画像asset renderer連携は将来工程で、正本CSVと本番経路は変更しない
