...
**取り込みは引き続き保留**。(2) を単独の企画として立てるか en 版を待つ。(2) を入れれば新しい行は
出ないままでも**偽の conflict は消える**（zh 側に `VDD-0.4` の行が在れば値の一致で正しく対応する）。
それまで `check_sources` は 67/68 を報告し続ける（正しい報告なので黙らせない）。
