...
| R-30 | RM option bytes章の版間齟齬（識別子・復位値） | 文書ごとにzh/enで綴りが逆転（V003: zh `IWDG_SW`/en `IWDGSW`、V205はその逆）。X315はzh `USBHSDLEN`/en `USBFSDLEN`（**FSとHSの違い**）。X035の復位値はzh `xxxb`/en `xxb`（[7:5]の3bitなのでzhが正しい見込み）。FV2x/V3x enはSRAM分割fieldを無名のまま（zhは`RAM_CODE_MOD`と命名）第3列に復位値`xx1b`を置く | `option_byte_fields`でconflict＋両論（en記載を採りzhの異議をbasisへ）。裁定はreview（L2）の仕事として残す |
