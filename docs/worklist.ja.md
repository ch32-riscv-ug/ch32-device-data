...
- [ ] **境界で1字が重なる隣接セル（523件 → 本物42件）** — bit図に限って「左セルの末尾＝右セルの先頭」を
  ...
  **この項目は「資料側＋ツール側4セル」まで絞れた。** 残るツール側は
  `5USARTRST`|`T4USART3URST`（H417 p.38 bit19/18。自分の索引が右へ落ち、かつ右隣も壊れている）と
  `INTEN15`|`5INTEN1`（X315 p.73。同じ形）。ほかの13件は検出器の偽陽性（隣り合う実名が偶然
  1字を共有）か資料側。
