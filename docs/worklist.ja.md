...
| F-24 | **lead番号を持たないpad**を落としている（内部で別padに接続） | 9 pad・5 datasheet | 設計 | 🔜 判断待ち。下記 |
...
### F-24 lead番号を持たないpadを落としている

datasheetのpin表は、**在るのに封装の番号を持たないpad**を空欄で書きます。
CH32M103のPA13/PA14がそれで、中文版p.25が

> PA13/PA14はチップ内部でPA11/PA12に接続されている

と断っています。番号が無いのは「この封装に無い」（`-`と書かれる）のとは別で、
`-`と空欄を同じに扱っていたため**CH32M103はSWDIO/SWCLKを1つも持たないseries**
としてREADMEに出ていました。

該当は9 pad——CH32M103 PA13/PA14、CH32L103 PB5、CH32V205 BOOT1/PD0/PD1、
CH32H417 PB10、CH32V004 PA4、CH32V203 PB8。

（**F-25 で 3 pad は解消しました**——`PC13-TAMPER-RTC` などは lead 番号を持って
いて、pad 名が 8 文字を超えて落ちていただけでした。残るのは本当に番号を持たない
9 pad です。）

**持ち方が決まっていません。** `pins.csv`は`number`を主キー相当に使っていて、
空の番号を入れると

- 封装ごとのlead数（`packages.csv`のlead数）と合わなくなる
- `gpio_count`の突き合わせ（pin表のP pad数）が動く
- consumerが「pin番号でpadを引く」ときに引けない行が混じる

のどれが起きるか、consumer側の使い方を見ないと決められません。案:

| 案 | 持ち方 | 効き方 |
|---|---|---|
| 1 | `number`を空にして載せる | 逆引きは通る。lead数の検査に例外が要る |
| 2 | `number`に接続先のpadを書く（`PA11`） | 事実に近い。型が変わる（intでなくなる） |
| 3 | 別列`bonded_to`を足して`number`は空 | いちばん正確。列が増える |

今は**落とすが黙って落とさない**（`notes`に出す）状態にしてあります。

### F-21の残り（2026-08-24時点）

覆えない綴りは **110種1046行 → 34種154行（0.7%）**。内訳と行き先:

| 残っているもの | 行数 | なぜ残るか |
|---|---|---|
| `AETR` `AETR2` `TIETR`（CH32V003） | 18 | **資料側**。F-8。ADCトリガがADC 2 fieldのどちらか決まらない |
| CH32M030のモータ駆動 `HO0-3` `LO0-3` `ISINK1-2` `ISOURCE1-2` `ISN1-2` `ISP2` `QII1-2` | 74 | **設計判断**。周辺名＋instanceで、役割にあたる部分が無い。`(ISINK, 1)`とも`(ISINK1, OUT)`とも書けるが、どちらも資料の言い方ではない |
| `SWIM`（CH32M030 PA3） | 5 | 資料に対応の記述が見つかっていない |

**埋め方は`tools/check_tables.py`の`KNOWN_ROLE_GAPS`で数ごと固定**してあります。
閾値（「95%以上あればよい」）ではなく綴りごとの実測値を持つのは、片方が直って
別の穴が開いたときに気付けるようにするためです。**目標は0。**

解消したものの根拠:

| 綴り | 読み | 根拠 |
|---|---|---|
| `A0`〜`A13` | `ADC1_IN*` | CH32X035 datasheet p.16 の凡例 `A:ADC_ (A10:ADC_IN10)` |
| `C1P0` `C3N0` `C1O` | `CMP*_*` | 同 `C:CMP_ (C3N0:CMP3_N0)`＋EVT header の `CMP_STATR_CMP1_OUT` |
| `O1N0` `O2O0` | `OPA*_*` | 同 `O:OPA_ (O1N2:OPA1_N2、O2O0:OPA2_OUT0)` |
| `OPN0` `OPP0` `OPO` | `OPA1_*` | CH32V003 はOPAを1つしか持たない |
| `MCO` | `RCC_MCO` | EVT header の `RCC_MCO_SYSCLK` |
| `WKUP` | `PWR_WKUP` | EVT header の `PWR_WakeUpPinCmd` |
| `RST` `NRST` `BOOT0` `BOOT1` | `SYS_*` | 周辺が持たないチップ全体の端子 |
| `XI` `XO` `X0` `OSCI` `OSCO` | `OSC_IN` / `OSC_OUT` | 他familyが`OSC_IN`/`OSC_OUT`と綴る同じ端子。`X0`は`XO`の綴り違い（同じPA2） |
| `USBDM/DP` `USBHDM/DP` | `USB_*` / `USBHD_*` | CH32V103の比較表が`USB(FS) USBHD`と書く |
| `CC1`〜`CC4` `CC1(CC1R)` | `USBPD_CC*` | 比較表の`PDUSB USBPD Type-C`。括弧の中はRd内蔵側の呼び名（p.16） |
| `LED0` `LED1` | `ETH_LED*` | CH32V407の注記 `register FEATURE_SIGN` の `ETH_LED_EN` |
| `TAMPER` `RTC` `TAMPER-RTC` | `RTC_TAMPER` / `RTC_OUT` | PC13が兼ねる2つの端子。CH32L103は改行で分けて書き、CH32V103/V20x/V30x/V4x7は`TAMPER-RTC`と1語で書く。**1語のほうは2つの役割を名指す**ので両方に載せる（`COMPOUND`） |
| `ADCS0`〜`ADCS2` | `ADC1_S*` | CH32X315 p.12「the round count can be output through GPIO (ADCS0/PA10, ...)」 |
| pad名（`PA13` `PC13` `PD0`…） | **載せない** | padのGPIOとしての名前であって役割ではない |

**抽出の残りかすは抽出側を直しました**（層2では埋めない）:

- `DD33` `DDIO` `DDK` `IO18`（CH32H417・14行）——添字の判定`SUBSCRIPT`が
  「語の始まり」を空白でしか数えていなかった。中文版は同じ欄を`主V`と書き、
  漢字とラテン文字の間に空白を置かないので`主V`／`DD33`に割れていた。
  漢字→ラテン文字の変わり目も語の始まりとして数えるようにした。
- `A13RST`（CH32X035・2行）——`continues()`が「padも型も空なら前の行の続き」
  としていたが、CH32X035のPC3は**封装ごとに既定の多重化機能が違う**ので、
  pad欄が空で lead 番号だけを持つ行が続く。番号を持つ行は自分の行なので、
  繋いだせいで`A13`＋`RST`が`A13RST`になり、その封装だけが持つRSTが消えていた。
