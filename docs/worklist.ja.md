...
| F-20 | 行グループ見出しが属性名に混ざる（`Communication interface CAN`） | 全family | ツール | ✅ **実装済み**（2026-08-24）。`group_zh`/`group_en`を新設。下記 |
| F-21 | `pin_roles`が語彙で覆えない signal 110種 | 1046行（4.4%）→**34種154行（0.7%）** | ツール/資料 | 🔧 大半を解消（2026-08-24）。残りは下記 |
...
| CH32M030のモータ駆動 `HO0-3` `LO0-3` `ISINK1-2` `ISOURCE1-2` `ISN1-2` `ISP2` `QII1-2` | 82 | **設計判断**。周辺名＋instanceで、役割にあたる部分が無い。`(ISINK, 1)`とも`(ISINK1, OUT)`とも書けるが、どちらも資料の言い方ではない |
| `RTC` `SWIM` | 10 | 資料に対応の記述が見つかっていない |
