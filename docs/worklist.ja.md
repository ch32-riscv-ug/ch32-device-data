...
| F-74 | **`condition` の添字が基底から離れたまま残る**。版面は記号の右下に小さく添字を刷るので、セルの文字列にすると読み順が「基底 → … → 添字」になる。`=` の前後に接している形（脚注が挟まるものを含む）は綴りで戻せるが、**語をまたいで離れた形**（`Input V /2, DDA C = 50pF`・`V cannot be DD33A lower than VDDIO`）は綴りでは決められない | operating_conditions 88行・16綴り（`V_HREG` 12・`V_DD8` 12・`V_DDA` 11・`V_HV` 10・`R_AIN` 9・`R_LOAD` 7 ほか。2026-09-16 の実測。**接している形 1,007 行は同日に直した**）。別に `F_V5F` 84箇所・`F_V3F` 76箇所（計100行）は**綴りが corpus に無い**ので動かしていない | ツール | 🔧 **geometry の追加処理は要らない**（当初そう書いたが誤り）。converter の `text` が**既に正しく繋いでいる**——同じセルが `Input VDDA/2,⏎CLOAD = 50pF,⏎RLOAD = 4kΩ`。CSV 経路（`extract_low_power.join_fragments`）が読んでいるのは `text_split` のほうで、そちらは版面が割ったままの `Input V /2,⏎DDA⏎C = 50pF,⏎LOAD…`。**残る仕事は2つ**: (1) **記述の列だけ `text` から読む**——記号は `text_split` のまま（`I⏎DD`→`I_DD` の前提がそこに掛かっていて、替えると `I_DD` 系 1,207 行が落ちる。converter 1.7.0 で実測）。`join_fragments` が格子を2つ返し、`parse_table` が列ごとに読み分ける形になる。(2) **綴りを corpus に揃える**——`text` は下線を持たない（`VDDA`）が、条件文の実測は下線あり 1,759・下線なし 67 で下線形が正。記述の全セルに触るので差分確認が要る |
