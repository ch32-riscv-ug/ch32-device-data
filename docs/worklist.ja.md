...
- [x] ✅ **A1 エラッタ** — 21件、全件が中英両版のページ根拠つきconfirmed。`pipeline/extract/scan_errata.py`で増分検出（NEWがあれば終了コード1）
...
- [x] ✅ **A3 remap** — `remap_fields`/`remap_routes`。**2026-09-16 に `remap_routes` の確定化を済ませた**（案 B。datasheet の pin 表と application manual の両方が述べている経路が `confirmed` 4,354行・片方だけが `reference` 483行・`conflict` 1行）。併せて `basis` の版の印が `:en` の決め打ちだった嘘を直した（F-75）。`remap_fields` には**行ごとの確度を付けない**と決めた（同日）——行が複合で列ごとに出所が違うため。代わりに consumer が書く `bits` を固定した: header と manual が同じ register に違う bit を言ったら `conflict`（全corpus実測 0 件）。2026-08-20に作り直し: `bits`がbitごとにregister名を持つようになり、PCFR1とPCFR2にまたがるselectorを表せる。`peripheral`/`role`列で`TX1`/`UTX`/`USART1_TX`の綴り差を吸収。value=0の既定経路を同じ表に収録。CH32V407/V467はRM未mirrorでも header+datasheet から生成する。`tools/check_tables.py`が表だけで整合を検査する（bit形式、値の幅、route値がvalid_valuesに含まれること）
...
**現時点では生成READMEに画像を使いません。** ピン配置図は「パッケージ→型番→データシート」の
対応表で代替しています。**留保の理由は切り出しの品質ではなくなった**——2026-09-16 の実測で
C1（切り出し品質）は閉じたので、あとは**採否そのものの判断**（C3 の構成図が17シリーズ分
揃わないこと、画像をミラー側リポジトリに置く運用）だけが残っている。

- [x] ✅ **C1 切り出し品質** — 2026-09-16に**測り直したら、残っていたのは1つだけ**だった。
  `tools/extract_images.py` は **133枚**（architecture 27・pinout 81・package 25）を生成する。
  - **図の縁**: `check_images.suspicious` が挙げるのは133枚中**1枚**（`pinout_CH32V407WEU6_QFN68`）で、
    これは**偽陽性**。版面を描画して確かめたところ、`CH32V407DS0` p.20 は**ピン番号 18〜34 を
    フッタ罫線の上に刷っていて**（字形は y=774.35..782.28、罫線は 780.6..781.0）、図の帯に
    罫線が入るのは資料側の版面どおり。切れている画像は**0枚**。
  - **ファイル名と図中型番**: 実測は 81名中**7件**。うち**5件はツールの不具合で直した**——
    `find_pinouts` の `done`（代表型番の図で作ったか）が**文書ごと**だったので、同じピン配置を
    複数のデータシートが描くとき（`CH32V002F4P6` の TSSOP20 は V002・V004・V006 の3冊にある）
    **あとの文書が代表型番の図を上書き**していた。`main` で持ち回るようにして 7 → 2。
    残る2件は**検出側の誤り**で、版面の見出しは `CH32V103Cx`・`CH32V303RxT6/CH32V303RCT7` と
    伏字・スラッシュを含む。伏字を展開して当てると**食い違いは0件**。
    出力の注記を展開後の `parts[0]` から**版面に刷られた見出しそのもの**に変えて、
    この照合を出力だけで再現できるようにした。
  - **⛔ 縁の偽陽性を消すために本文下端を罫線の外まで広げる案は戻した**——`body_bounds` の
    既定（`page.height - FOOTER`）より罫線由来の値のほうが下になるページで、罫線を跨ぐ図が
    拾えるようにする案。V407 のピン番号は完全に入るようになるが、**12枚が動いて2枚が悪くなる**
    （`package_LQFP64`・`package_QFN68X7` が余白ごと広がり、フッタ罫線が画像の底に入る）。
    **再突入の条件**: 罫線を図の一部として扱うか外すかを、図ごとに決められる材料が要る。
- [x] ✅ **C2 ページ番号リンク** — 2026-09-16に `evidence/figures.csv` を新設して実装した。
  生成 README の `Packages & pinout drawings` の `[en]`/`[zh]` が `#page=N` 付きになり、
  読者が表紙ではなく図そのものに着く（12ファイル・81行が変わった）。
  - **表の形**: `series, part_number, kind, document, page_zh, page_en, confidence, basis`。
    `kind` はいま `pinout` だけ。**`page_zh`・`page_en` を別の列に持つ**——両版で割り付けが
    違うのは食い違いではないので、この表は構造上 `conflict` を持たない。**103型番すべてが
    両版に図を持つ**ので全行 `confirmed`。
  - **読み手は bundle**（`pipeline/extract/datasheet/extract_figures.py`）。PDF は開かない。
    素直に書くと落ちるところが2つあった: **(a) 章の終わりは行の順で決まる**
    （`CH32V003DS0.en` は p.13 の一番上に SOP8 の図を置き、その下に `2.2 Pin Description`
    が来る。ページ本文をまとめて見てから語を探すとこの図が丸ごと落ちる）、
    **(b) 見出しの型番が語で割れる版がある**（`CH32V006DS0.zh` p.12 は `CH32V0 06F8P7`。
    間隔が近い隣の語と繋いだ綴りも試す）。
  - **中文版の章見出しの綴りは `引脚排列`**だった。凍結tool `tools/extract_images.py` の
    `PINOUT_CHAPTER` は `引脚分布|引脚图` しか持っておらず、**英語版しか読まないので
    誰も気づいていなかった**。全16冊の中文版がこの綴り。
  - **図の見出しは型番に見えて型番でないことがある。** 2026-09-16 に ArduinoCore-CH32 から
    「`CH32V006E8R1` が `CH32V006DS0` に在るのに `products.csv` に無い」と報告があったが、
    **これは型番ではない**——版面が刷っているのは `CH32V006E8R`（温度グレードの桁を省いた形。
    zh p.10 x=258.00..322.55 y=555.91）で、続く `1` は**図の左側のピン番号**
    （x=170.16..174.46 y=562.61、6.7pt 下）。ページ本文を平坦化すると隣り合って見える。
    全corpus走査でも `CH32V006E8R1` はこの2箇所（zh p.10・en p.14）だけで、どちらも同じ形。
    `CH32M006A8U6`（`CH32V006DS2.zh` p.35）も同じ類で、**命名規則の説明の例**
    （`系列产品命名规则举例：`）であって订购の行ではない。
  - **受入は全206（103行×2版）を別の読み方で読み直した**。主は**ミラーの原本 PDF**を
    pdfplumber で開き、**語の座標を使わない読み方**（行の綴りから空白を落として `CH32` の
    位置で切る）で読む実装——**191が一致・食い違い0**。残る15は `extract_text_lines` が
    図の中の見出しを行として返さないためで、別々に当てた: 英語版7件は凍結tool
    `extract_images.py`（3つ目の実装。PDF直読み＋`extract_words`）の版面と**ページまで一致**、
    中文版8件は**原本を描画して目視**（`CH32V002DS0.zh` p.10 の F4P6/F4U6、`CH32V004DS0.zh`
    p.10 の F6P1/F6U1、`CH32L103DS0.zh` p.13、`CH32M030DS0.zh` p.13、`CH32V205DS0.zh` p.14）。
    英語版はこれとは別に、凍結tool が版面を報告する90行とも**食い違い0**で一致している。
  - 検査は `check_tables.figures_sane`（型番と document の対応・ページが正の整数・
    confidence がページの数と一致・`basis` の `p.N` が列と同じ・**全型番に行が在ること**）。
    **5通りに壊して落ちることを確認**した。
...
- [x] ✅ **D3 エラッタ増分検査** — `pipeline/extract/scan_errata.py`（ミラーPDFが要るのでCIではなく手動運用）
...
### D7 生成のGitHub Actions化（計画のみ・2026-08-23／**2026-09-16 に方針を決めた**）

**結論: 再生成のCI化は見送る。検査のCI化だけ続ける。** 2026-09-16 に実測して決めた
（ユーザーの指摘が起点）。

| | 実測 |
|---|---|
| `.cache/structured-bundles` | **1.3 GB**・68 bundle。commit していない |
| mirror（原本PDF） | **1.9 GB**・別orgの12 repo。この repo に無い |
| `--full --verify --human`（bundle が温かい） | **約12分**（うち `human` が 650秒） |
| 冷たい `convert_all` | RM 1本600ページで約4分 × 22 bundle ＋ datasheet 46 bundle → **1時間超** |

しかも `render_assets` は `pdfplumber.open(pdf_path)` で**原本を開く**ので、bundle が
あっても mirror のクローンが要る（`check_sources` も原本のSHAを見る）。そして CI が
「差分あり」で赤くなっても**直す作業はローカル**（コードを直して再生成して commit）
——CI は信号を出すだけで何も肩代わりしない。

**取りこぼす値打ちは1つだけある**: 「commit されている正本を、いまのコードが本当に
再現するか」。`check_baseline` は正本と台帳の**自己整合**しか見ないので、コードだけが
変わって正本が古いまま、は素通りする。それを安く取るなら**毎回の再生成ではなく**、
bundle を Actions cache に置き（キーは `structured/*/manifest.json` の SHA）、
`pipeline/` か `tools/` が変わった PR でだけ `--full --verify`（`--human` 抜き）を
回す形になる。`check_sources` のために mirror を要らなくする入口が別に要る。
**優先度は低い**——得られるのは信号だけなので。
...
~~実測で`build_all`が2並列16.6分~~ → **D18でPDF直読みの生成器が居なくなり、この見積りは
古い**（`build_all`は6.7秒。PDFはbundleへの変換のときだけ開く）。**2026-09-10の実測**:
`regenerate.py --full --verify --human` は**49段・35.2分**で、その内訳は
図の描画 310秒・Markdown書き出し 398秒・parity検査 435秒＝**人向けの3段で19分**、
残る46段が合計16分（`build_all` 6.7秒・`check_*` 各数秒）。**bundleは増分**なので、
この35分は「原本が動いていない」ときの数。原本が動いた文書は`convert_all`の再変換が
要る（1文書あたりの実測はまだ取っていない）。GitHub Actionsの標準runnerで収まる。

#### 決めていない点（着手時に決める）

- **成果物をどう扱うか。** 案は3つ。**2026-09-10の作業で(a)の根拠が一段強くなった**——
  資料が動いて再生成すると `conflict` が増えることがあり、**その1件が本物の版間齟齬なのか
  こちらの対応付けの失敗なのかは、データの形では区別が付かない**（今日の実例: `T_J` の
  en 105℃/zh 115℃ は原本を描画して本物と確認、一方 X035 の `V_DD 2.5V` は到達の失敗だった）。
  検査は「conflict が増えた」ことは必ず名指す（総数を `check_docs` が数え直す）が、
  **裁定はできない**ので、CIが取り込みまで進む形にはしない。
...
### D19 抽出の品質課題（D18から切り出し・2026-09-10）

D18（PDF構造化）は完了したが、**bundle を正しく読めているのに正本が惜しい**箇所が残る。
どれも「原本の読み取り」ではなく「読んだものの解釈」の問題で、D18の完了条件とは別軸。

- [x] ✅ **添字の復元が版で違う結果になる** — `CH32V007DS0` の `V_B`。**2026-09-10に直した**。
  zh のセルは2行組み（1行目に基底2つ `V +V`、2行目にその添字2つ `CC12V S`）で、
  `attach_value_subscript` が**平坦化してから**戻すために `CC12V` の宛先を決められなかった。
  行の構造が残っているうちに対応させる `pair_line_subscripts` を足した。値の欄でこの形は
  全corpusでこの1セルだけ。行数不変で `V_B` が conflict → confirmed（conflicts 192 → 191）。
- [x] ✅ **絶対最大定格表を読む** — 2026-09-10に測ったら**19行ではなかった**。
  原因も「記号が読めない」ではなく**3段階**: (1) 記号は `pair_line_subscripts(sep="_")` で
  `V_DD-V_SS` と読める（2記号を畳んだ `f_SCK/t_SCK` は `/` を含む結果を採らないことで従来どおり落ちる）、
  (2) そもそも**そのページが読まれていない**（`CH32V003DS0.zh` の `表3-1` は p14、読んだページは
  15・16・18〜20・25〜27）、(3) 決め手は**ヘッダ規則が意図的に除外している**こと——絶対最大定格表は
  `符号｜描述｜最小值｜最大值｜单位` で **`条件` 列を持たない**のに、読む条件が
  `{symbol, min, condition} ⊆ 列` になっている（コメントにも明記。**バグではなく設計判断**）。<br>
  入れるなら (a) `描述`/`Description` を `parameter` として受ける・(b) 条件列の無い**新しい表の形**を
  受ける・(c) 表題を `TABLE_CAPTION` に足す、の3点。全corpus実測で **33表・33版、表題は4綴りだけ、
  両版ある16文書はすべて 1/1 の対称**、データ行は **389**。A11（1,208行）と同じ型の企画で、
  「足した行を1件ずつ原文と照合」が受入条件。**測定だけして変更は戻した**（読まない限り発火しない
  死んだ設定になるため）。<br>
  → **2026-09-10に実装し、`operating_conditions` へ入れる案は撤退した**。読むこと自体は通った——
  続きの断片は列の対応を**x座標で組む**（条件の欄は見出しを持たず断片ごとに幅が変わるので、列数でも
  `same_edges` でも続きと見なせない）。**248行が増え、247行は別実装での読み直しと一致**（残る3行は
  `CH32V103DS0.en` の続きページで単位の欄が空になるもので、値は原文どおり）。conflict は増えなかった。<br>
  **撤退の理由は下流**——`index/parts.csv` の供給電圧が `CH32L103` で `1.8..3.6V` から
  **`-0.3..4.0V`（破壊限界）** に化けた。`build_index.operating_summary` と `build_readme` は
  `symbol == "V_DD"` の min/max を取るので、**同じ表に「推奨動作範囲」と「絶対最大定格」が混ざると
  区別できない**。列に印を持たない以上、この repository の外の consumer も区別できず、表が
  積極的に誤解を招く。<br>
  → **`evidence/absolute_maximum_ratings.csv` を別の表として新設して完了**（2026-09-10）。
  **250行・全行 confirmed**（reference 0・conflict 0——両版が完全に対称）。`typ` 列を持たない・
  条件の欄が無名という形の違いも別表と整合する。**受入は250行を1件ずつ原本と照合**——列の
  対応づけを使わない別実装（末尾3列が min/max/unit、記号と単位は空欄なら直前の行から継ぐ）で
  読み直し、**250行すべてが zh と en の両版に在る**ことを確かめた。既存の正本は1バイトも動いて
  いない（純粋な追加）。<br>
  **残した穴**: en版310行のうち**60行**は記号が物理量を名乗らないので落としている
  （`∑I_INJ(PIN)` 11・`|△V_DD_x|` 系 32・`△` が別の行に落ちた綴りの崩れ 12 ほか）——
  どれも本物の事実なので、記号に名前を付けられれば拾える。zhだけが行を持つ12行も出していない。
  **この項目は完了。**
...
- [x] ✅ **縦に結合された値のセルが覆われた行で消える** — 2026-09-10に上の項目の下調べで見つけた。
  資料は「2つの条件で同じ値」を**値の欄の縦結合**で書く——`t_erase_32k` の
  `DBMODE = 0, single 32K bytes` と `DBMODE = 1, single 64K bytes` は `3`/`10 ms` を共有し、
  `V_DDA` の「ADCを使う／使わない」は `2.4`/`3.6 V` を共有する。`extracted_rows` は結合セルを
  先頭の行にだけ置くので、覆われた行が**値ゼロ**になり、正本に「単位だけの行」として出ていた。<br>
  **全corpus実測で10行・18セル・5文書の zh/en 対称**。5文書とも**原本を描画して版面が縦結合で
  あることを確かめた**（`CH32H417DS0.en` p.104 の `t_erase_32k`、`CH32V203DS0.en` p.39 と
  `CH32V103DS0.en` p.22 の `V_DDA`）。`fill_rowspans` で min/typ/max の欄だけ写す——記号と項目名は
  呼ぶ側が別の継ぎ方をしているので写さない。**正本は3,178行のまま5行が埋まり、値ゼロの行が
  4 → 0**（confidence 分布は不変）。**この項目は完了。**

- [x] ✅ **続きの断片で単位が落ちる** — 2026-09-10に絶対最大定格の受入で見つけた。表の途中で
  ページが変わると、そこから単位の欄が空になる（資料は「上の行と同じ」を空欄で書く）。
  `CH32V103DS0.en` は p.21 の `I_VDD … mA` の続きが p.22 に在り、`I_Vss`・`I_IO` の単位が
  落ちて zh（1ページに収まる）と食い違っていた。`operating_rows.read_edition` は表ごとに
  記号・単位・項目名を捨てるので、続きの断片へ持ち越さない。<br>
  **⛔ 素直に持ち越すと広すぎた**——全corpus実測で50行が増え、うち**17行は min/typ/max が
  3つとも空**（それまで単位が空で `UNIT_FOR` に落ちていた行が、継いだ単位で通ってしまう）。
  conflict も 14 → 15。**変更は戻した**。<br>
  → **同日に再突入して完了**。まず「値を1つも持たない行は出さない」を決めた。**当初はこの4行
  （`t_erase_32k`・`t_erase_sec`・`V_DDA`×2）を「事実を持たない雑音」と見ていたが、原本を
  描画したら縦結合の取りこぼしだった**——上の項目で埋めて値ゼロの行は0になり、この規則は
  「増えた行が値を持つか」の検査として働くようになった（素の持ち越しが連れてきた15行の
  値ゼロ行を全部落とす）。<br>
  もう1つ要ったのが**項目名の折り返し**——続きの断片の1行目は項目名の後半だけを持つので
  （`t_CONV` の `Total conversion time` は前ページで切れ、断片には `(including sampling time)`
  だけが載る）、置き換えずに繋ぐ。小文字か括弧で始まるものだけ（`fold_header_wrap` と同じ
  見分け方）。<br>
  **正本は 3,178 → 3,209 行**（増32・置換1）。増えた32行は**引用ページの1行に値が揃うことを
  機械確認**（32/32。confirmed 行は zh/en 両版で）。内訳は **`V_PVD` の PLS レベル17行**
  （V002 6・V003 3・V103 8。en 版だけが表をページで割っていて後半が読めていなかった。値は
  zh 版と完全一致）・`t_Iatr` 5・`t_CONV` 3・`ACC_HSI` 2・`V_IOFFSET`・`V_OH`（単位が付いた）・
  `N_END`・`C_ADC`・`T_J`。<br>
  **新しい conflict 1件は資料側**——`T_J`（結温度範囲）の「型号末位为7」が **en 105℃ / zh 115℃**。
  en 版 p.41 を描画して 105 を確認した。周囲の環境温度は両版とも 105 なので zh の 115 の方が
  もっともらしいが、資料が食い違っているので両論を basis に残す。
  絶対最大定格の表（`extract_absolute_maximum.py`）は**新しい表なので最初から持ち越している**。
  **この項目は完了。**

- [x] ✅ **下付きが `*` になっている値（806 glyph・14文書）** — `0.45*V+*0.41` は
  `LOST_SUBSCRIPT` で落としている。**2026-09-02に「PDFのToUnicodeが添字glyphを`*`に写す破損」と
  記録したが、2026-09-10に原本を描画して確かめたら誤りだった**——`CH32H417DS0.en` p.104 の
  `0.45*V*+0.41` も `CH32V205DS0.en` p.56 の `CL = 50pF, V* = 2.7-3.6V` も、**PDFの版面
  そのものが小さなアスタリスクを刷っている**（pypdfium2で該当セルを切り出して目視）。
  文字層の問題ではなく**資料側の組版の欠落**なので、engineを変えてもOCRでもfont形状解析でも
  復元できない——`DD` はどこにも無い。zh/en の件数が完全に同数（H417 224/224・V203 28/28）
  なのも、同じ組版工程が両版を作っている裏づけ。<br>
  **埋めない**（推測になる）。人向けMarkdownの警告文が「印刷されたページには本当の添字がある
  ——PDFを読め」と**嘘を言っていた**ので直した（読者が原本を見ても同じ `*` しかない）。
  記録も4箇所（`lost_subscripts.py`・`pipeline/README` 日英・parity検査）を直した。
  **この項目は完了**——直しようがないと分かったので、資料側の欠落として印を出し続ける。
- [x] ✅ **bit図のセルで同じ名前が2回混ざる（16件・4文書 → 0件）** — 旧「270件の straddling-word split」を
  2026-09-10に**測り直した**もの。古い数字は「語がセル境界を跨いで割れる」広い形の実測で、挙げてあった
  例のうち `TIMCMV|V[15:0]` は既に直っている（いま1セルに `TIMCMV[15:0]` で入る）。
  いま残っているのは**同じ名前が2回、文字単位で混ざる**形だけ:

  | 文書 | 件数 | 例（出ている綴り → 本当の名前） |
  |---|---:|---|
  | `CH32FV2x_V3xRM.en` | 8 | `TIM9TRISMT9RST` → `TIM9RST`、`CCRCCFCARILCFAIL` → `CCRCFAIL` |
  | `CH32V003RM.en` | 5 | `SPI1SRPSIT1RST` → `SPI1RST`、`TIM1TEIMN1EN` → `TIM1EN` |
  | `CH32H417RM.en` | 2 | `CMDSCEMNTDCSENTC` → `CMDSENTC` |
  | `CH32V407RM.en` | 1 | `CCRCCFCARILCFAIL` → `CCRCFAIL` |

  **判定は曖昧さが無い**——出ている綴りが「ある文字列2つの完全なシャッフル」になっていて、
  復元される名前は全部その表の `Name` 欄に載っている。全部 en 版だけで、zh 版には出ない。
  **bundle のセルは正しい**（`CH32V003RM.en` p21 は `SPI1`（行1-3・列3-4）と `RST`（行5-6・同じ列）で、
  縦に繋げば `SPI1RST`）ので、壊れているのは `apply_bitfield` の縦連結より後の層。
  bit列が狭くて名前が2〜3行に折り返す図でだけ起きる。
  → **同日に11セル直した**（16 → 5）。`fix_doubled_names` は混ざりを解く仕組みを前から持っていて、
  **候補が2つ立って決まらなかった**だけだった（記述表の `SPI1RST` と、合成した `SPI1RST1`）。
  記述表に実在する綴りを合成候補より優先するようにした。52セルが直り長くなったセルは0、
  正本CSVは無変化、parity 68/68 clean。
  → **残る5件も同日に直り、混ざりは0件になった**。`CCRCFAIL`/`CMDSENTC` は候補が2つとも
  記述表の実名（`CCRCFAIL` と `CCRCFAILC`）で決まらなかった。決め手は**長さ**——混ざった綴りは
  16字で、丁度2倍になるのは8字の方だけ。`_is_doubled` を足して最後の絞りにした。
  2手で計57セルが直り、長くなったセルは0。**この項目は完了。**

- [x] ✅ **値の欄に数ではなく語が入る行が、中文版だけ落ちる** — 2026-09-11に
  `reference` 133行の内訳を測って見つけた（「相手の版が同じ記号を持つのに片版扱いのまま」を
  数えた130件のうち、本物の取りこぼしはこの1件）。`CH32H417DS0` の `R_HSEXO_ext`
  （XO ピンの外部プルダウン抵抗）は「V_IO18 の既定出力 1.8V」の行だけ典型値が抵抗値ではなく
  **`浮空`／`Floating`（＝抵抗を付けない）**で、`reads_as_value` が CJK を通さないので
  **中文版 p.82 の行が丸ごと落ち**、英語版だけの `reference` になっていた。<br>
  英語版が通っていたのは規則ではなく**偶然**——`Floating` が8字ちょうどで「記号そのもの」の
  枝（`len <= 8`）に当たっていた。`VALUE_WORDS = {"浮空": "Floating"}` を `norm_value` に足して
  語彙で揃えた（公開する綴りは英語版のもの。`docs` 以外に CJK は出ない）。<br>
  **語彙はこの1語だけ**——全corpus実測: 値の欄に CJK を持つ行は `浮空` の1行、数を含まない値は
  他に13種あって**全て記号**（`VDD` 34・`tHCLK` 24・`VDDA` 10・`F_HCLK` 2 ほか）で、語はこの
  `Floating` だけ。両版の版面を pypdfium2 で描画して、zh p.82 と en p.94 が同じ行に
  `1200 / 浮空(Floating) / — / kΩ` を刷っていることを確かめた。<br>
  **正本は3,780行のまま1行が reference → confirmed**（`basis` に両版が並ぶ）。**この項目は完了。**
...
- [x] ✅ **doctest がどの検査でも走っていなかった** — 2026-09-11。正規化の規則
  ...
  **doctest が仕様**で、規則を直すたびに例を足してきたのに、**その半分はどの検査でも
  ...
  「そのモジュールの関数か」を見るため、`module_from_spec` で作った**別インスタンス**に
  対して走らせると `sys.modules` の先に import された同名モジュールの方が返り、
  **例が0件のまま黙って通る**（最初の実装がこれで、103例のうち33例＝`operating_rows` 26 と
  `wrap_rules` 7 を取りこぼしていた）。`import_module` で sys.modules と同じ実体を使う。<br>
  **3つの枝すべて壊して落ちることを確認**（`norm_value` の期待値を1字変える／存在しない
  module を import するファイルを置く／`>>>` を語として含むだけのファイルを置く）。
  **この項目は完了。**

- [x] ✅ **境界で1字が重なる隣接セル（523件 → 本物42件 → ツール側0件）** — bit図に限って「左セルの末尾＝右セルの先頭」を
  数えると523件。**記述表の `Name` 欄を当てて選別した**（2026-09-10）——両方が実名なら隣り合う別
  フィールドの偶然の一致で、それが**477件**。残る46件のうち**6件を直した**（`fix_doubled_names` の
  `heads` ガードが `n[:1] != flat[:1]` と広すぎて、同じ行に壊れたセルが3つ並ぶと巻き添えで拒否していた。
  守りたい形は「先頭1字を落とすだけ」なので `n == flat[1:]` に絞った。`TUSARTU4RST` → `USART4RST` ほか）。
  → **42件を4類型に分けて測り、8件片付いた**（2026-09-10）。**6件は誤検出**——`STSS[30:16]` は
  記述表に `STSS[30:0]` があり、31bit を16bitずつ2図に分けた上半分で正当（選別に「範囲を外すと実名」を追加）。
  **2件は候補が2つ残って決まらなかった**もので、「丁度2倍」が効かない奇数長。**余りが少ないほう＝長いほう**を
  採るようにして解けた（`CTIMCETOIMUTECO` → `CTIMEOUTC`。隣も `DTIMEOUTC` で同じ clear ビットの並び）。
  この規則で**17セル**が直った（`CEATAENDC`・`DBCKENDC`・`PENDRESET13`・`FACT22` ほか。長くなったセル0）。<br>
  → **隣と接する1字を重複の出所として許した**（2026-09-10）。列の境界を跨いだ字形は両側のセルに入るので、
  `CSS_HSE_DIS`|`SReserSved` の `S` のように名前に無い文字でも出所が説明できる。ただし**素の緩和は9セル
  壊した**——候補が増えて `TXFIFOE` が壊れた形に戻り、SDIO_ICR の `STBITERRC` が MASK 側へ化け、
  `EXTI11`|`EXTI10`|`EXTI9` の `EXTI10` が左隣の `1` を借りて `EXTI0` へ縮んだ。歯止め3つで確定:
  **(a)** 自セルだけで候補が1つに決まるならそちらを使い、緩和は候補0のときだけ、**(b)** 借りるのは英字だけ
  （数字を借りると索引が変わる）、**(c)** 候補が描画文字の接頭辞になる形（`ADC2_ETRGINJ_RM` →
  ...
  **綴りでも字形でも決まらない**ところまで来た。次に効くとすれば記述表の名前を使わない別の裏づけ
  （同じレジスタの他版・EVTヘッダの綴り）。<br>
  → **その「別の裏づけ」は測ったら誤りだった**（2026-09-11）。語彙をページの外へ広げる案
  （＝他版・EVTヘッダ・文書全体の名前）を全corpusで実測すると、**86セルが壊れる**——
  `SWIER0`〜`SWIER15` が `SWIE` へ 45件・`TCIF8`〜`TCIF11` が `TCIF` へ 9件・`USART2_RM` が
  `USART2` へ・極端な例では `G1I0F` が `10` へ。字形の切れ落ちが**別のフィールドの実名**に
  当たってしまうためで、いまの「このページの記述表だけ」という歯止めは**広げてはいけない**。
  `register_fields.csv` に正解の綴りが在るか（`USART5RST`・`EOCIE` は在る）は問題ではなく、
  組み直した綴りのほうが `4USARTRST`・`EOCI` と違う形になっているので、語彙を足しても当たらない。<br>
  → **本当の歯止めの穴は1つだけだった**——**bit図が添える範囲**（`EPTYPE` の下の `[1:0]`）を
  記述表が書かないこと。`_rebuilt_name` で「範囲を外すと記述表の名前」も採るようにして
  **2セルが直った**（`CH32FV2x_V3xRM.en` p.373 の `EPTYPEEP[T1:Y0]P E` → `EPTYPE[1:0]`、
  `STAT_RXSTA[1T:0_]R X` → `STAT_RX[1:0]`。版面を描画して確認）。緩めすぎないために
  **字が同じで空白だけ違うセルは触らない**——全corpus実測18件のうち16件がそれ
  （`IACTS [31:16]` は既に正しく、版面がその空白を刷っている）。<br>
  **残るこのページの3セル**（`CTRC_TTXR`・`DTOGD_TTOXG`・`STAT_TXST[A1T:0_]T X`）は、
  字形から組み直すと `CTR_TX`・`DTOG_TX`・`STAT_TX[1:0]` と**正しく出る**のに採れなかった——
  TX側の記述が **p.376**（図は p.373）に在り、`description_names` の窓（そのページ＋次ページ
  ＋表の連鎖）の外だったから。連鎖は p.375 の**説明セルの中の入れ子表**（`EPTYPE[1:0]`/`EP_KIND` の
  小表）で切れていた。<br>
  → **同日に直した**（`_nested_in`。上の節）。入れ子表に連鎖を奪わせないようにして窓が届き、
  **3セルとも版面どおりになった**。全corpusで連鎖が伸びたのは9表だけで、正本は不変。<br>
  **残るのは別の壊れ方**——`SR`|`Res`（版面がそう刷っている）・`5USARTRST`|`T4USART3URST`
  （組み直すと `4USARTRST` で語順が違う）・`AWDIE`|`EOCI`（`EOCIE` の `E` が左セルの箱に入る）・
  `INTEN15`|`5INTEN1`・`IACTS15`|`5IACTS14`（組み直すと切れ落ちる）。**どれも語彙ではなく
  箱の切り方の問題**で、字形から組み直しても正しい綴りにならない。<br>
  **2026-09-15に仕組みまで解剖した**（`CH32X315RM.en` p.78 の PFIC `IACTS`）。名前が
  **自分の1bit欄より広く刷られていて隣へはみ出す**のが原因: c0 の箱は `64.81..93.47` で
  字形は `A@68.1 C@74.1 T@79.7 S@84.6 1@89.1 5@93.3`（先頭の `I` は箱の左外）、c1 の箱は
  `93.47..122.62` で `5@93.3 I@95.2 A@99.6 …`——**境界上の `5` が左右どちらの箱にも入る**。<br>
  **字形の間隔で名前を切り分けることはできない**——同じ名前の中の字間は 4.4〜6.0pt なのに、
  隣の名前との間は **1.9pt** で、むしろ狭い（実測）。だから「x の隙間で run に割って
  run ごと箱に入れる」案は効かない。<br>
  **`fix_doubled_names` の綴り合わせでも採れない**。`5IACTS14` から先頭の `5` を落とせば
  記述表の `IACTS14` になるが、落とす字が**数字**なので歯止め (b)（数字を借りない）に当たる。
  この歯止めは `EXTI11`|`EXTI10`|`EXTI9` の `EXTI10` が `EXTI0` に縮むのを止めるためのもので、
  外せない。<br>
  → **2026-09-15に直した**。歯止め (b) を外すこと自体は正しかった——外すと**7セルが直り**
  （`5IACTS14`→`IACTS14` ほか3・`PEND9 SET8`→`PENDSET8` 2・`PEND9 RST8`→`PENDRST8`・
  `MR2 M98`→`MR28`）、**壊れるのは `EXTI10`→`EXTI0` の2件だけ**だった（全corpus実測）。
  そしてその2件は**別の歯止めで直に守れる**。<br>
  `EXTI10` が無防備だった理由は、既存の「`flat == base + そのセルの列のbit番号`なら触らない」が
  **1bit幅のセルにしか効かない**こと——`AFIO_EXTICR` は1フィールドが2bitを占めるので
  （`EXTI10` は bit21:20）列のbit番号が引けず、記述表も `EXTI0`・`EXTIx` としか綴らない。
  **行を見れば決まる**: `EXTI15 EXTI14 … EXTI8` と公差1で並んでいる。`_indexed_run` で
  「索引が連番で揃っている行のセルは、それ自体が正しい」を足して守った。<br>
  **7セルすべて原本と照合済み**（X315 p.78 の `IACTS15 IACTS14 IACTS13 IACTS12`・
  X035 p.37 の `MR29 MR28 MR27`・V407 p.102/104 と X315 p.75 の `PEND`/`SET8`・`PEND`/`RST8` を
  版面と文字層で確認）。正本は不変・parity 68/68 clean。<br>
  → **同日にさらに2セル直した**（`_respill_index`）。`CH32H417RM.en` p.38 の RCC リセット欄は
  版面が `USART5RST｜USART4RST｜USART3RST｜USART2RST｜SPI4RST` と刷るのに、
  **各セルが左隣の索引を頭に持って**出てくる（名前が1bit欄より広く、末尾の索引の字形が
  隣へ落ちるため）。2通りで直る——**自分の索引が残っているなら頭を落とすだけ**
  （`2SPI4RST`→`SPI4RST`）、落ちているなら**右隣の頭の数字が自分の索引**なので
  それで記述表を引く（`3USARTRST` ＋ 右隣の頭 `2` → `USART2RST`）。
  全corpus実測: 前者1セル・後者1セル・候補が複数になるものは0。版面と照合済み。<br>
  **残るのは2種で、どちらも資料側**:<br>
  ① `SR`|`Res` 3件——**版面がそう刷っている**（字形から組み直しても `SR`）。<br>
  ② `AWDIE`|`EOCI`（`CH32V407RM.en` p.175 の ADC_CTLR1）——**これも資料側だった**
  （2026-09-15に版面を描画して確認）。bit図が
  `DISCNUM[2:0]｜JDISC｜DISC｜JAUT｜AWD｜SCAN｜JEOCI｜AWDIE｜EOCI｜AWDCH[4:0]` と
  **名前を切り詰めて刷っている**（記述表は `EOCIE`・`JEOCIE`・`JDISCEN` と綴る）。
  抽出は版面に忠実で、直すべきものが無い。<br>
  ③ ツール側で残るのは `5USARTRST`|`T4USART3URST`（H417 p.38 の bit19/18）だけ——
  **自分の索引が右へ落ち、かつ右隣も壊れている**ので埋める数字が取れない。
  `CH32H417RM.zh` p.35 の `1EN`/`2EN` 系5セルは別の形（名前の前半が別の行に在る縦割れ）。
  → **さらに大きい塊が見つかって直った**（同日・`_join_wrapped_rows`）。`CH32H417RM.zh` p.35 の
  RCC クロック使能欄は**名前が2段に割れたまま**だった（上段 `SWPMI｜I2C3｜DAC｜…`／
  下段 `EN｜EN｜EN｜…`）。`apply_bitfield` の縦連結は「跨るセルがある」か「**上段**が同じ
  綴りの繰り返し」でしか働かず、**下段**が繰り返しのこの形はどちらにも当たらない。
  **繋いだ綴りが記述表の名前になるか**で決めるようにした——2段のまま正しい図（TIM の
  CCMR は上段が出力名・下段が入力名）は繋いでも名前にならないので当たらない。<br>
  全corpus実測: **19表・257セル**が繋がり、名前にならない列は corpus 全体で1つだけ
  （`CH32X035RM.en` p.71 の `TIM1_RM[`。原本の側が切れている）。だから条件は「外れは1列まで」。
  18ページの差分を全部目視して、`SWPMIEN`・`RXFIFOHFIE`・`SMBALERT`・`ADD10`・`CC4DE` のように
  **どれも正しいフィールド名に繋がる**ことを確かめた。境界重なりの検出数は 39 → **17**。<br>
  → **2026-09-16、残るツール側を直した**（`_index_family`）。決め手は**行が索引の族**で
  あること——`USART5RST｜USART4RST｜USART3RST｜USART2RST` のように並ぶ行では bit 位置と
  索引の差が一定なので、**両端が読めていれば間は族が決める**。`CH32H417RM.en` p.38 の
  `RCC_HB2PRSTR` は bit20 の `USART5RST` と bit17 の `USART2RST` が正しく、間の2つが
  `5USARTRST`・`T4USART3URST` だった。<br>
  **歯止めは2枚**。(1) 直すセルが**まだ名前でない**こと（`Reserved` の行を族で埋めるのを
  止める。`INTEN14｜Reserved｜INTEN12` の bit13 がこれ）、(2) 族が言う綴りの**索引以外の
  字が、壊れたセルの中に順番どおり在る**こと（`USART4RST` の `USARTRST` は `5USARTRST` の
  中に在る。`INTEN` は `Reserved` の中に無い）。(2) を外すと doctest が落ちることを確認した。
  見るのは**族の両端の間**だけ——行全体へ広げると `FBM1514131211`（5つのラベルが1セルに
  潰れたもの）が `FBM11` に化けた。**族が言う綴りが記述表に在ることは求めない**——
  H417 p.38 の記述表は名前を2つしか持たず `USART4RST` を含まないし、`LCK11` はその文書の
  どこにも綴られていない。綴りを決めているのは**その行そのもの**で、外の語彙は引かない
  （引いても結果は同じ3セルだと実測した）。<br>
  **置く位置が要**——綴り合わせの**後**に呼ぶ。族の両端が読めていることが条件なので、
  端そのものが壊れているうちは働けない（この時点まで bit20 は `USART5URST` だった。
  先に置いて差分ゼロになり、一度踏んだ）。<br>
  **全corpus 68 文書を書き出し直して、変わったのは3セルだけ**: `5USARTRST`→`USART4RST`・
  `T4USART3URST`→`USART3RST`（H417 p.38）・`LCK1LC1K 1`→`LCK11`（`CH32FV2x_V3xRM.en` p.148
  の `GPIOx_LCKR`）。**3セルとも原本を描画して版面と照合済み**。正本 CSV は不変・
  parity 68/68 clean・doctest 178→182 例。<br>
  **残るのは資料側の2種だけ**——`SR`|`Res`（版面がそう刷っている）と `AWDIE`|`EOCI`
  （bit図が名前を切り詰めて刷っている。`CH32V407RM.en` p.175）。**ツール側は0件。**
  `INTEN15`|`5INTEN1`（X315 p.73）は**この項目の記録が誤っていた**——実際の行は
  `INTEN15｜5INTEN1｜4INTEN13I｜3INTEN12` で正しく読めているセルが1つしかなく、族が
  立たない。索引が右へ落ちる形の連鎖なので、`_respill_index` の側の課題として残る
  （検出器の数え方では1件）。

### 2026-09-15 入れ子表に連鎖を奪わせない（記述表がページを跨いで途切れる件）

D19 の最後の項目で「次に効くとすれば入れ子表をまたぐ連鎖」と書いた案を測って、入れた。

- **原因**は `document_chains` が**ページ末尾の表で無条件に `open_chain` を置き換える**こと。
  表の中の表（説明セルの中の小表）がページ末尾に来ると連鎖を奪い、**次ページの続きが
  宙に浮く**——続きの断片は見出し行を持たないので `description_names` が名称列を見つけられず、
  bit図の組み直しの正解表がそこで途切れる。`CH32FV2x_V3xRM.en` p.375 の
  `EPTYPE[1:0]`/`EP_KIND` の小表（USBD_EPRx の `EP_KIND` の説明セルの中にある）がそれで、
  TX 側の説明が載る p.376–377 が別の連鎖になっていた。
- **歯止めは箱の包含**（`_nested_in`）。連鎖を奪う位置の入れ子表は全corpusで142あるが、
  **これで実際に連鎖が伸びるのは9表**（7文書・22ページ）。人向けMarkdownの差は
  「宙に浮いていた断片が、結合表への案内（⬆ Table continued）になる」形。
- **正本は1バイトも動いていない**——`document_chains` は `extract_debug_wiring` と
  `extract_option_bytes` も使うが、伸びた9表はどれもそれらの章ではない（`check_baseline` で確認）。
- **p.373 の USBD_EPRx の bit 図が版面どおりになった**——
  `CTR_RX｜DTOG_RX｜STAT_RX[1:0]｜SETUP｜EPTYPE[1:0]｜EP_KIND｜CTR_TX｜DTOG_TX｜STAT_TX[1:0]｜EA[3:0]`。
  残っていた3セル（`CTRC_TTXR`・`DTOGD_TTOXG`・`STAT_TXST[A1T:0_]T X`）は、字形からは
  正しく組み上がるのに正解表が窓の外だったもので、窓が届いたので採れるようになった。

### 2026-09-15 単位の規則の抜けを塞いだ（3,907 → 3,939・reference 132 → 95）

落ちている行の次の塊は「単位が記号の名乗る量と合わない」170行。**半分は規則の抜け**だった。

- **中文版だけが漢字で書く単位**——`年`（`Years`）・`次`（`Times`）。`UNIT_FOR` は既に
  `^t_RET$`＝`years?`・`^N_`＝`times?` を持っていたが、`年`/`次` に当たらず**中文版の行だけ
  丸ごと落ちて**いた（`t_RET` 24行・`N_END` 25行）。`VALUE_WORDS` に足した（`浮空` と同じ型）。
- **語で書く単位は単複が揃わない**——`CH32V003DS0.en` は `year`、`CH32H417DS0.en` は
...
**正本は 3,907 → 3,939 行**（+32・消えた行0・新しい conflict 0）。reference は 132 → 95 に
減った——中文版の行が揃って confirmed になったため。

**残る76行の棄却は正しい**: 24行は上の「2記号の分割」の相方（`t_SCK` に MHz、`f_TXC` に ns）、
20行は `F_PLL_IN` に `%`（「PLL input clock duty cycle」の行が上の行から記号を継いだもの）、
残りも同じ型。**この検査は効いている。**

### 2026-09-15 1つのセルが2つの記号を並べる行を開いた（3,780 → 3,907）

抽出が落としている行を掃いたら、最大の塊が「2記号が1行に畳まれている」169行だった
（`t_/t_r(SDA)_r(SCL)` ほか14綴り・192セル）。**原本を見ると畳まれてなどいない**——
版面は基底を `/` で、添字を空白で並べている:

    t /t              f /t              C /C
    r(SDA) r(SCL)     SCK SCK           L1 L2

平坦化すると在りもしない綴りになるだけで、**位置で対にすれば2つの記号**。値はその行が
1つ持ち、両方に掛かる（`SDA と SCL の立ち上がり時間 max 1000ns`）。`paired_symbols` で
記号ごとに行を開いた。1行ずつに割れた版（`t /` `su(HSYNC)` `t` `su(VSYNC)`）も同じ形で扱う。

**量が違う組をここで判断しないのが要点**——`f_SCK`/`t_SCK` は周波数と周期で、行の値は
どちらか片方のもの。分けたあと `keep_row` の既存の単位検査が落とす: `t_PixCLK` と
`t_MDC` は単位が `MHz` なので落ち、`f_TXC` は単位が `ns` なので落ちる（TXC の行は
「clock frequency」と書いてあるが値は 7.2/8/8.8 ns ＝周期）。**判断を増やさずに済んだ。**

**正本は 3,780 → 3,907 行**（+127・消えた行0・全て confirmed で両版一致）。内訳は
`t_r(SDA)`/`t_r(SCL)` 32ずつ・`t_f(SDA)`/`t_f(SCL)` 16ずつ・`t_su/h(HSYNC/VSYNC)` 3ずつ・
`t_w(SCKH)`/`t_w(SCKL)`・`t_r(SCK)`/`t_f(SCK)`・`f_PixCLK` 3・`f_SCK` 2・`f_MDC` 2・
`t_TXC` 2・`C_L1`/`C_L2`。conflict も reference も動いていない。

### 2026-09-15 不変量を3つ測って、1つだけ足りなかった

正本に対する「安い機械的な不変量」をさらに探して、3つ測った。**2つは既に検査が在り、
1つは向きが片方だけだった**——測ってから足すと、重複した検査を増やさずに済む。

- **レジスタの field の bit が重ならない** → **使えない**。重なりは1,471組あるが、
  ほとんどが正当——EVT ヘッダは群のマスク（`LOW` = bit31:29）と個別ビット（`LOW0`〜`LOW2`）の
  両方を定義する。名前が前置でない組に絞っても2,178組残り、その大半は
  `ECDC_IV` の `31T0`/`63T32`/`95T64` のような**128bit レジスタの語**（配列なので全部 bit0 から）。
...
- **割り込み番号が一意** → **番号→名前は既に在ったが、名前→番号は見ていなかった**。
  ...
  （`CH32V20x` の `TIM5` を同じ条件で64番にも足す → 名指しで落ちた）。<br>
  **条件（SKU 変種のマクロ）を鍵に入れるのが要点**——`CH32V20x` の vector 61 は `_D6` では
  `UART4`、`_D8` では `ETH` で、条件が違えば同じ番号が別の割り込み（番号が重複する86行は
  すべて条件を持つ）。

### 2026-09-15 自動更新が置いていく凍結台帳（F-65）を直し、機械で持つようにした

資料取り込みの走行で、**取り込みと無関係な台帳のずれ**が1件混ざっていた——
`catalog/toolchains.csv` が origin の自動commit `fdb0e20`（update tool versions）で
書き換わったのに、凍結台帳が同じcommitに入っていない。handoff が「目録の自動更新は
台帳も同じcommitに含める」と決めている型の抜けで、**次に正本を触った人のcommitが
赤くなる**（実際、取り込みが動かした15表と区別する手間が要った）。

- `toolchains.yml` に `check_baseline.py --record` の段を足し、commit に
  `pipeline/baseline/tables.csv` を含めた。`update.yml` が `catalog/documents.csv` に
  対して既にやっていたのと同じ形で、理由も同じ（常時赤い検査は読まれなくなる）。
- **同じ抜けを機械で捕まえる `check_tables.workflow_ledger` を新設**——正本の置き場
  （`catalog/`・`evidence/`・`index/`）か `-A` を `git add` する workflow は
  `check_baseline.py --record` を走らせること。**正本を commit する workflow は2つだけ**と
  掃いて確かめ（`check.yml`・`structured-repro.yml` は commit しない）、両方外して
  落ちることも確認した。決まりを文章ではなく検査で持つ。

### 2026-09-15 F-64 を閉じた——版が食い違う pin 機能を `conflict` で1行にした

同じ pad で両版が違うことを言っている組を `conflict` にまとめた（`extract_pin_tables.pair_dissent`）。
それまでは zh/en それぞれの `reference` 2行で並び、**`index/conflicts.csv` に出ず総数にも
数えられていなかった**。

- **対応付けは signal を先に**。機能の identity は名前のほうで、AF 番号はその値だから。
  route を先に見ると、zh の `LTDC_CLK(AF14)` が en の `SDRAM_A3(AF14)` と当たって
  「AF14 の機能名が変わった」という**在りもしない食い違い**になる——原本を読むと
  中文版 V1.9 は `LTDC_CLK` を AF13→AF14、`SDRAM_A3` を AF14→AF15 と**それぞれ1つ
  繰り上げた**だけで、間に何かを挿れてはいない（両版のセルの全文を突き合わせて確認）。
  名前で当たらないものだけ route で当てる。**候補が1つに決まるときだけ**対にして、
  決まらなければ片版の `reference` のまま残す。
- **出すのは原典（中文版）の読み**で、英語版の言い分は `basis` に
  `+!pin-table:en(=af-13)`／`+!pin-table:en:signal(=QSPI2_SIOX0)` で残す。
  **中文版が正しいことは版面で裏が取れる**——英語版のままだと `QSPI2_SIOX0` が
...
- `build_conflicts` は手を入れていない——`field` は `basis` が名指す列名から決まる
  仕組みが既にあり、`!pin-table:en:signal(=…)` は `signal`、`!pin-table:en(=…)` は
  `ASSERTS` の `route` になる。

### 2026-09-15 区切りの無い `(AFn)` と、索引の「数字を全部繋ぐ」チャネル数

F-64 の対応付けを測る過程で、**在りもしない綴りが1つ正本に出ていた**のを見つけた。

- **`(AFn)` はそこで語が終わる**（`extract_pins.signals`）。`CH32H417DS0.zh` p.29 の `PF8` は
  `TIM11_CH3(AF13)QSPI1_SIO0(AF10)` と刷る（区切りの `/` が無い。英語版は同じ2つを改行で
  分けていて正しく読める）。分けないと `TIM11_CH3(AF13)Q` という綴りが `default` の機能として
  出て、続く `QSPI1_SIO0` が頭の `Q` を失って `SPI1_SIO0` になっていた。
  全corpus実測: `(AFn)` の直後に区切りが一切無い箇所は**この1つだけ**。
  正本は 28,473 → 28,471 行で、消えた2行はどちらも在りもしない綴り。代わりに
  `QSPI1_SIO0 af-10` と `TIM11_CH3 af-13` が **reference → confirmed**（両版が揃った）。
- **その綴りの崩れが索引まで届いていた**。`index/timers.channels` は役目の綴りから数字を
  **全部繋いで**いて（`re.sub(r"\D", "", "CH3(AF13)Q")` → `313`）、`CH32H417 TIM11` が
  **313 チャネル**と出ていた。`CH` の直後の数字だけを読むように直す。<br>
  **同じ壊れ方がもう1件あった**——`CH32V407 TIM1` が **13 チャネル**。こちらは綴りの崩れでは
  なく `TIM1_CH1_3` の `_3`（CH32V407 の remap 変種の番号）を繋いだもので、**資料は正しく、
  索引の数え方だけが誤り**だった。どちらも 4 になる。
- **`check_tables.timer_channels` を足した**。チャネル番号は **1から連番**（全corpus実測
  2026-09-15: 55 の timer×family すべて）で、`index/timers.channels` は pin に出ている
  最大値と一致する。2つの枝とも壊して落ちることを確認
  （`channels` を 9 に書き換える／`CH2` を `CH7` にして連番を崩す）。
  行が正しそうな顔をしたまま索引の数だけが化ける型を捕まえる。
...
ページ数はどちらも不変（DS 130・RM 879）だが中身が動いている。**コード変更とは別のcommit**。

- **review sidecar の引き直しが要った**。`structured/CH32H417RM.zh/review.json` は原本 hash に
  縛られていて（D16「原本更新時に古い review を自動流用しない」）、converter が第1段で止まる。
  34件の判断を**スクラッチへ変換した新版の bundle と突き合わせ**、表題（表番号＋本文）で
  1対1に引き直した——**34/34 が一意に決まった**（USB 端点の3件だけは本文が同じ表題を
  2群が共有するので、表番号込みの表題で解いた）。ページは章9〜31で2つ前へずれる。
- **新版が表番号の刷り間違いを2つ正していた**——`表10-35`→`表9-35`（GPIO 相关寄存器列表）・
  `表10-36`→`表9-36`（AFIO）。`printed as …` の note は役目を終えたので落とした。
  **残る刷り間違いは1つ**で、p828 の LTDC は章43なのに `表38-1` と刷る（canonical `43-9` のまま。
  新版の版面で確認。文書中に `表 43-N` の表題は1つも無い）。en 版の表題の有無を言う note 2件は
  刷り番号と無関係なので残した。
- **正本の動き**: `pin_functions` 28,483 → 28,518 行・`index/pinout` 24,982 → 25,017 行。
  中身は3種類——(1) `CH32H415REU6 PB10` の12機能が **reference → confirmed**（中文版に
  その行が入った）、(2) 中文版だけが持つ機能の追加（`PB12 LPTIM2_CH2`・`PB8/PB9 SDRAM_CKE0/1`・
  `PE9/PE10 USART8_RTS/CTS`）、(3) **両版が食い違うようになった行**が confirmed から
  zh/en 別々の reference 2行に割れたもの（`PB13 QSPI2_SIOX0`(en)/`SIOX2`(zh)・
  `PB14 SIOX1`/`SIOX3`・`PE14 LTDC_CLK af-13`/`af-14`・`PE14 SDRAM_A3 af-14`/`af-15`）。
  英語版はまだ改版されていないので、**資料側の齟齬**として両論が残る。
  ほかは RM のページ参照のずれ（`timers`・`dma_requests`・`registers`・`register_fields`）と、
  中文版が脚注記号 `（5）` を落としたことによる `product_attributes` の表示差、
  `SDIO DCTRL` が RM 番地照合を得て reference → confirmed。
- **⚠ この食い違いは `index/conflicts.csv` に出ない**（185行のまま）。同じ pad・同じ signal で
  AF 番号だけが違う形は、`pin_functions` の行の鍵（`part_number, pad, signal, route`）が
  `route` を含むため**別の行**になり、どちらも `reference` で並ぶ。人向け README では
  `PE14 | … | LTDC_CLK | LTDC_CLK | …` と同じ signal が2つの AF 列に出る。
  **`conflict` として数えるべき形**に見える——次の課題（下の F-64）。
- **取り込みと無関係な台帳のずれを1件見つけた**: `catalog/toolchains.csv` が origin の自動commit
  `fdb0e20`（update tool versions）で書き換わったのに**凍結台帳が同じcommitに入っていない**。
  handoff が「目録の自動更新は台帳も同じcommitに含める」と決めている型の抜け。今回の
  `--record` で一緒に直したが、**自動化側の穴は残っている**（F-65）。

### 2026-09-24 エラッタ台帳の腐りを検査に載せた／H417 RM のロット注記4件

**検査の穴**: `scan_errata` は「既知 23 件中 19 件を確認・未確認 4 件」と印字していたのに、
**終了コードは NEW の件数しか見ていなかった**——`regenerate.py --verify` は緑のまま通り、
台帳の行が根拠を失っても誰も気づかない。未確認を種類で分けて、**資料を引く行の `match` が
どこにも当たらなければ落ちる**ようにした。いまの文に無いのが正しい2種類（実測の行・過去の版を
名指す行）は除外する。`x315-clock-tree-lot5` の `match` を空振りさせて落ちることを確認した。

**未確認4件の実態**: `x035-*-open-drain` の2件は実測（2026-09-22）で、資料に文が無いのが正しい。
`h41x-lot5-features`・`h41x-clock-tree-lot5` の2件は**私が一度「X315 から類推して書かれた」と
見立てたが誤りだった**——ミラーの履歴で全版を読み直すと、`CH32H417DS0` の **V1.3〜V1.8 には
両版とも実在**し（「对于批号第5位为0的产品，未提供GPHA、Ethernet、SerDes、CAN功能」・
「上图中标蓝的部分仅适用于批号第五位大于0的芯片」）、**V1.9（zh 2026-09-10・en 2026-09-21）で
WCH が両方を消した**。RM のクロックツリー図の但し書きも同じ改版で消えている。
消した理由（制約が解けたのか、そのロットを出荷しなくなったのか）は資料が言わないので、
**行は残して出所を `CH32H417DS0.PDF@V1.8 p.4` と過去の版にした**（市中にそのロットは残る）。

**H417 RM のロット注記4件を足した**（`h41x-*-lot3`。条件は「批号第五位小于3」で既存の「=0」とは別）:
I3C の IBIF がデータ無し IBI で立たない（zh p.374／en p.440）・SAI の FSPOL=0 で外付けプルアップが要る
（p.672／p.792）・QSPI のメモリマップがデータ専用で命令アクセス不可（p.677,682／p.798,804）・
GPHA の `PL[13:0]` は MODE=011b のとき 0 より大きく（p.859／p.1009）。10箇所すべて KNOWN になった。
英語版は `match` を行の折り返し（` / `）に耐える形にした。manual を引く行は `--rm` の全走査でだけ
判定する（既定の走査は DS だけを見るため）。

**残したもの**: H417 RM の `--rm` 走査の残り10件はロット依存ではない誤検出（I2S の手順の
「倒数第」「penultimate」・SAI の `slot number` が `lot number` に当たる）。うち1件、en p.722 の
SDIO `DTMODE` の注記が「**CH32F20x_D8** のロット番号下6桁」と別の製品の話をしている——
他の manual からの写し間違いと思われる資料側の記録（en 版だけ）。`--rm` 全体の NEW 235 件の
...
- **converter 1.7.1**: 表セルの下付き復元を exporter から converter へ移した。詳細と
  ...
  `operating_conditions` の `I_DD` 系1,207行が消える）は
  [markdown-qa-log](markdown-qa-log.ja.md) の該当節と `pipeline/README` 7. に。
...
🔶 **セル格子 family（49件・high 13）に着手**。**入れ先が一意に決まる型**——「表領域の
外に落ちた最外列」——を converter 1.10.0 で取り込んだ（`recover_outer_column`）。
`位`列（`[31:15]`/`14`）や`复位值`/`Reset value`列が**Markdownのどこにも出ていなかった**
のを復元。約40表・16文書。素朴な条件では1,000表超が当たり大半が図の散乱ラベルだったので、
4条件（本物の罫線表・1列ぶんの幅・縁に接する・全行帯に中身）で49表に絞った。
検証: parity 68/68 clean・新経路CSV 4本 byte 一致・凍結batch 6/6 byte 一致。
**残した型**（入れ先が一意でない）: rowspanの誤りによる列ずれ・幽霊列・`△`の隣セル誤挿入・
**bit番号行が外れてフィールドが誤ったbit位置に出る**（V003RM.en p7。この family で最も
深刻だが`apply_bitfield`が絡む。次はここ）。詳細は [markdown-qa-log](markdown-qa-log.ja.md)。

### 2026-09-07 HIGH 46件の再突合と、そこから直した5件（converter 1.10.0〜1.10.2）

**まず再突合した**——検証4窓の high 46件を1.10.0の出力に対して再判定（33文書・46ページ）。
**18件はすでに解消**していた（読み順6・列の空3・セル内改行2・指数2・切れ端1・列ずれ2・
他2）。私が「family で最も深刻」と書いた `CH32V003RM.zh` p7 の bit 位置誤りもその中に
あり、**古い報告のまま着手すれば正しい出力を壊していた**。修正が別の指摘を巻き込んで
解消するのが18/46で起きるので、**再突合は毎ラウンド挟む工程**にする。

**直した5件**（詳細は [markdown-qa-log](markdown-qa-log.ja.md)、実装は `pipeline/README` 13〜17）:
最外列が表領域の外（約40表・16文書）・数式が走査線277枚に割れて消える（2文書）・
下付きの離脱（419件中338件）・幽霊行と幽霊列（候補18件。**1つの根が3つの症状**）・
rowspanのページ跨ぎ（3件）。
**検証**: 凍結tool 23出力・新経路CSV 4本 byte 一致（`operating_conditions` 2,796行含む）・
parity 68/68 clean・検査5本通過。正本CSVは1バイトも動いていない。

**⛔ 見送った4件**（測定結果と再挑戦条件をQAログに記録）: セル境界の大量二重取り
（4段階絞っても分離不能。797件の主体は**すでに下流で正しく出力**されている）・
見出しの昇格（サイズ4%差では分離不能、**PDFアウトラインでも救えないと確認**）・
`△`の隣セル誤挿入（組み直しでも直らないと確定）・WCH-Linkの✓✗（文字層に無い）。

**測定してから実装する**のが一貫して効いた——素朴な条件のままなら3つの型すべてで本物の
データを壊していた（最外列 1,000超→49で実装、二重取り 2,230→797で見送り、
見出し 1,115→426で見送り）。`clean_reset_column`が3回同じ事故を起こしたのは、この測定を
経ずに「語彙に無いものは消す」を入れたため。

### 2026-09-08 残り26件の再突合と、そこから直した3件（converter 1.11.0）

**再突合の結果**: 26件（21文書）を1.10.2の出力に対して再判定——`resolved` 5・
`partly_fixed` 6・`still_present` 15。判定前に機械で確認できた4件（V003DS0.en p20の
`VHSEH(1)`、V205DS0.en p64とL103DS0.zh p49の数式PNG、H417DS0.en p104の7列）は
エージェントの判定と**一致**した。

**副産物が本題になった**——`new_defect` 8件のうち**5件が前ラウンドの修復の副作用**で、
うち1件は**値の捏造**だった（`CH32M030DS2.zh` p3で`PA8`の`ADC_IN7/MCO/…`と`SPI_MOSI_3`が
引脚25 `ISP1`の行に被さった）。`extend_boundary_spans`が「境界の空セル＋直上が境界で
終わる」を箱の切断の署名として扱っていたが、**次ページの行が正当にその列だけ空**な場合と
見分けが付かない。全corpus実測: 発火11,015件のうち**1,185件が中身のある行に発火**。

**直した3件**（詳細は [markdown-qa-log](markdown-qa-log.ja.md)）:

| 直したもの | 母数 | 効果 |
|---|---:|---|
| 実データ行への吸い上げを止める | 1,185件 | 値の捏造が止まる |
| 空になった境界行をグリッドから消す | 2,283件 | 幽霊`<tr>`とrowspanのずれ（`VIL`が7→6等） |
| 一覧セルの括弧前の空白を無視 | 24セル | `ADTM3*GPTMCRC…`が1行1項目に戻る |
| グリッドから抜けた列を埋める（1.11.0） | 1表 | `WCH-LinkUserManual.zh` p4の行見出し14件 |

**検算**: タグを外した本文で旧出力と比較して**11,695ファイル中1ファイルだけ差があり、
欠けた語0・増えた語0**。その1件も改善（`ADC1`行がヘッダに吸収されていたのが分かれた）。

**手順の教訓**: parityは`.cache/structured-markdown`と照合するので、**export側を直したら
markdownを書き直してからparityを見る**。順序を守らないと自分の修復を欠陥と読む（今回
`CH32FV2x_V3xRM.en` p166で2件の誤検出が出た）。

### 2026-09-08 最外列の取り込みを緩めて408セルを回収（converter 1.12.0）

`CH32L103RM.en` p13の3つの指摘（`Bit`列が空・幽霊列・`[31:21]`の欠落）が**1つの根**
だった。`recover_outer_column`（1.10.0）が拾うべき形なのに、**縁との隙間8.4ptが閾値8.0ptを
0.4pt超えて**落ちていた。隙間は「列幅−文字幅」で決まるので閾値として筋が悪い。

3つ緩めた（測定つき。詳細は [markdown-qa-log](markdown-qa-log.ja.md)、実装は
`pipeline/README` 21〜22）:

| 緩めたもの | 増える | 質 |
|---|---:|---|
| 隙間の上限 → 縁までの全幅が1列ぶん | 41 | 全部本物。図の断片（隙間219pt）は落ちる |
| 3行以上 → 2行以上 | 64 | 全部`Bit`/`Resetvalue`/`复位值`/`位`/レジスタ名 |
| 片側でreturn → 左右の両方 | 13表 | `Bit`と`Reset value`が両方外に出ていた表 |

**合計94表・408セル**（既存49表と合わせて143表）。これらは行の中心が表bbox内なので
`reading_order`からも外れ、`CH32V003RM.en` p132では`Reset value 0xFFFF`が表の外に
`Reset`/`value`/`0xFFF`/`F`と**値を分断された本文**として落ちていた。

取り込んだ列の綴りは**視覚行の切れ目に`\n`を入れて`cell_html`へ渡す**——`Reset`/`value`は
空白で繋ぎ`0xFFF`/`F`は繋がない判定を既に持っている。語間の空白は隙間から復元（35セル、
全部`Reset value`型の見出し、閾値0.25〜0.50で結果が同じ）。

**同じ周に入れた残り2件**: 角セルの二重出力（74件・26文書。`H417DS0.en` p3で`QEU6`〜
`REU6`が2列ずれていた分）と、ページ跨ぎ結合の幽霊列（1.12.0で再測して47表→colspanが
保たれる14表だけ寄せる。`L103RM.en` p13・`V407RM.en` p402・`X315RM.en` p155は
**最外列の回収で対象外になった**＝幽霊列の指摘の一部は最外列の欠落が原因だった）。

**検算**（詳細は [markdown-qa-log](markdown-qa-log.ja.md)）: 文書単位の非空白文字で
**失われた文字387＝74件の重複除去と一致**・増えた1,634、空の`<tr>`は2,314→**29**
（残りは全て旧出力にも在った）、正規CSV 60本 byte一致、**parity 68/68 clean**。

**次周へ**: 継続断片に欠けた最外列を結合表の列境界で拾う（**174件・36文書**。
`CH32xRM.zh` p180の`R32_USART3_GPR`を含む）。出力に174セルを新規追加する変更なので、
6つの修正が乗った今回の周とは分ける。**再突合を先に挟む**——今回の修正で解消した分を
...
### ✅ CH32X035DS0.zh V2.3 の取り込み——撤退したが同日に再突入して完了（2026-09-09）

V2.3 は出力電圧特性を**ポート群ごと**に分けた（`VOH（PA0-PA23）` は 30/50mA、`（PB0-PB21）` は
6/10mA、`（PC0-PC7…）` は 8/16mA。**PA が 50mA を引ける**という新しい事実）。ところがポート群が
記号セルの中にあるため `norm_symbol` が `VOH_(PA0-PA23)` を返し、凍結 `build_operating.keep_row` の
白名簿 `KEEP`（`V_` の形しか通さない）が**新しい12行を全部落とす**。その結果 en（まだ V2.2）の4行が
相手を失い、2件が reference へ、2件が **偽の conflict** になる（`2.8`/`0.3` の出所は p32 の別表
「静态输出高电平」で一般I/Oではない）。取り込むと正本に偽の食い違いが入り、新しい規格は1行も
入らない——**撤退**し、正本4表・manifest・bundle を控えから戻した（作業ツリーは HEAD と同一・検査4本通過）。
...
正本2,797行がともに byte 一致）。そのうえで再突入を**3つに分解して測った**:

1. **ピン群を記号から条件へ** ✅ 済み（`split_pin_group`）。`P`＋英字＋数字で始まる括弧に絞った——括弧つき
   記号は全corpus 399件・31種類で**ピン群は0件**（他は`t_SU(LSI)`等の下付き修飾）。出力は byte 不変。
2. **新しい表に到達できない** ✅ 済み（`TABLE_CAPTION`）。`read_edition` は「見出し語のページ＋続き1ページ」しか
   読まず、V2.3 で見出し語が p26→p25 に動いて表3-14（p27）が窓の外に出た。**ページ規則を緩める案は測って捨てた**:
   現行の生の行数 3,309 に対し、見出し語に表の題を足すと 3,932（+19%）だが**正本から34行が消える**
   （版で非対称に増えて zh/en の対応が崩れる。行が続く限り 5,561（+68%）・列が同じ限り 5,999（+81%）はもっと大きい）。
   採ったのは**表題でその表だけを読む**形——bundle は表ごとに表題を持つ（`bundle_pages.captioned_tables`）ので
   ページの隣接に依らない。増えるのは出力電圧の記号だけ **67行**で、消える行は無い。全corpusに出力電圧特性の表は48あり
   **31表がページ規則では届いていなかった**（`CH32V103DS0` は zh だけ届いて en が届かない非対称もこれで解消）。
3. **基礎行は en 主導** ✅ 済み（en を待たずに済む形にした）。正本2,797行で **zh のみを根拠にする行は 0** だった
   （en のみ181・両方2,616）。**ピン群を持つ zh 余りに限って** `reference` で出す経路を足した——群がある＝
   「en が一般名で言っている同じ規格の適用範囲を分けた版」なので en の英語名を借りるのが正しい。群の無い zh 余りは
   別の表の別の事実であることが多く（`CH32X035DS0.zh` p32 の `静态输出高电平`）、記号だけで英語名を借りると
   **名前を偽る**ので出さない。V2.2 時点で群を持つ zh 余りは **0行**——この経路は「ポート群で分けた資料」だけで働く。
   併せて**対応付けを二段**にし（値の一致する対を先に全部取る）、**conflict の相手を「群が揃い条件の有無も揃う」ものに絞った**。

**取り込み結果**（`--accept-sources`）: `operating_conditions` **2,864 → 2,876 行**（X035 は 98 → 110）、
`index/conflicts.csv` 197 → 198、confidence は confirmed 2,638 / ref 210 / conflict 28。**鍵ごと消えた行 0**・
データ列の **CJK 0**・値まで同じで信頼度が違う組 **0**・**偽の conflict 0**。入った12行は原文と1セルずつ照合済み
（PA と PB は sink が同じ 50/100mA で source だけ 30/50 対 6/10mA という資料どおりの非対称も保っている）。
同時に動いた既存**11行は全部 `CH32X033;CH32X035`**（更新した文書そのもの。他の文書は1行も動いていない）で、
全部原文で裁定した——一般I/Oの `V_OH`/`V_OL` 4行が confirmed → `reference` になるのは正しい
（zh V2.3 が一般I/O行をポート群ごとの行に置き換えたので en V2.2 の一般I/O行が裏を失った）、X035 `I_DD`（zh 560µA 対 en 290/480µA）は**資料の本物の食い違い**で
conflict が正しい、`V_DD Performance may be reduced` は conflict → confirmed の改善、
**`V_DD Recommended not less than 2.5V` の conflict は残る1件の誤り**で、原因は照合ではなく**到達**だった——
正しい相手は zh p.34 `表3-26 OPA运放特性`／p.35 `表3-27 CMP电压比较器特性` の `建议不低于2.5V 2/5/5.5` で
en 行と完全一致するのに、**その2ページが窓の外**（X035 で読めている zh は 9・23–27・32・33、en は 10・26–29・36・37）。
相手を失った en 行が p.32 ADC 表の `额定性能`（3.0/5/5.3）を取って偽の conflict になった。
**直し方は出力電圧と同じで、OPA/CMP の表題を `TABLE_CAPTION` に足す**（同じ穴で `V_CM`・`V_IOFFSET` 2行も
en 単独の reference）——**同日に別commitで直し、confirmed に上がった**（穴は5つあって正本は 2,876 → 3,140 行。
[markdown-qa-log](markdown-qa-log.ja.md) の「OPA/CMP の特性表が読めていなかった」）、`I_DD Standby/Stop` 4行は V2.3 の表番号振り直しで zh 側の裏が
取れなくなり confirmed → reference（同じ表の Run/Sleep 行は confirmed のまま）。
**入れなかったガード**: 「conflict は min/typ/max のどれかが一致すること」を足すと 22件中3件が reference に落ちるが、
その3件に上の X035 `I_DD`（本物）と `CH32V203DS0` の `g_m`（17 対 25.3。本物）が入る——**本物を隠すので入れない**。
台帳は動いた4表を中身を見たうえで再記録した。詳細は[markdown-qa-log](markdown-qa-log.ja.md)。

**4点とも入れた**（同日。「en 版を待つ」は選べないとのユーザー判断）。(1) ピン群を条件へ、
(2) 表を**表題で**選ぶ（`bundle_pages.captioned_tables`。ページ規則を緩める案は正本から34行を
消したので捨てた）、(3) 対応付けを二段に（先に一致・後で食い違い）、(4) 食い違いの相手を群と
条件の有無で絞り、**ピン群つきの zh 余りを reference で出す**。

現行原本（V2.2）での正本: **2,797 → 2,864 行**（増えるのは出力電圧の記号だけ67行）、
**消えた行 0**・**CJK 0**・conflict **36 → 27**、信頼度が動いたのは15行（6件 reference→confirmed、
8件 conflict→reference＝偽の食い違い、1件 conflict→confirmed）。全corpusの出力電圧特性48表のうち
**31表がページ規則では届いていなかった**のが埋まる。

V2.3 を差し込んだ実測: ポート群ごとの**12行が入り**（`PA0-PA23, I_IO = 30mA` など。PA が 50mA を
引けるという新しい事実）、en がまだ言っている一般I/Oの4行は reference、**偽の conflict は 0**。
X035 の行は 98 → 110。en 版が V2.3 になれば群が揃って confirmed に上がる。
詳細は [markdown-qa-log](markdown-qa-log.ja.md)。
...
- **作業中に原本が動いた**: timer の pull で `CH32X035DS0.zh` が V2.2→V2.3（電気特性章 p23–35 の
  ...
  コード変更の検証を先に回す。取り込みは別コミット。
  このリポジトリ自身の pull は作業ツリーが dirty で跳ばされている（設計どおり）。
  **取り込みは同日に完了**——一度撤退したうえで `operating_conditions` を作り直して再突入した（上の節）。
  正本は 2,864 → 2,876 行で、ポート群ごとの出力電圧12行が入った。
...
| F-51 | CH32H417 の PF12/PF13/PE7 の remap 欄に**中文版だけ**が `UHSIF_PORT0_1`〜`_2` と書く（英語版は空欄）。RM の格子はその3 padを値0（既定）に当て、値1は PC1→`PORT3` のようにずらす | 3行 | 資料 | ✅ **解決**（2026-09-23。**資料側が直った**）。**英語版 RM の 1.7→1.8 改版**で格子が値1を PF12/PF13/PE7 に当てるようになり、`remap_routes` に `afio-uhsif-port-remap` 値1 の3行が入った（`UHSIF_PORT0`→PF12・`UHSIF_PORT1`→PF13・`UHSIF_PORT2`→PE7。**全部 confirmed**——`candidates(datasheet-pin-table+rm-remap-grid:en+zh)` で pin 表と両版の格子が揃った）。`index/pinout.csv` の selector が空の `remap-N` 行は **3 → 0**、`check_tables.KNOWN_SELECTOR_GAPS` は空になった。中文版の pin 表だけが書いていた主張を、英語版の格子が追認した形 |
...
| F-59 | CH32V407 の **en 版 RM が DMA 表の題を `Figure 11-2 DMA1 peripheral mapping table…`／`Figure 11-3 DMA2 …` と誤植**し、`build_dma_requests` が両表を既定の DMA1 と読む | V407 の DMA2 38 要求（en では DMA1、zh では DMA2 になって対にならず reference 76 行） | 資料→ツール | ✅ **解決**（2026-09-04、en 版 RM が mirror に追加された日）。`CAPTION` に `图|Figure` を足した。図の題は既存の `CAPTION_TABLE`（「映射表」/「mapping table」）で弾くので図は表にならない。全 RM bundle を走査して `Figure … mapping table` は V407 en の 2 件だけ——他 family の出力は byte 不変。V407 は 73 行が zh+en の confirmed に戻る。誤植そのものは台帳に記録 |
| F-60 | CH32V407/V467 の **`FSMC_NADV` の remap 格子の読みが zh 版と en 版で割れる**。表10-40 は zh/en とも「`FSMCEN=0` も `=1` も PB7」だが、en 版は表がページ跨ぎで見出し無しの断片（p126、先頭行が `NADV PB7 PB7`）になり、`build_all` の格子読みが zh を `remap 1→PB7`、en を `remap 0→PB7` と読む | `remap_routes` 2 行（`afio-fsmc-remap,1,FSMC_NADV,PB7` V407/V467）が落ち、`pin_functions` の PB7/FSMC_NADV 6 型番が confirmed→**conflict**（`!rm-remap-grid(=remap-0)`） | ツール | ✅ **修理済み**（2026-09-10）。原因は**見出しを読めない表が前の格子を終わらせていなかった**こと。`extract_remap` は `read_header` が失敗した表を黙って飛ばすので `pending`（直前の格子の列→field 対応）が生き残り、**その表の続き断片が前の格子の field に付く**。V407 の表10-40（FSMC）は列見出しが `FSMCEN=0` で `*_RM=値` の形ではないため読めず、次ページの見出し無しの断片25行が直前の `ADC2_ETRGREG_RM`（ADC のトリガ選択）に付いていた——`A16 PD11` から `FSMC_NADV PB7` まで ADC の経路として出ていた。**表題を持つ表は前の格子を終わらせる**（続きの断片は表題を持たない）ようにし、`is_header_row` でも終わらせる。全corpus実測で**偽の経路82件が消え、増えた経路は0**（V407.en 52・V407.zh 28・FV2x.en 2）。結果: `remap_routes` 4,836 → **4,838**（落ちていた `afio-fsmc-remap,1,FSMC_NADV,PB7` のV407/V467 が戻った）、`pin_functions` の6行が conflict → **confirmed**、`index/pinout` の同じ6行が**偽の `remap-0` から `remap-1` へ**（索引は格子を採る側なので、公開している経路が間違っていた）。conflict 総数 191 → 185 |
| F-61 | **選択ビットが `*_RM` でない remap 格子を読み飛ばしている**。列見出しが `FSMCEN=0`/`DVPEN=1` のような**周辺の有効化ビット**や、`SDIOEN=1&ETHMACEN=1` のような**論理積**で、`COLUMN_HEADER`（`*_RM\d?=値`）に当たらない | **全corpus実測（2026-09-15）で16表。2026-09-16 に測り直して14表・17 signal・110行** | ツール | ✅ **解決**（2026-09-16。案 C）。以下は経過。<br>**(1) 修理した**: `read_header` が「1列でも読めないと表ごと捨てる」のを**読めない列だけ `None` にする**に変えた。CH32V407 の DVP/SDIO は3列中2列が普通の `*_RM` で、`extract_remap` の経路が 374 → 424（zh/en 対称・消失0、版面の表10-42/10-43 と全件照合）。併せて `1DVP_RM` という在りもしない field を直した（`DVPEN=1DVP_RM=0` の `1` を巻き込む正規表現の誤り）。<br>**(2) 読んでも何も増えない**: `CH32V407RM` の FSMC 格子（`FSMCEN=0 默认映射｜FSMCEN=1 重映射`、zh 18行）は**18行とも2列が同じ pad**——「FSMC の pad は動かない」と書いているだけで、経路の情報を持たない。`复用功能｜默认映射` の1列だけの表（6行）も既定の pin 配置そのもの。<br>**(3) 残るのは6行で、これは schema の概念が要る**: `CH32FV2x_V3xRM` の4表で列が実際に違うのは`FSMC_NADV PB7→PD2`・`DVP_D5 PB6→PB3`・`SD0 PC8→PB14`・`SD1 PC9→PB15`・`I2S3_MCK PC7→PA8`・`I2S3_SD PB5→PA9` の6行だけ。**条件が入れ子**（`SDIOEN=1` が `SDIOEN=1&ETHMACEN=1` を含む）なので、`(値→pad)` の平らな対応では同じビット並びに2つの pad が付く——「より限定的な条件が勝つ」という優先順位の概念が要る。`remap_fields` の selector は AFIO の書き込める field を前提にしていて、RCC の有効化ビットを selector にすると consumer が `GPIO_PinRemapConfig` で書こうとする。**新しい概念を足すかは consumer と決めること**。なお `SD0`/`SD1` の2行はpad セルが `PB1`+`4` に割れてもいる（その表の列境界の問題）。<br>**(4) 2026-09-16、ユーザーが案 C（新しい表）を選んだので解決した。** `evidence/pin_conditions.csv`（110行・6 series・confirmed 96／reference 14）と `pipeline/extract/rm/extract_pin_conditions.py` を新設。1行が `(series, signal, condition, pad)` で、`condition` は資料の綴りの連言（`SDIOEN=1&ETHMACEN=1`）。**項が全部成り立つ行が効き、項の多い行が勝つ**——入れ子はこれで解ける。selector には置かないので consumer が `GPIO_PinRemapConfig` で書こうとすることもない。<br>**測り直したら (3) の「6行」は足りなかった**: 対象は2文書14表で、pad が条件で動く signal は `CH32FV2x_V3xRM` に8つ（`FSMC_NADV`・`DVP_D5`・`SDIO_D0`・`SDIO_D1`・`SPI3_NSS`・`SPI3_MOSI`・`I2S3_MCK`・`I2S3_SD`）、`CH32V407RM` に9つ（`FSMC_D4`・`FSMC_NADV`・`DVP_D2`・`DVP_D3`・`DVP_D5`・`DVP_D10`・`SDIO_D0`・`SDIO_D1`・`SDIO_D6`・`SDIO_D7`）あった。(2) の「V407 の FSMC 格子は18行とも2列が同じ pad」も**続き断片を見ていなかった**——`D4` は `PE7`→`PC4` と動く。<br>**列割りが壊れる表はセル文字列ではなく字形の位置で読む**（見出し列の x 範囲 × 行の帯）。`CH32FV2x_V3xRM.zh` の表10-44 は `PB14` がセルの中の在りもしない縦罫で `P`／`PB1`／`4` に割れ、表10-46 の `PA8` は3列目に落ち、表10-42 の見出しは `&RB_UC_RS` で切れていた——字形から読むと全部揃う。<br>**読めない表は1つ残る**: `CH32FV2x_V3xRM.zh` の表10-45（SPI3）は罫線が中央1列しか囲っておらず行ラベルも2列目も表の外。SPI3 の4行は英語版だけの `reference`。<br>**pin 表が裏付ける行だけ出す**（RM は family 単位なので、そのままでは SDIO も DVP も無い CH32V203/V208 に同じことを言う）。裏付けは signal ごとに全部か無か——片方の pad だけ在るときに在るほうだけ出すと選択肢が1つの表になる（`CH32V467` の `FSMC_D4` で実際に出た）。落ちたのは 42 行。<br>検査は `check_tables.pin_conditions_sane`（条件の形・同じビットの重複・`(series, signal, condition)` の一意・**1 signal に条件2つ以上**・pin 表の裏付け）。3通りに壊して落ちることを確認した。<br>**datasheet の注記（`curated/errata.csv` の `v30x-*-default-map` 6件）が同じことを散文で言っていて内容は一致する**が、出所としてはまだ結んでいない。注記を読む経路を足せば裏が2重になり、格子が書いていない**批号の範囲**も機械可読になる（次の仕事） |
...
| F-67 | **`remap_fields.csv` の `basis` が固定文字列で、格子を持たない family でも `rm-remap-grid` を名乗る**。F-66 で経路表は直したが、フィールド表の `FIELD_BASIS`/`MANUAL_BASIS` は `build_remap.py` の定数のままだった | 287行中**22行**（CH32X035 10・CH32X033 10・CH32X315 1・CH32X305 1。2026-09-15の監査） | ツール | ✅ **同日に修理**。候補側で**格子と説明文を分けて数え**（`grid_values_of`／`described_values_of`。混ぜた集合を「RM remap格子」と呼んでいたのが根）、selector ごとの出所を `sources` に載せて `build_remap` がそれで組む。格子を持たない series で格子を名乗る行は **22 → 0**。行は1つも動いていない |
| F-68 | **`remap_routes` の `basis` が、格子が言っているのに言っていないことにする**（F-66 の直しが狭すぎた）。格子は `SD0`・`B[0]`・`PORT9` と書き、datasheet の pin 表は `SDIO_D0`・`LTDC_B0`・`UHSIF_PORT9` と書くので、(signal, pad) が結び付かない | **336行**（2026-09-15の監査）: `SDIO_D0` と `SD0` 32・`LTDC_G4` と `G[4]` 96・UHSIF の PORT22以降 26・`ETH_MII_RXD0` と `ETH_RXD0` 38・pad の綴り（`PC14-OSC32_IN` と `PC14`）86・`T2CH1ETR` と `TIM2_CH1`＋`TIM2_ETR` 58 | ツール | ✅ **解決**（2026-09-15 に大半、2026-09-16 に残り。ADC は F-73 待ち）。**あいまい照合ではなく語彙**で直した——末尾一致でも `canonical_signal` でも揃わないので、`extract_remap.qualify_grid_signal` に **field ごとの周辺名**を書いた（`DVP_RM`→`DVP`、`SDMMC_RM`→`SDMMC`、`UHSIF_PORT_RM`→**`UHSIF`**、`LTDC_RM`→`LTDC`＋括弧を落とす、`SDIO_RM`→`SDIO`＋`SDn`を`Dn`に）。5件とも pin 表の綴りと突き合わせて確かめた（`SDMMC_SDCK` は pin 表もそう綴る）。裸の行ラベルは **0件**になり、`remap_routes` の **128行**が格子を名乗るようになった（行は1つも動いていない）。<br>**残るのは3種**——`ETH_RXD0` と `ETH_MII_RXD0`/`ETH_RMII_RXD0`（MII と RMII のどちらを指すのかを原文で決める必要がある）・pad の綴り（`PC14-OSC32_IN` は封装名つきで、比較の正規化を pad 側に足す話）・`T2CH1ETR`（1つの pin 機能が格子の2行に対応する）。いずれも「同じものか」を原文で裁定してからでないと入れられない。<br>**2026-09-16、残りを裁定して 193 行を直した**（`remap_routes` の行は1つも動かず、`basis` だけが増えた。失った出所は0。`rm-remap-grid` 193・`rm-field-description` 169）。<br>**pad の綴り 82行**: pin 表は pad に別の機能を継ぎ足して刷り（`PA0-WKUP`・`PC13-TAMPER-RTC`・`PC14-OSC32_IN`・`PC15-OSC32_OUT`）、格子は港とピン番号だけを書く。全corpus実測: `-` を含む pad は6綴りで5つがこの形、残る `VREF-` は `-` が名前の一部（港+番号で始まらないので当たらない）。`signal_vocabulary.port_pin`。<br>**合成名 67行**: `TIM2_CH1_ETR`／`T2CH1ETR` は格子の `TIM2_CH1` と `TIM2_ETR` の2行に当たる。原文の裏: `CH32V003RM` 表7-9 は両者を**4列とも同じ pad** に置き（PD4/PC5/PC1/PC1）、pin 表は同じ pad に `T2CH1ETR` を1つ置く。`signal_vocabulary.combined_parts`。<br>**`LPT_INn` ↔ `LPTIM_CHn` 6行**: `CH32L103RM` 表10-15 は `LPT_IN1`=PB12(値0)/PB5(値1)・`LPT_IN2`=PB13/PB7、pin 表は `LPTIM_CH1`=PB12(default)/PB5(remap-1)・`LPTIM_CH2`=PB13/PB7 で**完全に一致**。同じ表の `LPT_ETR`/`LPT_OUT` は pin 表も同じ綴りなので寄せる必要が無い。`signal_vocabulary.ROLE_ALIAS`。<br>**ETH の接口名 38行**: 格子は `ETH_RXD0`、pin 表は `ETH_MII_RXD0`／`ETH_RMII_RXD0`。`CH32FV2x_V3xRM` 表10-41 の `ETH_RXD0` は `ETH_RM=0`→PC4・`=1`→PD9 で、pin 表はその PC4 に MII と RMII の default を、PD9 に両方の remap-1 を置く——**一緒に動く**。**RGMII は外した**（同じ PC4 に `ETH_RGMII_TXD1` が居るが `remap-1` の行を持たず、`ETH_RM` で動く裏が資料に無い）。`signal_vocabulary.interface_stripped`。<br>**残るのは ADC のトリガ 28行だけで、それは F-73（資料の食い違い）待ち**。当たらないまま残る 25 行は**当たらないのが正しい**——ETH の RGMII 15行（役割名が違う）と `SPI3_MOSI`↔格子の `SPI3_NSS` 3行（別の signal）、`ADC_IN4`/`A4` 7行（入力チャネルでトリガではない） |
| F-69 | **RM の「レジスタ説明文」を `basis` が名乗れない**。`build_candidate` は説明文由来の経路を `_source == "description"` と印を付けているのに、`route_sources` は格子しか見なかった | **362行**（CH32X035 166・CH32X033 71・CH32V407 60・CH32V467 59・L103/M103 6。2026-09-15の監査） | ツール | ✅ **同日に修理**。`rm-field-description` を語彙に足し、経路と field の両方で名乗る。`remap_routes` の3,429行が説明文を根拠に挙げるようになり、**CH32X035 の166行**は格子ではなく `R32_AFIO_PCFR1` の説明文を指すようになった。行は1つも動いていない |
| F-70 | **`CH32L103`/`CH32M103` の CAN 重映射の格子が `USART1_RM` として取り込まれている**。表10-22 の列見出しが値だけ（`00｜10｜11`）なので `read_bare_header` が行ラベル `TX`/`RX` から周辺名を引き、`signal_vocabulary.split('TX')` が既定で `USART1` を返す | 偽の `USART1_RM` grid 経路6件（2026-09-15の監査） | ツール | ✅ **同日に修理**。**表題が名乗るならそれを周辺名にする**（`caption_peripheral`）。全corpus実測: 値だけの見出しの格子は22表で、表題と行ラベルが食い違うのは**この1表だけ**。`canonical_field` が `CAN1_RM` と `CAN_RM` を同じに畳むので selector は変わらない。併せて中文版の格子経路が 195 → 211 に増えた（表題から周辺が決まり、行ラベルでは決まらなかった表が読めた） |
| F-71 | **ページ境界で割れた rowspan セルの続きが、既に出した行へ戻らない**。`CH32X035DS0.en` V2.3 で版面のページ割りが変わって2か所で出た——(1) 表3-5 の `T_S_vrefint` の項目名が p.26 で切れ、続きの `reading the internal reference voltage` だけを載せた断片が p.27 にある。`operating_rows.read_edition` は続きを `param` に繋ぐが、**p.26 の行はもう出した後**なので `ADC sampling time when` のまま出る。(2) 表3-7 （Sleep の消費電流）が p.27/28 をまたぎ、条件の rowspan セル `Runs on the high-speed internal RC oscillator (HSI). Uses HB prescaler to reduce the frequency.` が割れた。`extract_low_power.fold_page_continuations` は繋ごうとするが `_continues` が「上が `.` で終わっていれば別物」で止まる（同一ページ内で新しい条件行を誤って繋がないための歯止め） | operating_conditions 43行（条件30・項目名12・重複解消1。実測は X035 だけでなく V20x_30x・V103・V407・H417・L103・M007/V007・V002・V004・V203・V205・V208 にも及んだ） | ツール | ✅ **同日に修理**。`_continues` は綴りの見分けなので、**続きなのに続きに見えない**形は取れない（上が `.` で終わり下が大文字で始まる）。ページ境界に限って綴りに拠らない2つの根拠を足した（`fold_page_continuations`）——(1) **値を1つも持たない行は行ではない**ので、非空セルは上のセルの尻尾しかありえない（`Supply current in`＋`STOP mode 2(2)`、`只开启`＋`LSI`）、(2) **まだ閉じていない rowspan のセルはページをまたぐ**——項目名の列が続きだと分かった行では、上のセルが2行以上に跨っている条件列も続き（`F = 8MHz HCLK` のような行ごとの条件は跨っていないので当たらない）。併せて `operating_rows` が、断片の1行目で項目名を繋いだときに**前のページで既に出した行へ書き戻す**ようにした。全corpus実測: `_continues` の拒否は62回あって**間違っていたのは4件だけ**（残りは `F = 48MHz HCLK`／`F = 24MHz HCLK` のような本物の別行）。`CH32V003` の40行は `Uses **A**HB prescaler` と綴りが違うだけで欠けていない。直った43行は**全部が欠けの補完**で、値の取り違えは無い |
| F-72 | **`curated/errata.csv` の `v30x-*-default-map` 6件が `CH32V317` を名指していない**。datasheet の表題は `CH32V303/305/307/317数据手册` で、注記はseries を限定せず批号だけで区切っている。`CH32V317` の pin 表は `SDIO_D0`（PC8/PB14）と `DVP_D5`（PB6/PB3）の両方の pad を載せているので、少なくともこの2件は V317 にも効くはず。ただし同じ文書由来の `v30x-vio-min-1v2` は V317 を**含んで**いるので、除外が意図的かもしれない | — | 資料 | ✅ **変更不要**（2026-09-16 に原文で裁定）。**除外は正しい**——datasheet は V317 に**別の pin 表と別の注記**を与えていて、そちらには批号の句が無い。表3-1（`CH32V303/305/307引脚定义`。zh p.24-34／en 〜p.41）の注7〜12 は批号の条件つき、**表3-2（`CH32V317引脚定义`。zh p.35-43／en p.42-50）の注8・注9 は条件なし**（`当寄存器RCC_HBPCENR的bit[14]ETHMACEN=1与bit[10]SDIOEN=1时、SDIO_D0和SDIO_D1默认映射自动改到PB14和PB15。` だけ。zh/en とも）。表3-4（zh p.45-49／en 〜p.55）の注が批号つきなのは、その表が前置きで**「下表中的引脚功能描述针对的是所有功能，不涉及具体型号产品」**と断っている型番非依存の表だから。型番ごとの表が V317 について条件なしと書いている以上、`v30x-*-default-map` の `condition` を V317 に付けると**資料が課していない制限を足す**ことになる。<br>V317 の注記が SDIO_D0/D1 と DVP_D5 の2件だけなのも表3-2 と整合する——FSMC_NADV は表3-2 に無く、I2S3_SD/I2S3_MCK/SPI3_MOSI は変更先の pad（PA9/PA8/PA15）を表3-2 が載せていない。<br>`pin_conditions` の V317 3行が `errata` 空なのは「ロットの条件は記録されていない」という意味で、これが正しい読み。`CH32V407`/`CH32V467` の4行ずつも同じ——`CH32V407DS0` の注記には批号の条件がそもそも無い |
| F-73 | **`CH32M030` の `ADC_ETR` の既定 pad を RM と datasheet が逆に書く**。RM 表6-15（`CH32M030RM.zh` p.55／`.en` p.62）は `ADC_ETRGIN_RM=0 默认映射`→`ADC外部触发转换与PB6相连`、`=1 重映射`→`…与PA14相连`。datasheet の pin 表（`CH32M030DS0.zh` p.20／`.en` p.25-26）は PA14 に `ADC_ETR`（接尾辞なし＝既定）、PB6 に `ADC_ETR_1`（`_1`＝AFIO の値1）を書く。**zh/en とも各資料の中では一致**しているので版の食い違いではなく**資料どうしの食い違い**。`ADC_ETRGIN_RM` は1ビットなので「どちらかが既定」しか有り得ず、両立しない | 1 signal・2経路。併せて ADC のトリガ格子 28 行が `basis` に `rm-remap-grid` を名乗れない（F-68 の残り） | 資料 | ✅ **解決**（2026-09-16。ユーザーが案 B を選んだ）。**`remap_routes` に `conflict` を導入した**——他の証拠表と同じ作法で、片方に寄せず両論を残す。値は pin 表を保ち、格子の言い分を `basis` に `!rm-remap-grid(value=0)` と書く（`build_remap.route_basis`）。`build_conflicts.ASSERTS` に `remap_routes: value` を登録したので `index/conflicts.csv` が `alternative` に格子の値を持つ（207 → 208 行）。<br>併せて文章の行ラベルを繋いだ（F-68 の残り）——**どちらのトリガかは field が言っている**ので文を読まずに決める（`extract_remap.GRID_PROSE_SIGNAL`。`ADC1_ETRGREG`→`ADC1_RETR`・`ADC1_ETRGINJ`→`ADC1_IETR`・`ADC1_ETRGIN`→`ADC1_ETR`）。27 行が格子を名乗り、1 行が `conflict` になった。同じ pad に居る `ADC_IN4` は入力チャネルなので当たらない。<br>**訂正の歯止めを2つ足した**（`build_candidate`）: 格子の値を採る訂正は、(1) pin 表が同じ (signal, 値) を別の pad に与えているとき、(2) **格子が pin 表の言う値を別の pad に置いているとき**——どちらも「pad ごとの誤り」ではなく**対応そのものの食い違い**の印——には効かない。(2) は封装に依らないのが要点で、`CH32M030G8R7` は PA14 を封装していないので (1) では見えない。実測: 訂正は 13 → 12 になり、残った 12 は全部 `CH32V103` の TIM3（pin 表が `remap-1` と書くのに格子が値1を使わない＝0/2/3 だけ、という従来からの正しい訂正）。異論は 5 SKU 全部に付いた。<br>検査 `check_tables.remap_conflicts`: `conflict` は異を唱えた出所と値を `basis` に書いていること・印と確度が両方向で揃うこと・**異論の値が採った値と違うこと**。3通りに壊して落ちることを確認した |
| F-74 | **`condition`・`parameter` の添字が基底から離れたまま残る**。版面は記号の右下に小さく添字を刷るので、セルの文字列にすると読み順が「基底 → … → 添字」になる（`F = 16MHz HCLK`・`Input V /2, DDA C = 50pF`。式として読めない）| operating_conditions 1,461 行（2026-09-16 の実測。**離れたまま残る行 0・下線なしの綴り 0** になった）| ツール | ✅ **解決**（2026-09-16）。3段。(1) **綴りで戻せる形**を戻す（値の後ろ・`=` の手前・その間に脚注が挟まる形・`=` と値の間）。(2) **記述の欄を converter の綴りから読む**——`text_split`（版面が割ったまま）ではなく `text`（converter の下付き結合）。`extract_low_power` は `join_fragments` が格子を2つ返して記述の列だけ入れ替え、`operating_rows` は `fill_rowspans` の中で同じことをする。**役割が決まった列だけ**に限るのが要点——限定しないと `I⏎DD` が `IDD` になって `KEEP` に落ち **128 行が消えた**（一度踏んだ）。記号・値・単位は `text_split` のまま。(3) **綴りを揃える**——converter の結合は下線を持たない（`VDDA`）が、この表は下線形が正（実測 1,759 対 67）。<br>**歯止めはどの段も同じ**: `<記号>_<添字>` がこの表のどこかで実際に使われている綴りのときだけ動かす。行数 3,940 → 3,939 で、減った1行は**同じ事実が2通りに壊れていた重複の解消**。値・記号・単位・確度は不変。<br>**`F_V5F` 84箇所・`F_V3F` 76箇所（計100行）は動かさない**——CH32H417 の2つのコアの名前で、綴りがこの壊れた形にしか出てこない |
| F-75 | **`remap_fields`/`remap_routes` の `basis` が版の印を `:en` で決め打ちしていた**。`build_candidate` は application manual の zh と en を**両方読んでプールする**（全11 RM が両版ある）のに、末尾の印は常に `en` だった——中文版しか言っていない経路も `en` を名乗る | 実測: 説明文由来の経路は zh のみ 503・en のみ 465、格子由来は en のみ 29（2026-09-16）| ツール | ✅ **同日に修理**。索引を `(signal, pad) → {値: 版の集合}` に替え、`route_sources` が `editions=True` で「出所 → その出所が言っている版」を返せるようにした。`basis` は述べた版を書き、manual が述べていない行には印を付けない（`candidates(datasheet-pin-table-default)`）。`remap_fields` からも同じ理由で `:en` が消えた |
| F-76 | **`CH32V006K8U6`（-40..85℃ の QFN32）を datasheet が名指していない**。実機・WCH の EVT・probe-rs の3つが名前を知っているのに、`catalog/products.csv` の出所である**比較表と订购信息だけが書いていない** | 1 型番。**全corpus（34文書・両版）で `CH32V006K8U` の綴りは `K8U7` だけ**（`CH32V006DS0` の zh/en に5回ずつ）で `K8U6` は 0 件——抽出の取りこぼしではない | 資料 | ✅ **解決**（2026-09-16。ユーザーが「実機があるので追加して」と裁定）。**資料側の穴は残るが、目録は出荷されている製品に追いついた。**<br>**出所は4つあった。** (1) **WCH の EVT の project**——`wui_demo.wvproj` と `.template` の `MCU=CH32V006K8U6`。EVT 全体で `CH32V00[567]` の完全な型番はこれ1つで、`K8U7` は EVT に出てこない。(2) **EVT の device header のコメント**（`evt_variants` が拾っていた）。(3) **実機**——`CH32V006K8U6-EVT-R0` が chip_id `0x00600620`（[7:4] を伏せて `0x00600600`）・flash 63488・uid `f799abcd5fc7bc61`。(4) **probe-rs 0.32.0** のターゲット一覧。回路図 `EVT/PUB/SCHPCB/CH32V006K8U-R0` は温度接尾辞を落とす綴りなので両グレードを指す。<br>**入れ方**: `build_tables.evt_only_rows` が EVT の project が名乗る**完全な**型番を拾い、**末尾の温度グレードの桁だけが違う兄弟が正本に在るときだけ**行を作る。他の列はその兄弟から来る（`rule:pn-temp-grade-sibling`）。**実測で裏づけた規則**——末尾1桁だけ違う組は6つあり、**両方の桁がグレード（6/7）の5組は他の列が完全に一致**する。残る `CH32M030C8U3`/`C8U7` は `3` がグレードではなく封装の変種で封装が実際に違うので、両方の桁がグレードであることを条件にした。歯止めが効いていることは同じ走行で確かめられる——`CH32V307RVT6` は10以上の `.template` が名乗るが資料のどこにも無く兄弟も無いので、報告して捨てている。<br>**行**: `CH32V006K8U6,CH32V006,CH32V006,QFN32,63488,8192,31,-40..85C`。確度は`part_number` confirmed（`evt:project`＋`silicon:wch-linke`）・`package` confirmed（兄弟＋`rule:pn-letter` の検査）・`flash_bytes` confirmed（兄弟＋実測）・`series`/`sram_bytes`/`gpio_count`/`temperature` reference。実測は `curated/parts-measured.json`（R-27 の `debug-data-measured.json` と同じ流儀）。**この file だけでは行を作らない**——WCH の資料が名乗っていることが先。<br>**pin も付いた**（`extract_pin_tables.resolve` に段を1つ足した）。引脚定义の表は`CH32V006引脚定义（除CH32V006F4U6以外）` と **series 全体**を表題にしていて、QFN32 の列がたまたま 105℃ グレードの綴りで見出しを持っているだけ（グレードを落とした `CH32V006E8R` の列は既に両グレードが読んでいる）。**見出しの型番の封装がこの型番と同じ列が丁度1つで、表題が除外していない**ときだけ採る。pins +33・pin_functions +291 で、**どちらも `CH32V006K8U7` と1バイト違わず、既存の行は1つも動いていない**。<br>**ピン配置図は主張しない**——図の見出しは `CH32V006K8U7` としか刷られていないので `figures.csv` に行は無く、`figures_sane` の被覆はデータシートの表が載せる型番だけを見るようにした。<br>**動いた数**: products 103→104・pins 4,563→4,596・pin_functions 28,448→28,739・`index/pinout` 24,947→25,207・`index/parts` 103→104・`part_number_count` 26→27・series CH32V006 7→8・packages QFN32 8→9。**ほかに2つ自動で埋まった**——`device_ids` はch32-data の伏字 yaml（`CH32V006x8x6`）が展開して 71→72 になり、その値 **`0x00600600` は実機の実測と一致**した（第三者DBと実機が独立に一致）。`eval_boards` の `CH32V006K8U-R0` は両グレードを指すようになり、`evt_variants` は 56→57 |
...
| `index/pinout.csv` の `remap-N` 行で `selector` が空 | 104型番へ展開したあとの**行数** | **0** | F-51（CH32H417 の `UHSIF_PORT0`〜`2`）が 2026-09-23 に資料側で解けて空になった |
...
| G13 | **折り返しただけの見出しを「群」と読んで、比較表の行名が切れる** | `CH32X035.md` の Type-C の行が `DRP`（英語版 V2.2）→ `Sink DRP`（V2.3）。資料の見出しは `Type-C` / `Source` / `Sink DRP` と3段に**折り返しているだけ**で群は無いのに、`extract_products.read_row_layout` が「最後の段が見出し・その上は群」と決めるので `group = "Type-C Source"` になり、`build_readme.leaf_of` が `NAMES_ITSELF`（`DRP` が当たる）で剥がしてしまう。**版面の折り返し位置が変わると表示名が変わる**のが印で、2026-09-16 の `CH32X035DS0.en` V2.3 受入で動いた。証拠（`product_attributes.label`）は`Type-C Source Sink DRP` と正しく、崩れるのは表示だけ | ✅ **同日に修理**。**根で直した**——型番が行の表では属性は**列**で、見出しの段は1つのセルの折り返しでしかないので、`read_row_layout` は群を名乗らないことにした。全corpus実測: この並びで出る「群」は50種あって**全部が折り返しの上半分**（`看 门`／`看 门 狗`、`Code`／`Code FLASH`、`Serial`／`Serial port`）で、本物は1つも無い。`Communication interface CAN` のような本物の群は `read_column_layout` 側で、あちらの段は**別々のラベル列**なので無傷（剥がれる見出し77種はそのまま）。証拠は `group` 列が149行で空になっただけで**他の列は1文字も動かない**。表示は `Sink DRP` → `Type-C Source Sink DRP` |
...
| 7 | C1〜C3 画像 | **C1・C2 は 2026-09-16 に閉じた**（C1 は実測で切れ0・名前の食い違い0、C2 は `evidence/figures.csv` でページ直リンク）。残るのは C3——採否の判断 |
...
| F-51 | CH32H417 datasheet zh | pin表2-1-1 の PF12/PF13/PE7 の重映射欄に `UHSIF_PORT0_1`／`_1`／`_2`。**英語版の同じ欄は空**で、RM の格子はその3 pad を `UHSIF_PORT_REMAP=0`（既定）に当て、値1は PC1→`PORT3`・PC2→`PORT4`・PC3→`PORT5` とずらして書く | `pin_functions` に `reference`／`pin-table:zh` として残す（既定側は両版一致で `confirmed`）。格子に裏づけが無いので `pinout` の `selector` は空。`KNOWN_SELECTOR_GAPS` に3行として記録<br>**2026-09-23 に資料側が直った**——英語版 RM の 1.7→1.8 で格子が値1を PF12/PF13/PE7 に当てるようになり、中文版の pin 表の主張が裏づけられた（F-51 解決）。記録として残す |
...
| F-59 | CH32V407 RM **en 版** | DMA の周辺映射表の題が `Figure 11-2 DMA1 peripheral mapping table for each channel`・`Figure 11-3 DMA2 …`（zh 版は `表11-2`/`表11-3`。図の `Figure 11-2 DMA2 request mapping` と番号も重複） | `build_dma_requests` が `Figure` も表の題として受ける（「mapping table」を含むものだけ）。値は zh/en 一致で confirmed |
