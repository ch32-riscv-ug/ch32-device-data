...
| F-9 | USBが48MHzを要求する根拠が散文 | 文は特定済み | ツール | 🔜 表への入れ方だけ未決。下記 |
...
| F-14 | `flash_bytes`が零等待領域ではなく総容量を指すfamilyがある | 11 part | ツール | ✅ **V30xは修理**（2026-08-22）。X305/X315は散文なので残る。下記 |
| F-15 | 比較表の**行グループ**（左セルが複数行にまたがる）が1行に潰れる | H41x 5 part | ツール | 🔜 `sram_bytes`が896KBのうち128KBだけになる。下記 |
...
**残る穴**: CH32X305/X315とCH32H41xは列が1つしかなく、分割は脚注の散文にある。

```
注：1.480KB闪存包含192KB的零等待程序运行区域和288KB非零等待区域。
```

X315のEVTの既定`Link.ld`は`FLASH 192K`で、`products.csv`は480Kのまま。
**CH32X305/X315は`flash_bytes`が過大**。脚注を読む抽出が要る（F-3と同じ、散文からの
抽出）。
...
直すには`extract_products`が「左セルが空の行は直前のグループの続き」と読み、
`SRAM.内核1高速ITCM`のような複合キーで出す必要がある。`sram_bytes`が
どれを指すべきか（896KB全体か、零等待データ領域の256KBか）は
**consumerが何に使うかで変わる**ので、決める前に用途を確認する。
...
1案目を採る前提で、残っているのは**12 family × 2言語のRM/datasheetから
この文を機械的に拾う抽出**。`build_operating.py`の隣に置ける。
`F_USBCLK`が48MHzでない family があるかどうか（USBHSを持つH417・X315・V307など）は
まだ確認していない。
