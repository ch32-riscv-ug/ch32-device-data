...
**CH32V103C6T6 の 6 番**は別口です。この datasheet は pin 表を2つ持ち
（x8 品の3封装用と x6 品の2封装用）、x6 側は同じ端子を `OSC8M_IN`/`OSC8M_OUT` と
綴ります。`choose_table` が採るのは1つだけなので、**x6 専用の表を読んでいません**。
`OSC8M_IN` が lead 5 に入っているのは別経路（両言語の突き合わせ）からで、
`OSC8M_OUT` は9文字あり `PAD_TOKEN` の8文字にも掛かります。**表を1つしか選べない
のが本体**で、`PAD_TOKEN` を広げるだけでは直りません（実測で確認：広げても
`OSC8M_OUT` は増えず、CH32V205 の壊れた名前 `VVDD_IO_1` が増えるだけ）。
...
**代わりに、判断ではなく事実として取れる出所があります。** WCH自身が
どのpadを使っているか——`EVT/EXAM/`の例題コードと`EVT/PUB/SCHPCB/`の評価ボード
回路図です。「WCHの例題がSPI1をPA5/PA6/PA7で書いている」は判断ではなく事実で、
`basis`に例題のパスを置けます。`evt_examples.csv`と`eval_boards.csv`が既に
その在り処を持っています。

これなら`pin_roles.csv`の約束を壊さずに答えられます（`preferred`ではなく
`used_by_evt`のような、何を数えたかが名前で分かる列）。**次に着手します。**
