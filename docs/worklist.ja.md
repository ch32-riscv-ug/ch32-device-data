...
| F-76 | **`CH32V006K8U6`（-40..85℃ の QFN32）を datasheet が名指していない**。実機・WCH の EVT・probe-rs の3つが名前を知っているのに、`catalog/products.csv` の出所である**比較表と订购信息だけが書いていない** | 1 型番。**全corpus（34文書・両版）で `CH32V006K8U` の綴りは `K8U7` だけ**（`CH32V006DS0` の zh/en に5回ずつ）で `K8U6` は 0 件——抽出の取りこぼしではない。`evidence/device_ids.csv`（ch32-data 由来・`reference`）も `0x00600600` を持たない（V006 は `0x00610600`＝`E8R6` のみ）| 資料 | **記録のみ（2026-09-16 時点）。人の裁定待ち。**<br>**出所は3つある。** (1) **WCH の EVT**——`CH32V006EVT.ZIP` の `EVT/EXAM/APPLICATION/WUI/wui_demo.wvproj` と `.template` が `MCU=CH32V006K8U6` と宣言する。EVT 全体で `CH32V00[567]` の型番トークンは**この2箇所だけ**で、`K8U7` は EVT に出てこない。回路図も `EVT/PUB/SCHPCB/CH32V006K8U-R0`（11ディレクトリとも温度接尾辞を落とす綴りなのでグレードは決まらないが、QFN32 の評価ボードが在ることは確か）。(2) **実機**——ArduinoCore-CH32 の作業台の `CH32V006K8U6-EVT-R0` が chip_id `0x00600620`（rev nibble [7:4] を伏せて `0x00600600`）・flash 63488・uid `f799abcd5fc7bc61` を返す。(3) **probe-rs 0.32.0** の CH32V00X ターゲット一覧。ずれは系統的で、**62K/8K の品だけが「上流に6グレード・DS0に7グレード」**（V006 の `K8U6`/`F8U6`/`F8P6`、M007 の `E8U6`）。V002・V004・V005 は両者一致。**非対称が決め手**——`CH32V007K8U6` は documented でproducts.csv に在る。WCH は V007 の 85℃ QFN32 を書いて V006 のそれを書いていない。<br>**裁定が要るのは2つ。** (a) `catalog/products.csv` は `tools/build_tables.py` が**比較表＋订购信息から生成する**表で、手で足した行は次の再生成で消える——EVT を読む経路を生成器に足すかどうか。(b) 足すなら `*_basis` の語彙に「EVT が名乗っている」を表す token が要る（いまは `products:zh|en`・`ordering:zh|en`・`pin-table`・`rule:*` だけ）。温度だけは既存の `rule:pn-temp-grade` で決まる。<br>**動く数**: `part_number_count` 26→27・`index/parts.csv`・生成 README・凍結台帳。2026-09-02 に `CH32V006DS2.zh`（＝CH32M006 の datasheet）で下した「比較表に無い SKU はcatalog に置き場が無い」と**同じ形の問いだが、あちらには WCH の別資料が無かった**——今回は EVT が在るので、同じ答えになるとは限らない。<br>**consumer の当座**: `CH32V006K8U7` の行で足りる（QFN32・63488・8192・GPIO 31 と**温度以外の全列が同じ**）|
...
| F-76 | CH32V006 datasheet | `CH32V006K8U6`（-40..85℃ の QFN32）を名指していない。**WCH 自身の EVT**（`wui_demo.wvproj` の `MCU=CH32V006K8U6`）・実機・probe-rs 0.32.0 は知っている。62K/8K の品だけ「上流に6グレード・DS0に7グレード」という系統的なずれで、`CH32V007K8U6` は documented | `products.csv` は比較表＋订购信息から生成するので、EVT を出所として足すかは**人の裁定待ち**（F-76）。WCHへ報告する材料 |
