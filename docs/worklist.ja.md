...
| F-10 | CH32V205・CH32X315のRMから経路が0件 | V203CCT6のUSART5-8 | ツール | 🔜 原因未調査 |
...
### F-10 CH32V205とCH32X315のRMから経路が1件も取れない

`extract_remap`の格子も`extract_registers.routes_in`の説明文も、この2 familyでは
**0件**を返す（他familyは7〜23種）。影響が見えるのは**CH32V203CCT6**で、この型番だけ
`USART5`〜`USART8`のピンを持つのに、**CH32V205 familyから作られる**ためremap
selectorが1つも付かない（`route_selectors`が空）。F-2でRM由来selectorを認めるように
したが、経路が取れない以上この2 familyでは効かない。

原因未調査。RMの表の作りが他と違うのか、キャプションの拾い方かのどちらか。
