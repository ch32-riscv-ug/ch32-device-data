...
- [ ] **bit図のセルで同じ名前が2回混ざる（16件・4文書）** — 旧「270件の straddling-word split」を
  ...
  **候補が2つ立って決まらなかった**だけだった（記述表の `SPI1RST` と、合成した `SPI1RST1`）。
  ...
  正本CSVは無変化、parity 68/68 clean。**残る5件**は `CCRCFAIL`（3文書）と `CMDSENTC`（2文書）で、
  そのページの記述表に名前が載っていない——名前の出所を同じレジスタ章まで広げれば解ける見込みだが、
  広げると偽の候補も増えるので**範囲を測ってから**。
