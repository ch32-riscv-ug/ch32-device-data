...
| R-31 | flash消去後の読み出し値（wch-protocols・優先度中） | ✅ **実装**（2026-09-06）。`flash_geometry.csv`に5列——RM原文のまま4列（`erased_read_word`/`half`/`byte_even`/`byte_odd`）＋EVT IAPの判定値`blank_check_word`。系統B（V20x/V307/V407/X315/H417）は4値とも明記、系統A（L103/M030/V006/V205/X035）はRMが`字读- 0xFF`とwordしか書かないので**残り3列は空**。zhを一次・M030だけen版（`basis`に`rm-en`）。V003/V103はzh/enとも記述が無く**空欄＋note**（他repoの実測を引用）。EVT側12 familyの判定値は自分で照合（H417は`Common/hardware.c:70`） |
| R-32 | main flashの消去/書き込み**手順**の分類（ch32rv 0004・優先度中） | 📋 **受領・未着手**（2026-09-06）。指摘された齟齬2件を先に確認済み——①「V307に`PG_STRT`(bit21)が無い」は**事実でない**（`register_fields.csv`にV20xと同じbit21が在る）、②V103の`BUF_LOAD`/`BUF_RST`は確かに在るが実機では標準half-wordでしか書けない（bitの有無と手順は一致しない、が依頼の主旨）。`register_fields`のFLASH_CTLR bitだけで2系統に割れ（PgStart: H417/V20x/V307/V407/X315、Buffered: L103/M030/V003/V006/V103/V205/X035）ch32rvの実機5 familyと一致するが、依頼の本体は**RMが名指す手順の順序**なので闪存章の編程手順の抽出が要る |
