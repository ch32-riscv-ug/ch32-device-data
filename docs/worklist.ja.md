...
- [ ] **続きの断片で単位が落ちる** — 2026-09-10に絶対最大定格の受入で見つけた。表の途中で
  ...
  **再突入の条件**: 「値を1つも持たない行は出さない」を先に決めること。**当初はこの4行
  （`t_erase_32k`・`t_erase_sec`・`V_DDA`×2）を「事実を持たない雑音」と見ていたが、原本を
  描画したら縦結合の取りこぼしだった**——上の項目で埋めて値ゼロの行は0になった。つまりこの
  規則は今や「増えた行が値を持つか」の**検査**として使える。そのうえで単位の持ち越しを入れ、
  増える行を1件ずつ原本と照合する。絶対最大定格の表（`extract_absolute_maximum.py`）は**新しい表なので
  最初から持ち越している**——そちらは250行が全行 confirmed で裏が取れている。
