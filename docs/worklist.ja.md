...
| consumerからの依頼 | 10 | 2（**R-28〜R-30を2026-09-01にch32rvから受領**。R-29は完全解決（debug_wiring＋全27 series確定）、R-30はRMから取れるぶん完結（option 2表＋WRPR粒度。残りは実測照合）、R-28は表2枚新設済みで**gap 7 seriesの値の実機実測待ち**（読み出し番地は全family提供済み——ch32rvへ測定依頼を出すこと）。R-27 は H417 の実測待ちが1行。R-20は機械収集ぶんまで、残りはconsumerの要否次第） |
| 表示（G系） | 12 | 0 |
...
| R-28 | chip ID（device_id）のevidence表新設（ch32rv 0001・優先度高） | 🔶 **表2枚を新設**（2026-09-02、`tools/build_device_ids.py`）: [`evidence/device_id_addresses.csv`](../evidence/device_id_addresses.csv)（12行——**全familyの読み出し番地**をEVTの`DBGMCU_GetCHIPID()`から。第三者DBに無いgap familyも埋まった: V205/V407/X315=0x1ffff704、**M030=0x1ffff384**。memory_mapのCHIPID行と相互検査）＋[`evidence/device_ids.csv`](../evidence/device_ids.csv)（71行——ch32-data取込・**全行reference**。第三者DBは一次資料ではないので、confirmedは実機読み`device-id:wch-linke`との突き合わせのみ）。`id_source`（memory/attach）・`dont_care_bits [7:4]`・V103のSTM32互換note込み＝依頼書の列仕様どおり。**残り＝gap 7 seriesの値**（V205/V407/V467/X305/X315/M030/M103——読み出し番地はこちらが提供済みなので、ch32rv側のboard群でwlink status等の実測が可能。測定依頼を出すこと）と、実測が届いた行のconfirmed化・memory/attach同値性の記録。なおch32-dataには目録と**末尾グレード桁だけ違う**型番が複数ある（CH32V006F8P6 vs F8P7等——対応付けせず落として見えるまま） |
...
| 5 | **D18** PDF構造化の本実装 | 🔧 **進行中**（2026-09-01）。ingest・L1結合・人向けMarkdown（差ゼロ検査67文書合格・図caption 100%）・一括再生成entry point済み。**凍結PDF tool 19本すべて新経路で従来以上**、operating_conditions・debug_wiringは切替済み。残り: L2 sidecar・CSV群の一括切替・zh単独版・R-30の抽出 |
...
| **出所・矛盾の縦持ち索引**（`claim_id, subject, predicate, value, source, ...`） | 🔧 **安い部分を実装**（2026-08-29）→ `index/conflicts.csv` | `basis` が1セル内の DSL なので横断で引けない、というのは事実。まず `confidence=conflict` の行だけを集める索引を作った（**証拠の表の conflict は 214 行**。memory_configs 67・register_fields 38・operating_conditions **40**（A11受入とX315 zh改版で2026-09-01に30→40）・product_attributes 25・pin_functions 12・option_byte_fields 8・option_bytes 6（R-30の新表。2026-09-02）・clock_symbols 5・opa_cmp_registers 5・registers 4・adc_internal 2・flash_geometry 1・timers 1。この数は `check_docs.py` と `check_tables.py` が数え直す）。`basis` から `!<出所>` と `(=<値>)`（新経路の `(address=…)`・`(field=…)` 形も）を取り出すので、**139行は「表が採った値」と「相手が言う値」が横に並ぶ**（例: CH32V407 `RCC_CFGR2.UTMI1ON` は EVT が bit31・RM が bit30）。残る75行は、食い違いを散文で記録している表（memory_configs・timers の68行）と、**相手が「値を書かない」型**（新しいX315 zh版がFlash時間のmaxを載せない等）。**分かったこと**: `product_attributes` の25行は仕様の差と**言い回しの差**（`Typical: 72MHz` / `Typ. 72MHz`）が混ざっていて、conflict の印が「本当の食い違い」と同義ではない表がある。claim 表まで広げるかは、この索引が使われるかを見てから |
...
| R-30 | CH32M030 RM **en版** | option bytes章の「User option bytes information structure」表の番地が**0x1FFFF800**（他familyからのコピペ疑い）。zh版は0x1FFFF300で、EVTヘッダ`ch32m030.h`のOB baseも0x1ffff300 | `option_bytes`はEVT（第三の証拠）が支持するzhを採り、enの異議を`!CH32M030RM.PDF:en(address=…)`でbasisへ（6行conflict）。同章のWRPR群名もenは`WRPR0 - WRPR3`とコピペ（M030のWRPRは2バイト。zhは`WRPR0–WRPR1`）——両論をreference 2行で保持 |
| R-30 | RM option bytes章の版間齟齬（識別子・復位値・WRPR粒度） | 文書ごとにzh/enで綴りが逆転（V003: zh `IWDG_SW`/en `IWDGSW`、V205はその逆）。X315はzh `USBHSDLEN`/en `USBFSDLEN`（**FSとHSの違い**）。X035の復位値はzh `xxxb`/en `xxb`（[7:5]の3bitなのでzhが正しい見込み）。FV2x/V3x enはSRAM分割fieldを無名のまま（zhは`RAM_CODE_MOD`と命名）第3列に復位値`xx1b`を置く。**X315のWRPR粒度**はzh「1扇区（4K字节/扇区）」/en「DBMODE=1: 8K・DBMODE=0: 4K」（enはH417と同文——X315にDBMODEがあるかはRMのFLASH章の記述で裁定すべき） | `option_byte_fields`でconflict＋両論（en記載を採りzhの異議をbasisへ）。裁定はreview（L2）の仕事として残す |
