...
- [ ] **境界で1字が重なる隣接セル（523件 → 本物42件）** — bit図に限って「左セルの末尾＝右セルの先頭」を
  ...
  **残り34件**——`SR`|`Res`（`Reserved` の断片）・`SReserSved`（`Reserved` に大文字 `S` が無いので重複判定を
  通らない）・`SWPMI`|`I2C3`（zh の bit 図は記述表に無い略記）・`5USARTRST`|`T4USART3URST`（数字まで混ざる）。
  次に効きそうなのは**隣のセルの綴りを重複の出所として使う**こと——いまは自セルの中だけ見ている。
