...
- [ ] **続きの断片で単位が落ちる** — 2026-09-10に絶対最大定格の受入で見つけた。表の途中で
  ...
  **再突入の条件**: 「値を1つも持たない行は出さない」を先に決めること（既存の正本にも4行ある
  ——`t_erase_32k`・`t_erase_sec`・`V_DDA`×2。これらも事実を持たないので落とすのが筋だが、
  正本が動くので単独で測って裁定する）。そのうえで単位の持ち越しを入れ、増える行を
  1件ずつ原本と照合する。絶対最大定格の表（`extract_absolute_maximum.py`）は**新しい表なので
