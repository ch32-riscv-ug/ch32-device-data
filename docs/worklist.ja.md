...
### ⚠ CH32X035DS0.zh V2.3 の取り込みは撤退（2026-09-09）
...
2. **新しい表に到達できない** ❌ 全corpus拡張。`read_edition` は「見出し語のページ＋続き1ページ」しか
   読まず、V2.3 で見出し語が p26→p25 に動いて表3-14（p27）が窓の外に出た。緩める案の生の行数:
   現行 3,309 → 表の題を足す **3,932（+19%）** / 行が続く限り 5,561（+68%）/ 列が同じ限り 5,999（+81%）。
   **A11 の +1,208 行と同じ「新しい表選択規則」の企画**で、足した行を1件ずつ見るのが受入条件。
3. **基礎行は en 主導** ❌ 設計。正本2,797行で **zh のみを根拠にする行は 0**（en のみ181・両方2,616）。
   zh だけが V2.3 の今、新しい PA/PB/PC の行は原理的に出ない——en 版の V2.3 を待つか設計を変える。
