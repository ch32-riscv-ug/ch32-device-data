...
| 項目 | 判断 | いま言えること |
|---|---|---|
| **電気特性・低消費電力**（絶対最大定格・動作/待機/スリープ電流・Flash 書換回数と保持期間・各低消費モードのウェイクアップ時間・GPIO 駆動能力と入力閾値・ADC 精度） | 🔜 **次に価値が高く、思ったより安い**（2026-08-29 に実測） | **抽出器を書く仕事ではなく語彙を広げる仕事**だった。`build_operating.py` は**すでに電気的特性表と絶対最大定格表を歩いていて**、`KEEP` 正規表現（`F_*`・`f_ADC`・`V_DD`・`ACC_*`・`DuCy_*`）で落としているだけ。`KEEP` を外して全 datasheet を走らせた実測: **304行 → 1,521行**（新規 1,217行・**164 記号**・**27 series 全部**・confirmed 1,014 / ref 140 / conflict 63）。監査が名指しした項目はほぼ全部この中にある——`N_END`（書換回数 50K）・`t_RET`（保持 20年）・`I_DDA`/`I_DD(HSI)`/`I_DD(LSI)`（消費電流）・`t_SU(HSI)`/`t_SU(LSI)`/`t_SU(HSE)`/`t_STAB`/`t_RSTTEMPO`（起動・ウェイクアップ）・`V_IH`/`V_OL`/`V_hys`/`I_lkg`（GPIO の閾値・駆動・リーク）・`t_s`/`t_CONV`/`f_S`/`R_ADC`/`C_ADC`（ADC）・`t_prog_page`/`t_erase_sec`（Flash 書き込み）・`T_J`/`V_POR/PDR`/`V_DDA`。<br>**そのまま入れてはいけない。** 測ったのは `KEEP` を無効化しただけの生データで、既存の健全性検査（`UNIT_FOR`）は既存の記号にしか当たらない。新規 1,217行のうち**数値だけで読めるのは 1,130行（93%）**、87行は非数値だが多くは正当な記号上限（`0.8VDD`・`VREF+`・`100K`）で、**本当に壊れているのは十数行**（`V+0.DD3` のような添字の食い込み、`I_DD` の min に条件文 `F=HCLK48MHz` が流れ込む、記号が `C_in（LSE）` と全角、`t_/t_w(SCKH)_w(SCKL)` の交互合成）。既存の `SYMBOL_FIX`/`TEXT_REPAIRS`/`UNIT_FOR` が同種の修繕の受け皿を持っているので、**採る記号を決めて単位規則を書き、壊れている十数行を潰す**のが作業の中身。<br>**この過程で tool 側の穴を1つ塞いだ**: `build_operating.py` だけ `--out` を受けず（argparse 自体が無かった）、渡しても黙って無視して `evidence/` に書いていた。**安全に試す方法が無い**状態で、実際に一度正本を上書きした。`--out` を足して、他の生成器と同じく試験用の出力先を受けるようにした |
