...
| F-73 | **`CH32M030` の `ADC_ETR` の既定 pad を RM と datasheet が逆に書く**。RM 表6-15（`CH32M030RM.zh` p.55／`.en` p.62）は `ADC_ETRGIN_RM=0 默认映射`→`ADC外部触发转换与PB6相连`、`=1 重映射`→`…与PA14相连`。datasheet の pin 表（`CH32M030DS0.zh` p.20／`.en` p.25-26）は PA14 に `ADC_ETR`（接尾辞なし＝既定）、PB6 に `ADC_ETR_1`（`_1`＝AFIO の値1）を書く。**zh/en とも各資料の中では一致**しているので版の食い違いではなく**資料どうしの食い違い**。`ADC_ETRGIN_RM` は1ビットなので「どちらかが既定」しか有り得ず、両立しない | 1 signal・2経路。併せて ADC のトリガ格子 28 行が `basis` に `rm-remap-grid` を名乗れない（F-68 の残り） | 資料 | 🔧 **繋ぐ前にスキーマの決めごとが要る**。格子の行ラベルが文章の表（`ADC外部触发规则转换`）は、**どちらのトリガかを field が言っている**ので綴りは決められる（`extract_remap.GRID_PROSE_SIGNAL` に対応表を置いてある。**まだ繋いでいない**）。繋ぐと `CH32M030` で同じ値に pad が2つ並ぶ——`remap_routes` に `conflict` の持ち方が無いのが根で、F-61 と同じ形。歯止めは入れた: 格子の値を採る訂正は、**pin 表が同じ (signal, 値) を別の pad に与えているときは効かない**（`build_candidate`）。実測で確かめてある（歯止めが無いと PB6 が値1→0 に動いて `ADC_ETR` の値0が PA14 と PB6 の2つになる。入れると pin 表の値1が保たれる）。**どう表すかは consumer と決めること** |
