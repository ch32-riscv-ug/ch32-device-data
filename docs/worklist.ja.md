...
| F-72 | **`curated/errata.csv` の `v30x-*-default-map` 6件が `CH32V317` を名指していない**。datasheet の表題は `CH32V303/305/307/317数据手册` で、注記はseries を限定せず批号だけで区切っている。`CH32V317` の pin 表は `SDIO_D0`（PC8/PB14）と `DVP_D5`（PB6/PB3）の両方の pad を載せているので、少なくともこの2件は V317 にも効くはず。ただし同じ文書由来の `v30x-vio-min-1v2` は V317 を**含んで**いるので、除外が意図的かもしれない | `pin_conditions` で V317 の3行がロット依存に繋がらない（2026-09-16 の実測） | 資料 | 🔧 **人の裁定待ち**。抽出側では足さない（`errata` は curated で、`series` は人の判断）。`extract_pin_conditions` が「注記が裏付けたが errata に繋がらない行」を series ごとに数えて出すので、裁定が入れば数で確かめられる。`CH32V407`/`CH32V467` の4行ずつが繋がらないのは**正しい**——`CH32V407DS0` の注記には批号の条件がそもそも無く、自動の切り替えは常に効く |
| F-73 | **`CH32M030` の `ADC_ETR` の既定 pad を RM と datasheet が逆に書く**。RM の表6-15 は `ADC_ETRGIN_RM=0 默认映射`→**PB6**・`=1 重映射`→**PA14**、datasheet の pin 表は **PA14=default・PB6=remap-1**。zh/en とも各資料の中では一致しているので、版の食い違いではなく**資料どうしの食い違い** | 1 signal・2経路。併せて ADC のトリガ格子 28 行が `basis` に `rm-remap-grid` を名乗れない（F-68 の残り） | 資料 | 🔧 **繋ぐ前にスキーマの決めごとが要る**。格子の行ラベルが文章の表（`ADC外部触发规则转换`）は、**どちらのトリガかを field が言っている**ので綴りは決められる（`extract_remap.GRID_PROSE_SIGNAL` に対応表を置いてある。**まだ繋いでいない**）。繋ぐと `CH32M030` で同じ値に pad が2つ並ぶ——`remap_routes` に `conflict` の持ち方が無いのが根で、F-61 と同じ形。歯止めは入れた: 格子の値を採る訂正は、**pin 表が同じ (signal, 値) を別の pad に与えているときは効かない**（`build_candidate`）。実測で確かめてある（歯止めが無いと PB6 が値1→0 に動いて `ADC_ETR` の値0が PA14 と PB6 の2つになる。入れると pin 表の値1が保たれる）。**どう表すかは consumer と決めること** |
