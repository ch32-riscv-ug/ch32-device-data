...
en 単独の reference。コード変更なので取り込みとは別commit）、`I_DD Standby/Stop` 4行は V2.3 の表番号振り直しで zh 側の裏が
