...
| 表示（G系） | 12 | 0 |
| 既知の穴（F系） | 48 | 9（下の F 台帳で ✅ が付いていない行の数。資料側の記録が7件——F-7・F-33・F-43〜46・F-51——と、ツール側が2件（F-57・F-58。どちらも見出しの綴りで zh/en の行が対にならないもの）。F-4 と F-24 は残りだけが資料側で実害なし） |
...
- [x] ✅ **A10 型番×能力の縦持ち索引** — `index/capabilities.csv`新設（2026-08-29。1,714行）。`evidence/product_attributes.csv`（158種類の綴り・1,721行）**だけ**から作る。`index/parts.csv`は横長なので比較表の属性を13列しか持てず、残り145種類は綴りを知っている人しか引けなかった。属性→能力の対応は**総当たりの辞書**（`tools/build_capabilities.py`の`CAPABILITIES`）で、regex で畳まないのは`adc`（チャネル数）と`adc_unit`（ユニット数）のように**似た綴りで意味が違う**ものが混ざるため。辞書に無い属性が現れたら生成が落ちる。`stated`列が資料の言い方（`count`＝素の整数1,186行／`marker`＝`√`160行／`text`＝`8+2`等368行）を言い、`count`が入るのは`stated=count`のときだけで**数える規則は当てない**。**新しい PDF は読んでいない**（既存の証拠の並べ替え）
...
| F-57 | 比較表の zh/en が**同じ値を違う綴りで書くと対にならず、同じ属性が2行になる** | 1組（CH32H416RDU6 の SRAM 共有領域。zh `512KB` / en `512K`） | ツール | ⬜ **記録のみ**（2026-08-29。`index/capabilities.csv` を作る過程で見つけた——同じ (型番, 能力, qualifier) が2行になる唯一の組）。`crosscheck_languages.canonical_value` が `512kb` と `512k` を別物と読むので LCS の対応付けが外れ、`product_attributes` は `sram_shared_code_and_data_area` と `sram_shared_code_and_data_area_`（末尾 `_` で衝突回避）の**2行**を持ち、どちらも `reference`（本来は1行の `confirmed`）。**値そのものは失っていない**（両版とも 512K）。直すなら `canonical_value` に容量の単位の同一視（`KB`↔`K`）を足すが、**比較表の全 family の対応付けに効く**ので、`build_tables.py` のフル実行と全表の差分確認を伴う別作業にする。`index/capabilities.csv` はこの1組を `KNOWN_DOUBLED` で数として固定している |
| F-58 | 比較表の **GPIO 数の英語見出しが同義語表に無く**、中文版だけが昇格する | 4型番（CH32V003）＋同じ見出しを持つ family | ツール | ⬜ **記録のみ**（2026-08-29。G5 の過程で見つけた）。`build_tables.CANONICAL` は `gpio_count` の同義語に中文の `通用IO` を持つが英語の `General purpose I/O` を持たないので、`promoted()` が中文版の行だけ列へ上げ、英語版の行は `product_attributes` に残る。結果、比較表に **`GPIO` と `General-purpose I/O` が同じ値で2行**並んでいた（表示側は G5 で畳んだ）。直すのは同義語1つだが、**`promoted()` は全 family の全見出しに効く**ので `build_tables.py` のフル実行と全表の差分確認を伴う別作業にする。`gpio_count` の値そのものは中文版から取れていて正しい |
...
| 項目 | 判断 | いま言えること |
|---|---|---|
| **電気特性・低消費電力**（絶対最大定格・動作/待機/スリープ電流・Flash 書換回数と保持期間・各低消費モードのウェイクアップ時間・GPIO 駆動能力と入力閾値・ADC 精度） | 🔜 **次に価値が高い**。ただし PDF を読む本物の抽出作業 | このリポジトリで**最大の未収集**で、型番選定や電池寿命の見積もりには pin 情報より効く。出所は datasheet の電気的特性章で、`build_operating.py` がすでに同じ章を読んでいるのでその拡張になる。**3 family で試験抽出してから広げる**（章の体裁が family で揃っているかを先に測る） |
...
| **出所・矛盾の縦持ち索引**（`claim_id, subject, predicate, value, source, ...`） | ⬜ 保留。**安い部分だけ先に切り出せる** | `basis` が1セル内の DSL なので横断で引けない、というのは事実。ただし「両版が食い違う全仕様」は `confidence=conflict` の行を全表から拾えば出るので、**まず conflict だけの索引**から始めるほうが安く、効果も測れる（**証拠の表の conflict は 165 行**。memory_configs 67・register_fields 38・product_attributes 25・pin_functions 12・opa_cmp_registers 5・operating_conditions 5・clock_symbols 5・registers 4・adc_internal 2・flash_geometry 1・timers 1。この数は `check_docs.py` が数え直す） |
