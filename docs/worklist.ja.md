...
| F-61 | **選択ビットが `*_RM` でない remap 格子を読み飛ばしている**。列見出しが `FSMCEN=0`/`DVPEN=1` のような**周辺の有効化ビット**や、`SDIOEN=1&ETHMACEN=1` のような**論理積**で、`COLUMN_HEADER`（`*_RM\d?=値`）に当たらない | **全corpus実測（2026-09-15）で16表。中身まで測ったら、読めていない情報は6行だけだった** | ツール | 🔧 **半分修理・残りは裁定済みで記録のみ**（2026-09-15）。<br>**(1) 修理した**: `read_header` が「1列でも読めないと表ごと捨てる」のを**読めない列だけ `None` にする**に変えた。CH32V407 の DVP/SDIO は3列中2列が普通の `*_RM` で、`extract_remap` の経路が 374 → 424（zh/en 対称・消失0、版面の表10-42/10-43 と全件照合）。併せて `1DVP_RM` という在りもしない field を直した（`DVPEN=1DVP_RM=0` の `1` を巻き込む正規表現の誤り）。<br>**(2) 読んでも何も増えない**: `CH32V407RM` の FSMC 格子（`FSMCEN=0 默认映射｜FSMCEN=1 重映射`、zh 18行）は**18行とも2列が同じ pad**——「FSMC の pad は動かない」と書いているだけで、経路の情報を持たない。`复用功能｜默认映射` の1列だけの表（6行）も既定の pin 配置そのもの。<br>**(3) 残るのは6行で、これは schema の概念が要る**: `CH32FV2x_V3xRM` の4表で列が実際に違うのは`FSMC_NADV PB7→PD2`・`DVP_D5 PB6→PB3`・`SD0 PC8→PB14`・`SD1 PC9→PB15`・`I2S3_MCK PC7→PA8`・`I2S3_SD PB5→PA9` の6行だけ。**条件が入れ子**（`SDIOEN=1` が `SDIOEN=1&ETHMACEN=1` を含む）なので、`(値→pad)` の平らな対応では同じビット並びに2つの pad が付く——「より限定的な条件が勝つ」という優先順位の概念が要る。`remap_fields` の selector は AFIO の書き込める field を前提にしていて、RCC の有効化ビットを selector にすると consumer が `GPIO_PinRemapConfig` で書こうとする。**新しい概念を足すかは consumer と決めること**。なお `SD0`/`SD1` の2行はpad セルが `PB1`+`4` に割れてもいる（その表の列境界の問題） |
