...
このrepositoryの成果物は **`catalog/`（目録8表）・`evidence/`（証拠41表）・`index/`（索引13表）と、そこから生成する各family
...
uv run pipeline/checks/check_doctests.py  # 正規化規則の仕様（pipeline/ と tools/ の doctest）
