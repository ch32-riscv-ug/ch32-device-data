...
| 5 | 難所の標本監査 | 🔧 第一回 2026-09-01 | fixture 7文書をbundle化し既知難所8種を検査。**全難所がL0に材料を残す**。RMは表の85%がページ継続でL1結合が主戦場。zh/en対応はcaption番号だけでV003 30/30・H417 104/104の1:1（衝突0） |
...
- **8 表示用途**: ✅ 利用者に確認した（2026-09-01）。**用途は「PDFと同じものを
  ...
  - 第一歩は**header/footerの除去だけ**。L0は行に`header`/`footer`のroleを既に
    付けているので、rendererがそのroleの行を落とすだけで成立する（roleの付与精度は
    fixture監査の続きで検証する）
