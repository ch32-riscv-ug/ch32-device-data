...
多bit field を **`00b` / `000b`（2進、b付き）** と書く行が全部 None になっていた。
2進を足して 45行→7行。残る7行は EXTEN の `CTR`（CH32M030 の `ISINK*_ADJ`、
CH32V20x の `ETH_10M_EN`）と CH32V103 の `TIM4_REMAP`/`USART2_REMAP` で、
それぞれのレジスタ表が復位値の列を持たない／別の形で書いているもの。**RM 自身が
0 と書いていないものは 0 と埋めない**（推測しない）。

**F-35**: valid_values の出所は「RM 格子・pin 表・ヘッダ・EVT デコーダ」の和だったが、
pad に出ない経路（`TIM5CH4_RM=1` は LSI 内部クロックへの remap）はどれにも
現れない。RM の field 説明文が `0：默认映射…；1：重映射…` と**値を列挙している**
ので、それを5つ目の出所（`RM説明文`）にした。`x` は展開（`1x：` → 2,3）。
TIM5CH4 は `0;1` に。他 field でも説明文が列挙する値が入るようになった（幅を
超える値は従来どおり除外）。

**F-36**: 条件欄の添字（`f_S` の `S`、`V_DD33` の `DD33`、`T_A` の `A`）が PDF の
文字層で別の行になり、`f > 1MHz S` のように孤立していた。この表に出る添字は
9 種しかないので、孤立したそれを**手前の裸の記号（比較演算子の前に単独で立つ
f/V/T/I）へ戻す**。13行が直り、残り0。行数・確度分布は変わらない。
