...
各規約の例外は根拠を絞りました。`/`規約で改行が区切りになるのは**前の行がAF番号の括弧を閉じているとき**だけ（`TIM11_CH3(AF13)` ⏎ `QSPI1_SIO0(AF10)`）、改行規約で改行が名前の途中になるのは**次の行が2文字以下の切れ端のとき**だけ（`ETH_MII_PPS_OU` ⏎ `T`）。`pin_functions.csv`は29493→27850行になり、`DD`/`SS`/`LTD`/`Main V`/`ART10_RTS_3LED0`は0行、`VSS`/`OSC_OUT`/`VDDK`/`VREF+`/`VDD33`が組み立て直されました。**remapのbit解釈には影響しません**（EVTデコーダとch32-dataの一致数が動かなかった）。残るのは片方の言語版だけに出る6行です。
