...
**添字は独立したセルになります（未修理。[worklist F-1](worklist.ja.md#f-1-pin表の電源pin名が添字で分断される)）。** datasheetは電源ピンを`V`＋添字`DD33`のように組んでいて、テキスト層では2つに割れます。`pad`列は`VDD33`と正しいのに`signal`列が`V`と`DD33`に分かれ、`pin_functions.csv`の約2.9%（`V`が569行、断片が283行）がこの形です。remap経路には影響しません（電源ピンにroute selectorは無い）。同じ機構でsignal名も切れます（`LTD`←`LTDC_*`、`I2S3_W`←`I2S3_WS`、`ART10_RTS_3LED0`←`USART10_RTS`と`LED0`の混線）。`tools/build_operating.py`の`norm_symbol`が電気的特性表で同じ組み直しをしているので、前例はあります。
