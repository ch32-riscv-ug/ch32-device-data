...
挙げるのはCH32V20x・CH32V307・CH32V407・CH32X315の4 familyですが、
**CH32X315のコメントだけ嘘です**——`CH32V4x7RM.PDF`を指して576K/136Kを挙げるのに、
X315のheaderに`RAM_CODE_MOD`は無く、datasheetは
「480KB闪存包含192KB的零等待程序运行区域和288KB非零等待区域」と固定で書いています。
V407からのコピー忘れで、**RM側と突き合わせるとこれが自動的に浮上します**。
...
**改行の意味はデータシートごとの規約で、セル単位で決まります（修理済み）。** 2種類あります——**`/`で区切る**family（H417・V407・X035・V003・M030など）では改行は名前の途中で、**改行で区切る**family（V20x・V30x、`/`を一切使わない）では改行が区切りです。改行ごとに判断すると必ず片方を外します。4 familyで区切りを挿入していた458箇所のうち、**3箇所を除いて全部が名前の途中**でした（`V`+`SS`、`SD`+`RAM_D20(AF12)`、`LT`+`DC_G5(AF14)`、`I2`+`C4_SMBA(AF4)`、`OSC_OU`+`T`、`D`+`VP_VSYNC(AF15)`）。レンダラは**列幅が尽きた位置で折り返す**ので、断片はトークンになっていません。

電源ピンは別の機構です——datasheetは`V`＋添字`DD33`のように組んでいて、**添字が独立した行になります**。`pad`列は`VDD33`と正しいのに`signal`列が割れていました。pin表に1文字のsignalは存在しないので、末尾が単独の大文字なら常に連結できます。

**規約が同じセルの中で混ざることがあります（2026-08-22追記）。** CH32V407のPB5は
`/`規約なのに、`/`が落ちた改行を1つ持ちます:
...
`MOSI`と`I2S3_SD`の境目には**セルの中に手がかりが1つもありません**。当初の規約
（「`/`のあるセルでは裸の改行は区切りでない」）では`SPI3_MOSII2S3_SD`という
存在しないsignalになり、30行が壊れました（V407/V467のSPI3・I3C、V208の
ADC/USART2/TIM3/TIM5、OPA2）。

**手がかりは同じ表の他のセルにあります。** `/`で囲まれて改行を含まないセグメントは
1つのsignalそのものなので、そこから**その表が使う周辺機器名の語彙**を集められます。
次の行が`I2S3_`で始まるなら`I2S`は語彙にあるので区切り、`RAM_D20(AF12)`で
始まっても`RAM`は語彙に無いので継続（CH32H417の`SDRAM_D20`の後半）。
インスタンス番号は落として比較します——表が`I2S2_CK`を明示していても
`I2S3_SD`は改行の後にしか出ないことがあるためです。1文字の語幹は除きます
（除かないと`LTDC_R2`が`LTD`/`C_R2`に割れたときに`C_R2`をsignalと読みます）。

各規約の例外は根拠を絞りました。`/`規約で改行が区切りになるのは**前の行がAF番号の括弧を閉じているとき**だけ（`TIM11_CH3(AF13)` ⏎ `QSPI1_SIO0(AF10)`）、改行規約で改行が名前の途中になるのは**次の行が2文字以下の切れ端のとき**だけ（`ETH_MII_PPS_OU` ⏎ `T`）。`pin_functions.csv`は29493→27850行になり、`DD`/`SS`/`LTD`/`Main V`/`ART10_RTS_3LED0`は0行、`VSS`/`OSC_OUT`/`VDDK`/`VREF+`/`VDD33`が組み立て直されました。残るのは片方の言語版だけに出る6行です。

その後、上の「規約がセルの中で混ざる」分を直して**27982行**になりました。EVTデコーダとの一致は261→**263**、ch32-dataとの一致は203→**211**（不一致はどちらも増えず、ch32-data側の既知の1件のみ）。

**この「残留0件」の数え方が狭すぎました。** 語幹（`I2S3_`のように`_`を持つ接頭辞）を含む連結だけを数えていて、「同じdatasheetが綴る**名前そのもの**を内側に含む」まで広げると残っていました（`USART10_RTS_2USART10_RTS_3LED0`）。**改行が1つも無いのに2つ以上の名前が詰まっているセル**があり、datasheetが`/`を打ち忘れていて版面には手がかりが残っていません。繋いだ後の名前を、datasheetが綴る語彙で切り直しています（`extract_pins.resplit`。worklistのF-13の続き）。最終的な行数は**27926**、`confirmed`は24572→**27719**、`reference`は3410→**207**です。


- **pad名に脚注が付く**（`PA7(7)`、`PC16(4)(9)`）。`VDD`が`V\nDD`と改行で分断されることもあります。折返しが脚注の内側に入る例もあり（CH32M030の`PA13(7\n)`）、空白除去を脚注除去より先に行わないと1 pin落とします
- **その括弧は半角とは限りません。** 中文版は全角で`（7）`と打ちます。`build_operating.py`側では最初から全角を見ていましたが（下記の動作条件表の項）、`extract_pins.py`の脚注regexは半角だけでした。結果、**pad名`PA5（4）`が正規化に通らず行ごと落ち**（CH32V30xで38 pad）、signal名には`SDIO_D0（7）`・`PD0（4）`・`SPI3_MOSI（12）`のような**括弧付きの名前が46種・364行**残っていました。AF番号`（AF15）`も同じ理由で`route`に化けず名前に残ります。**同じ規約は同じregexで見ること**（2026-08-22に修正）
- **ASCII前提の書き方が中文版で崩れる型が3つ出ました**（2026-08-22。どれも「中文版だけ取れない」という同じ症状になります）:
  - **括弧が全角**（`（7）`）。半角だけ剥がすとpad名が正規化に通らず行ごと落ちます（F-16）
  - **`\b`が境界にならない**。中文は「的48MHz时钟」と続けて書き、**CJKも語構成文字**なので`\b48`は当たりません。数字の続きでないことだけを見る（`(?<![\d.])48`）ようにします
  - **空白が無いので語頭が取れない**。`squash()`で空白を落としてから部分一致を見ると、`FMC SDRAM`が`sram`に、`programmable`が`ram`に当たります。squashする時点で語頭を記録し、**字種が変わる位置も切れ目**として扱います（`非零等待Code FLASH`）（F-15）
...
- **中国語版の文章中のpadは拾えていませんでした（2026-08-22に修理。[worklist F-3（修理済み・記録はarchive）](worklist-archive.ja.md)）**。文章からpadを拾う正規表現が`\bP[A-H]\d{1,2}\b`で、Pythonの`\w`はCJKを含むため`与PD1相连`の`PD1`の前に語境界が立ちません。中国語版のADC触发表（`ADC外部触发注入转换与PD1相连`）が0件になります。いまは英語版が同じ表を"connected to PD1"と書いているので和で埋まっていますが、**英語版RMが無いCH32V407/V467では埋まりません**（あちらの格子はpadを裸で書くので現状は影響なし）。前後をASCIIだけで見る形（`(?<![A-Za-z0-9])P[A-H]\d{1,2}(?![0-9])`）にすれば直りますが、中国語版の文章由来経路が全familyで増えるため、依頼の範囲外として手を付けていません
...
- **レジスタfield表が格子の続きとして読まれる**。remap格子のすぐ後ろにAFIO_PCFR1のfield表（`位｜名称｜访问｜描述｜复位值`）が来ます。その描述セルはpad名で埋まっているので、続きとして読むと**そのfieldに全部のpadが帰属します**。CH32V00xの英語版RM p.86〜87で`ADC_ETRGREG_RM`が**値1で35 pad**（PA0〜PD7のほぼ全部）になり、格子の言う`PC2`1つを埋めていました。この汚染で`(PA2, 1)`が`ADC1_ETRGINJ`と`ADC1_ETRGREG`の2候補になり、`ADC_IETR`が決まらなくなっていました。field表の列見出しを持つ行は続きと見なさず、**bit番号（`[22:20]`・`19`）が行見出しの行は読まない**ようにしました——格子の行見出しは信号名か信号についての文章で、bitではありません。field表はもともと`extract_registers`が読むので失うものはありません
- **ページを跨いだところで別の表と合体する**。格子はページを跨ぐので、続きページはヘッダ行を持ちません。列数が一致すれば続きとみなす、という判定を入れていたのですが、**次の格子が始まっているのに列数が一致する**ことがありました。CH32V407のp108にTIM3の格子（4列）、p109にTIM4の格子があり、TIM4のヘッダは2行に割れて空セルを1つ含みます（`["復用功能","TIM4_RM=0默認映射","","TIM4_RM=1重映射"]`）。ヘッダ判定が「全列がfield=値に一致すること」を求めていたためこの空セルで却下され、4列という一致だけで前ページの続きになりました。結果、**TIM4の経路がTIM3_RMに、値1が値3として**入りました。V103のTIM3、CH32V30xの`FSMC_NADV`/`DVP_*`も同型です。空セルは列位置を保ったまま`None`として許し、**field=値を含む行やその格子の行見出し（`復用功能`/`Alternate function`）を持つ行は続きと見なさない**ようにして解決しました。**静かに間違える種類**で、値まで変わるので影響が大きいです
...
**F-5は2026-08-22に直しました。** 見出しの読み方が原因で、(1) register名が行末に
無い（適用範囲をもう1つの括弧で後ろに足す `（DMAy_CFGRx）（x=1/2/...，y=1/2）`）、
(2) placeholderが小文字（`DMAx_INTFR`・`GPIOx_CFGLR`）の2つ。当たらないと
`register`が前の値のまま残り、その章の表が全部前のregisterのものになります。
**見出しが来たら名前が読めなくても`register`を必ず入れ替える**（読めなければ
捨てる）ことにして解消しました。実測で新しく名前が取れる見出しが
CH32FV2x_V3xRMで161・CH32H417RMで271、**取れなくなるものは0**。
CH32H417の`AFIO_EXTICR2`のfieldは59→0になり、DMAのfieldは`DMAy_CFGRx`へ移りました。
下記の2段の条件は、この修理後も**RMが共有されることによる持ち込み**（ETH）に
対して要るので、そのまま残しています。
...
| 段 | 根拠 | 強さ |
|---|---|---|
| 1 | RMの格子がこの(signal, 値, pad)をそのまま書いている | field名まで一致するので最強。ただし格子側の崩れ（上記）は伝播する |
| 2 | signalが名乗る周辺が自分のselectorを持っている | 名前は「誰の経路か」を言う |
| 3 | (pad, 値)が1つのselectorだけを指す | 名前を読まずに済む代わりに**誰の経路かは言えない**。格子由来の経路を先に見る |
| 4 | selector名がsignalの周辺名を前置している（`ADC_ETR`→`ADC1_ETRGINJ_RM`、`ISINK1`→`ISINK1_ADJ`） | 名前の部分一致 |

**段3と段4の順序は、名前が読めるかどうかで入れ替わります。**

| signalの語彙規則 | 順序 | 理由 |
|---|---|---|
| 当たる | 1 → 2 → **3** → 4 | padはフィルタで監視できるので段4より強い。CH32V00xの`ADC_IETR`は2つのADC fieldのどちらかで、格子のpadが決める |
| 当たらない | 1 → 2 → **4** → 3 | padは無検査で答えるので、selector名の構造一致のほうが確か。CH32M030の`ISINK1`はPA6をTIM2と共有していて、pad優先だと`afio-tim2-remap`になる |

段3は「名前を読まなくても引ける」利点と引き換えに持ち主を言えないので、**名前が読めるときは名前が許すselectorでしか答えません**（反証されないだけでは足りません）。CH32V002の`ADC_IETR`はPA2をTIM1と共有していて、段3が`TIM1_RM`を返していました。

「反証されないだけでは足りない」のは、**自分のfieldを持たない周辺があるから**です。CH32V30xの`I2S3_*`は`SPI3_REMAP`が経路を決めるので、`I2S3`という名前のselectorは存在せず、**I2S3について反証できるものが何もありません**。反証だけを条件にすると、`I2S3_MCK`のPC7が`TIM8_RM`（TIM8_CH2が同じpadを使う）で通ってしまいます。積極的一致を求めれば未決定のまま残り、RMがその経路を書いていない以上それが正しい状態です。CH32V407では同じ信号が格子に載っているので段1で決まります——同じ周辺が、資料の書き方次第で決まったり決まらなかったりします。

段1も無条件ではありません。**signalが名乗る周辺が自分のselectorを持っているなら、別のselectorが持ち主であることはあり得ない**ので、そこだけ却下します。CH32V407の`TIM4_CH1`が`TIM3_RM`として出てきた例がこれで、TIM4にselectorがあるので名前だけで反証できます。一方CH32V30xの`DVP_D5`は同じ形で出てきますが**反証できません**（headerに`DVP_REMAP`が無いのでselectorが存在しない）。この場合は却下せず、格子の崩れを直したうえで未解決に落ちます。

**共有フィールドは例外指定なしで通ります。** CH32V407の`I2S3_WS`は本当に`SPI3_REMAP`が経路を決めていて、`I2S3`という名前のselectorは存在しないので、上の判定は何も言いません。

段4には1つ追加があります。CH32H417は`UHSIF_CLK_RM`と`UHSIF_PORT_RM`を両方持つので、`UHSIF_PORT33`は前置一致だけでは2つに当たって決まりません。**selector名＋数字**（`UHSIF_PORT` + `33`）で選べます。

段3は既定経路（値0）でも使いますが、**格子由来だけ**です。datasheetのpin表の「default」列とpadを掛け合わせるのは無意味で（リセット時はpadを共有する周辺が全部値0なので誰も勝たない）、一方でRMの格子の値0の列は**そのfieldについての言明**です。名前フィルタが他の周辺を除くので、上の2つの反論はどちらも当たりません。CH32V00xの`ADC_IETR`は`PD1`が既定・`PA2`が値1で、格子がその両方を言っています。

段3は経路の出所も見ます。上の表の3つの出所のうち**RMのregister説明文だけは散文を正規表現で読んでいる**ので、register field表の行が次のfieldへ流れ込むと関係ないpadを吸い込みます。CH32V00xの`ADC_ETRGREG_RM`は説明文から**値1で35 pad（PA0〜PD7のほぼ全部）**として出てきますが、格子（表7-15）は`PC2`1つだけです。この汚染のせいで`(PA2, 1)`が`ADC1_ETRGINJ`と`ADC1_ETRGREG`の2候補になり、格子が一意に決めているはずの`ADC_IETR`が決まりませんでした。**格子由来の経路だけで先に引き、それが何も言わないときだけ全体に落ちる**ようにしています（CH32X035は格子を持たないので、常に後者になります）。

なおRMの本文（p.72）は`19 ADC_ETRGREG_RM RW 1：ADC外部触发规则转换与PC2相连；0：…与PD3相连`、`18 ADC_ETRGINJ_RM … 1：ADC外部触发注入转换与PA2相连`と書いており、datasheetのpin表の`ADC_RETR`(PC2, remap-1)・`ADC_IETR`(PA2, remap-1 / PD1, default)とpadで一致します。つまり`RETR`=规则(regular)=`ETRGREG`、`IETR`=注入(injected)=`ETRGINJ`は**資料から確認できる**対応で、推測ではありません。ただし語彙規則としては入れていません——padの一致で決まるので要らないからです。

`tools/check_tables.py`が生成後に同じ判定で逆向きに検査します（「そのperipheralが自分のselectorを持つ」か「名前が同じで番号だけ違う」なら誤帰属として報告）。
