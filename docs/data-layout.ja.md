...
- **結合した1ファイルが正本**（全型番・全family。generator はこれを読む）。加えて、問いが型番から始まる表は**型番ごと（family の周辺なら family×型ごと）の分片**を同じ生成器が切り出す。分片は正本の行をそのまま並べ替えただけで、GitHub で表として開ける大きさ（数百行）になる。**分片を連結すると正本に一致する**ことを検査する
- 「どの型番が」を探す表（比較表・機能索引）は横断なので正本だけ（分片なし）
...
| 索引 | **CSV。結合した正本1ファイル＋型番／family ごとの分片**（`index/pinout.csv` と `index/parts/CH32V203C8T6.csv`） | generator は正本を1つ読めばよく、自分で結合しない。人は分片をクリックして表として読める。分片は `.gitattributes` で `linguist-generated` にして PR の diff では折り畳む（同じ変更が2度見えないように）。行＝1事実で JSON より形が崩れにくい | 型番ごとの JSON（generator には便利だが GitHub で読めず、入れ子が schema の揺れを招く。**8/25 に消した `devices/*.json` と同じ形に見える**——あれは手で書く正本、これは生成物、という違いはあるが、いま戻す理由が無い） |
...
index/       索引  正本: parts.csv, pinout.csv, routes.csv, registers.csv, register_map.csv,
                        dma.csv, timers.csv, features.csv, register_layouts.csv, manifest.csv
                   分片: parts/<PART>.csv（pinout）, series/<SERIES>/routes.csv,
...
- **`pins.html` は GitHub Pages で配る**（main の root、`.nojekyll` 付き。有効化は要確認）。viewer は分片 `index/parts/<PART>.csv` を1つ fetch すればよくなる
...
| `pin_roles` | **置き換え** → `index/pinout.csv`（正本）＋ `index/parts/<PART>.csv`（分片） | `pins` の lead と `remap_routes` の selector/値を結び、機能の無い lead も行に持つ。consumer は `pin_roles.csv` の代わりに `pinout.csv` を読む |
| `pin_alternate` | `evidence/`（安定） | そのまま |
| `product_attributes` | `evidence/` | `attribute` は付与識別子として残す（`label_zh/en` が隣にある）。`#` 列を入れて列順の規約に合わせる |
| `feature_tags` | `index/features.csv` | 名前だけ。分片なし（横断） |
...
| `register_layouts` | `index/register_layouts.csv` | 導出（ハッシュ）なので索引。横断なので分片なし |
...
証拠 32表・目録 7表・索引 正本10表（うち6表に分片）。

## 6. 新しく作る索引の中身

正本と分片の対応。分片は正本と同じ列構成で、鍵の列の値ごとにファイルを切っただけ（連結すれば正本に戻る）。

| 正本（generator が読む） | 分片（人が開く） | 切る鍵 |
|---|---|---|
| `index/pinout.csv` | `index/parts/<PART>.csv` | `part_number` |
| `index/routes.csv` | `index/series/<SERIES>/routes.csv` | `series` |
| `index/registers.csv` | `index/families/<FAMILY>/registers/<TYPE>.csv` | `family`, `type` |
| `index/register_map.csv` | `index/families/<FAMILY>/map.csv` | `family` |
| `index/dma.csv` | `index/families/<FAMILY>/dma.csv` | `family` |
| `index/timers.csv` | `index/families/<FAMILY>/timers.csv` | `family` |
| `index/parts.csv` `features.csv` `register_layouts.csv` | なし | — |

大きさの目安: `pinout.csv` は今の `pin_roles.csv`（2.5MB）より lead 行と selector 列が増えて 3MB 台、分片も同量。`registers.csv` は `register_fields`（3.7MB）に offset を足したもの。索引全体で正本＋分片 15MB 前後を見込む（生成物なので保守費は無い。git は差分で持つ）。
...
**`index/pinout.csv`**（U1/U3: この型番の足。分片 `parts/<PART>.csv`）。1行＝1（型番, lead, 機能）。列の先頭に `part_number, series, family`。機能の無い lead（電源・NC・GPIO だけ）も1行持つので、**port＋pin→lead が1表で引ける**。
`part_number, series, family, pin, pad, port, gpio, kind, peripheral, role, signal, route, selector, value, af, #, confidence, basis`
...
**`index/routes.csv`**（U3: remap 値を選ぶ。分片 `series/<SERIES>/routes.csv`）。1行＝1（series, selector, 値, 信号）。
`series, selector, register, bits, value, peripheral, role, signal, pad, port, gpio, #, confidence, basis`
——`remap_fields`×`remap_routes` を結び、語彙で `peripheral`/`role` を付けたもの（いまの `remap_routes` の B 列が移る）。

**`index/registers.csv`**（U4: ヘッダ生成。分片 `families/<FAMILY>/registers/<TYPE>.csv`）。1行＝1（family, 型, register, field）。
`family, type, register, offset, width_bits, count, field, define, bits, mask, value, description, access, reset, #, confidence, basis`
**`index/register_map.csv`**（分片 `families/<FAMILY>/map.csv`）: 1行＝1（family, block, register）で**絶対番地**を持つ（`family, block, type, register, address, width_bits, #, confidence`）——「register の絶対アドレスを1表で」に答える。
**`index/dma.csv`**（分片 `families/<FAMILY>/dma.csv`）: `family, variant, dma, channel, request_id, peripheral, direction, request, remap, condition, note, #, confidence, basis`。
**`index/timers.csv`**（分片 `families/<FAMILY>/timers.csv`）: `family, timer, kind, counter_width_bits, channels, complementary, update_vector, condition, #, confidence`（`channels` は `pinout.csv` から数える）。
...
- 索引: **証拠に無い行が無い**（いまの pin_roles 検査を全索引に広げる）、語彙で覆えない綴り 0、**分片を鍵順に連結すると正本に一致**（分片の数＝鍵の値の数。`parts/` は `catalog/products` の行数）、`index/manifest.csv` の sha256 が一致
...
4. **索引の生成器**（新規4本）: `build_index_parts.py`（`parts.csv`・`pinout.csv`。`build_pin_roles` の語彙処理を引き継ぐ）、`build_index_routes.py`、`build_index_registers.py`（`registers`・`register_map`・`dma`・`timers`）、`build_index_shards.py`（正本を鍵で切って分片を書き、`manifest.csv` を作る。全索引共通）
...
8. consumer 側（ArduinoCore-CH32）: lock のパスを `index/`・`catalog/`・安定な `evidence/` に付け替え、`pin_roles.csv` 読みを `index/pinout.csv` に（列は上位互換）。これは別 repository の作業
...
- 索引は CSV。**結合した正本1ファイルが契約**で、型番／family ごとの分片は人が読むための切り出し（連結＝正本を検査）。consumer の契約は目録＋索引の正本＋「安定」印の証拠
