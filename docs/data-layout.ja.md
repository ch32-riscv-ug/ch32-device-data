...
作成: 2026-08-26、同日に実施。42表の役割の確認（[worklist-archive](worklist-archive.ja.md) の「表の役割の確認」）を入力に、
**「どう管理するのがいいか」をゼロベースで**考え直し、この形に移した。利用者はいま1人（ArduinoCore-CH32
のgenerator＝自分）なので破壊的な変更を前提にしている。判断が要る箇所は**推奨を1つ**に絞った。
実施で案から変えた点は末尾の「実施メモ」。
...
- **1表1ファイル**（全型番・全family。generator はこれを読む）。CSV は機械が読むもので、人が型番や機能で絞って見るのは viewer（`pins.html`）の仕事。型番ごとのコピーは持たない（正本と重複するだけ）
...
| 索引 | **CSV、1表1ファイル**（`index/pinout.csv`） | generator は1つ読めばよく、自分で結合しない。行＝1事実で JSON より形が崩れにくい。人向けの絞り込みは viewer（HTML）に任せる——GitHub の CSV ビューアで読める大きさに切ることは、正本の重複を生むだけなのでしない | 型番ごとの JSON（generator には便利だが GitHub で読めず、入れ子が schema の揺れを招く。**8/25 に消した `devices/*.json` と同じ形に見える**——あれは手で書く正本、これは生成物、という違いはあるが、いま戻す理由が無い） |
...
- 文書: `evidence/README.ja.md`（約1,000行）を3つに割る——`catalog/README.ja.md`（鍵の定義）、`evidence/README.ja.md`（保守者向け。列の A/付与の区別、確定基準、basis 表記）、`index/README.md`＋`README.ja.md`（**利用者向け。英語も置く**——mirror README から辿ってくる人の入口になる）。`docs/table-reliability.ja.md` は証拠の表ごとの信頼度のまま
...
| `pin_roles` | **置き換え** → `index/pinout.csv` | `pins` の lead と `remap_routes` の selector/値を結び、機能の無い lead も行に持つ。consumer は `pin_roles.csv` の代わりに `pinout.csv` を読む |
...
| `register_layouts` | `index/register_layouts.csv` | 導出（ハッシュ）なので索引 |
...
大きさの目安: `pinout.csv` は旧 `pin_roles.csv`（2.5MB）に lead 行と selector 列が増えて 3MB 台、
`registers.csv` は `register_fields`（3.7MB）に offset を足したもの。索引全体で 9MB 前後（生成物なので保守費は無い）。
...
**`index/pinout.csv`**（U1/U3: この型番の足）。1行＝1（型番, lead, 機能）。列の先頭に `part_number, series, family`。機能の無い lead（電源・NC・GPIO だけ）も1行持つので、**port＋pin→lead が1表で引ける**。
...
**`index/routes.csv`**（U3: remap 値を選ぶ）。1行＝1（series, selector, 値, 信号）。
...
**`index/registers.csv`**（U4: ヘッダ生成）。1行＝1（family, 型, register, field）。
`family, type, register, offset, width_bits, count, field, define, bits, mask, value, description, access, reset, #, confidence, basis`
**`index/register_map.csv`**: 1行＝1（family, block, register）で**絶対番地**を持つ（`family, block, type, register, address, width_bits, #, confidence`）——「register の絶対アドレスを1表で」に答える。
**`index/dma.csv`**: `family, variant, dma, channel, request_id, peripheral, direction, request, remap, condition, note, #, confidence, basis`。
**`index/timers.csv`**: `family, timer, kind, counter_width_bits, channels, complementary, update_vector, condition, #, confidence`（`channels` は `pinout.csv` から数える）。
...
- 索引: **証拠に無い行が無い**（いまの pin_roles 検査を全索引に広げる）、語彙で覆えない綴り 0、`index/manifest.csv` の sha256 が一致
...
4. **索引の生成器**（新規4本）: `build_index_parts.py`（`parts.csv`・`pinout.csv`。`build_pin_roles` の語彙処理を引き継ぐ）、`build_index_routes.py`、`build_index_registers.py`（`registers`・`register_map`・`dma`・`timers`）、`manifest.csv` の生成
5. **検査**の拡張（7章）、`build_readme`・`pins.html` を索引の列名へ
6. **文書**: README 3分割、`docs/` と README.md/README.ja.md のパス（`tables/` 参照は docs 12ファイル・workflows 3箇所）、glossary の区分の定義、`worklist-archive.ja.md` はこの文書に吸収して削除
...
- `products.csv` は割らずに `catalog/` に置いた（識別＋列ごとの根拠つき主要仕様）。仕様列の
  確度は4資料（比較表 zh/en・ordering zh/en）の突き合わせの結果で、`product_attributes` からは
  再構成できないため。利用者向けの比較表は `index/parts.csv`
- `registers.member_path` は足していない。平坦化名 `sTxMailBox[0].TXMIR` が構造体メンバー名を
  `.` で繋いだものそのままで、情報を失っていない
- 索引の生成器は1本 `tools/build_index.py`（`--only` で表を選ぶ）。`build_pin_roles.py` は廃止
- **分片（型番／family ごとに切ったコピー）は作らない**。案では GitHub で表として読めるように切る予定だったが、CSV は機械が読むもので人は viewer（`pins.html`）で絞ればよく、正本の重複を持つ理由が無い（実施中に判断）
- `index/registers.csv` は配列の要素（`EXTICR[1]`）に offset を付ける（先頭＋添字×幅）
