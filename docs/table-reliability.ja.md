...
| conflicts（`index/`） | 200 | 元の行を引き継ぐ（全行が `conflict`） | **証拠の `conflict` 行の数と一致すること**・`field` がその表に実在する列であること | 資料が食い違っている箇所を1表に集めたもの。`basis` の DSL から「どの出所が異を唱えるか」（`!<source>`）と「その出所は何と言うか」（`(=<value>)`）を取り出す。**125行に相手の値が入り、75行は空**——`memory_configs` の67行と `timers` の1行は食い違いを散文で記録していて DSL に持たないため（空欄自体が「evidence/README を読め」の意味）、残りは**相手が値を書かない型**（新しいX315 zh版がFlash時間のmaxを載せない等）。`product_attributes` の25行は**言い回しの差が混じる**（`Typical: 72MHz` と `Typ. 72MHz`）ので、仕様の食い違いと同一視しないこと | ✅ |
