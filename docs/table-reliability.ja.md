...
| テーブル | 行数 | confidence | 検査 | 既知の穴 | 総合 |
|---|---:|---|---|---|---|
| pinout（旧 pin_roles） | 24,947（機能行 24,231＋機能の無い lead 716） | 元の行を引き継ぐ | **pin_functions に無い行が入れば失敗**・語彙の穴は**0であることを検査** | **覆い100%**（2026-08-25。最後の26種を原典で所属確認して語彙へ）。pin_functions の conflict 12 を引き継ぐ。`port`/`pin` は alias からも埋まる | ✅ |
