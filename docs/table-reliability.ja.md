...
| errata | 21 | 列ごと | 結合・scan_errata で増分監視 | curated（人手）。両版のページ番号は照合済み。**2026-09-16、`CH32X035DS0.en` V2.3 で `x035-adc-ch-i2c-unavailable` の英語の言い回しが変わった**（`I2C function is not available` → `not applicable`）。`match` の正規表現に `applicable` を足し、同時に scan が挙げた en p.8（ADC の章。中文版 p.8 と対応）を `source_en` に足して 3 → 4 ページにした——中文版と件数が揃った | ✅ |
...
| テーブル | 行数 | confidence | 検査 | 既知の穴 | 総合 |
|---|---:|---|---|---|---|
| pinout（旧 pin_roles） | 25,207（機能行 24,489＋機能の無い lead 718） | 元の行を引き継ぐ | **pin_functions に無い行が入れば失敗**・語彙の穴は**0であることを検査** | **覆い100%**（2026-08-25。最後の26種を原典で所属確認して語彙へ）。pin_functions の conflict 35 を引き継ぐ。`port`/`pin` は alias からも埋まる | ✅ |
